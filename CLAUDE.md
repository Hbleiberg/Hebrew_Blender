...
| Accounts, sign-in, the header account chip, `js/supabase-config.js`, `js/ivrit-account.js`, cloud saves and `js/ivrit-saves.js`, Font Maker cloud projects and `js/ivrit-projects.js`, `db/migrations/` | `docs/reference/accounts-and-cloud.md`, `db/README.md` |
...
2. **Only `js/supabase-config.js`, `js/ivrit-account.js`, `js/ivrit-saves.js` and `js/ivrit-projects.js` (Font Maker projects, loaded by that page alone) talk to Supabase** — a tool page adds the `<script>` tags and a few lines of wiring, never its own auth or SDK code (one copy to audit, one place to switch off).

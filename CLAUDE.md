...
- Starting Fonts (`?start=<id>`): intake only through the `/addOSFont` skill (discovery comes from the weekly `scripts/audit-os-fonts.mjs` report in `starting-fonts/AUDIT.md`, which never stages anything); never hand-edit `starting-fonts/manifest.json`, a staged `LICENSE.txt`, or `LINKS.md`; `project.osFont` presence = license-locked, and partner exports never phrase the IvritSuite line as required.

...
- **iOS** uses them via the `apple-touch-startup-image` `<link>` tags in
  `index.html`'s `<head>` (alongside the `apple-mobile-web-app-capable` meta).
  The full tag block is mirrored in `splash/apple-startup-links.html`.
...
3. Only if `DEVICES` changed (NOT for a plain version bump): re-sync the
   `apple-touch-startup-image` block in `index.html` from
   `splash/apple-startup-links.html`. A version bump alone keeps the same
   filenames, so `index.html` needs no change.
4. Commit & push.

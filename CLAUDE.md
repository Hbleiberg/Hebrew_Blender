...
2. **Only `js/supabase-config.js`, `js/ivrit-account.js`, `js/ivrit-saves.js` and `js/ivrit-projects.js` (Font Maker projects, loaded by that page and the account page) talk to Supabase** — a tool page adds the `<script>` tags and a few lines of wiring, never its own auth or SDK code (one copy to audit, one place to switch off).

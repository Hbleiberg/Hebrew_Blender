...
- [ ] P4 (**NEW S397 Pass F — measured, and the one page where the header is not the first thing a keyboard reaches**) | hebrew_dictionary.html | **The floating `#fsBtn` (Toggle fullscreen) sits before `<header>` in DOM order, so it was tab stop 1 on the page and is now stop 2, ahead of the whole header.** Every other page puts its fullscreen control inside the header or after the content. Converging it is an F-shaped move (one line of markup), but the button is `position:fixed` and its place in the source may be load-order-sensitive — check before moving. | found S397
- [ ] P4 (**NEW S397 Pass F — found while measuring the tour outlines; L's arm, not F's**) | flash_cards.html | **The rendered outline is `h1, h3×8, h4` — no h2 at all, so the page skips a level independently of the tour card.** Static source carries three `<h1>` tags but only one renders, so a source grep misreads this page twice over. | found S397
...
- [ ] P4 (**RE-MEASURED S397 Pass F — the S354 framing was wrong on 3 of its 7 carriers; maintainer answered the gate, so this is now a 4-page job, NOT a ×7 sweep. Receipts in `loop-findings.md`; do not re-derive from a grep**) | hebrew_blend_generator.html, hebrew_dictionary.html, classroom_dashboard.html, torah_trainer.html | **The tour card's `<h4 id="tourCardTitle">` skips a level on these four only** (DOM outlines: generator `h1,h2,h4`; dictionary `h1,h4` — two levels; dashboard `h1,h2,h4`; torah `h1,h2,h4`). Flash cards, Font Maker and trope all render an h3, so theirs do not skip, and the Font Maker's is one of seven h4s. Fix = `h2` + a class, `.tour-card h4` re-pointed, on the four. | found S354, re-measured S397
...
- [x] 2026-09-17 | (S395 close-out) | branch/deploy note | **S395 CONTINUED `claude/loving-einstein-cdmbbz` (PR #246 open, unmerged, draft, `mergeable_state: clean`, base `b304db0` = `origin/main`, read through the API not assumed) — 4 commits + this close-out on top of S391–S394's 17; a human merges and confirms the Pages run.** **Drift: NONE a FOURTH consecutive session** — `origin/main` still …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-17 | (S395 iter 5) | Hebrew_Font_Maker.html | **The S389 VOID is closed — the account chip's phone geometry here is MEASURED CLEAN, so the candidate is struck rather than carried a seventh session.** S389 read "menu not open" in all 8 cells because the ungated onboarding wizard covers the header; trap 47's dismissal (remove `.open` from `#wizardOverlay`) clears it. 320 and 390 × …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-17 | `04892f3` | hebrew_blend_generator.html | **Three shadowed CSS declarations deleted** (S395, P4, `selector-declared-twice-in-one-stylesheet`'s 2nd fix and its first on the SHADOWED class). `.main-toolbar` had two full blocks under two "MAIN TOOLBAR" headings 380 lines apart with the same nine properties; `body.dark header p` was named in a grouped h1/p rule and again on the …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-17 | `dd2aac6` | classroom_dashboard.html | **The first-run card answers back when 📍 Current Location is refused** (S395 Pass H, P3). The setup card a new teacher meets on load delegates to the drawer's handlers, and both of that handler's answers address the drawer by id: the refusal note went to `#locNote` **behind the card's own backdrop**, and the busy state to …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-17 | `3e40f28` | classroom_dashboard.html | **A refresh mid-typing no longer eats the student roster** (S395 Pass H, P3 — the pass's own find). `commitRosterFromTextarea()` had one `blur` listener and the `input` listener only redrew the "N names" count, so the count said 5 while nothing was stored. The page already owns the fix two lines below: the `visibilitychange`+`pagehide` …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | (S394 close-out) | branch/deploy note | **S394 CONTINUED `claude/loving-einstein-cdmbbz` (PR #246 open, unmerged, draft, `mergeable_state: clean`, base `b304db0` = `origin/main`, read through the API not assumed) — 4 fix commits + this close-out on top of S391/S392/S393's 12; a human merges and confirms the Pages run.** **Drift: NONE a THIRD consecutive session** — …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | `d14c4f9` | Hebrew_Font_Maker.html | **The mode toggle's focus ring stops being invisible** (S394, P3, a11y). `.mode-btn:focus-visible` is declared twice — its own rule at `outline-offset:-2px` ("parity with `.npnl-mode-btn`") and, 600 lines later, the shared 2px-gold ring group at `+2px`, which wins; `.mode-toggle` is `overflow:hidden` with the segments filling it, so the …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | `7de7842` | hebrew_blend_generator.html | **The Self-Check flip-card rules are declared once, not twice** (S394, P4, first carrier of the new `selector-declared-twice-in-one-stylesheet` row). Two byte-identical copies 200 lines apart are why `a05e1cf` and `ec8bace` each had to be applied in two places. Proved a no-op: every computed property of all four flip elements, at rest …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | `a3d288d` | js/ivrit-account.js | **The account chip's menu re-clamps when the window or the device turns** (S394, P3; closes the S389 measured-not-driven candidate). `clampMenu()` ran only on open and on a re-render; the module had no `resize`/`orientationchange` listener at all. Opening wide then narrowing, 8 pages × EN/HE × 2 moves: **15 of 32 cells left the viewport — the …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | `767064e` | torah_trainer.html | **A locked Copy All stops lighting up on hover** (S394, P4) — **the LAST open carrier of `hover-feedback-survives-the-disabled-state`, closed.** The missing step S392 could not find is `#ttCopyBulkCb`, the Copy bar's "Select verses" toggle. Locked `#ttCopyAllBtn` stepped gold→gold-light on background and border, a delta IDENTICAL to the same …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | (S393 close-out) | branch/deploy note | **S393 CONTINUED `claude/loving-einstein-cdmbbz` (PR #246 open, unmerged, draft, `mergeable_state: clean`, base `b304db0` = `origin/main`, verified through the API not assumed) — 3 fix commits + this close-out on top of S391/S392's 8; a human merges and confirms the Pages run.** **Drift: NONE for a second consecutive session** — …[full …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | `ec8bace` | hebrew_blend_generator.html | **The bingo card number stops printing white on gold** (S393, P3). "CARD #1" is 11.2px bold white on `#c9922a` = **2.75:1** against a 4.5 floor, in both themes, and the only failing cell on the card (column letters and Hebrew cells both 14.81:1). Navy instead — the `.beta-tag` idiom this page already uses for small bold text on gold — …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | (S393 Pass D iter 4) | hebrew_blend_generator.html | **REFUTATION: "Header Labels does nothing on Word Search and Crossword" (S392) is wrong — struck, not fixed.** Both drills honour it; the S392 probe sampled before the debounce fired. `scheduleLiveSync()` waits **`liveIsHeavyMode() ? 1000 : 500`** ms, and word search and crossword are exactly the heavy modes. At 900ms: 14 …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | `12c170e` | hebrew_blend_generator.html | **Answer keys and bingo cards stop printing in ink the dialog drops** (S393, P3 — S392's top ungated candidate). Chrome's print dialog ships "Background graphics" OFF, so a background-color plate is never painted: `.answer-key-banner` printed **1.5:1 gold on white on all NINE answer keys**, `.bingo-card-header span` **1:1** (the column …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | `d285c88` | flash_cards.html | **A pasted share code can no longer script the results screen** (S393, **P1 security**, surfaced by Pass D). The end-of-session per-card list interpolated a card's Hebrew face and emoji straight into `innerHTML` while the transliteration on the same line went through `esc()`. In Words mode that face is not built-in content: a share code embeds its …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | (S392 close-out) | branch/deploy note | **S392 CONTINUED `claude/loving-einstein-cdmbbz` (PR #246 open, unmerged, draft, `mergeable_state: clean`; base `b304db0` = `origin/main`) — 2 fix commits + this close-out on top of S391's 5; a human merges and confirms the Pages run.** **Drift: NONE — `origin/main` still `b304db0` with nothing on it this branch lacks; the clone arrived …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | `a05e1cf` | hebrew_blend_generator.html | **Self-Check cards stay inside their own cell on paper** (S392 Pass G, P2). `.rw-flip-front/.rw-flip-back` had no `box-sizing`, so `width/height:100%` + 10px padding + 1.5px border made every face 23px bigger than its grid cell: each one covered its neighbour's part-of-speech tag ("PROPER NOUI", "ADJECTIVI", "VERI" on all 30 cards) and …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | `bf5126d` | hebrew_blend_generator.html | **Page Size and Header Labels stop wiping a Real Words sheet** (S392 Pass G, P2). Both print-layout controls — the two a teacher touches right before printing — called `renderWorksheet(false)`, which only knows `blends[]`, empty in Real Words mode, so the sheet was replaced by an empty "Hebrew Blends". Both now call the page's own …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S396 (2026-09-17):** account **2026-09-17 (`b15ef0e`, `ffe9d9a`; C S396 — its first dedicated pass of ANY kind; still never M, never H, never D, never G)**; js/ivrit-account **2026-09-17 (`3cc3ae5`; P S390, N S389)**; classroom_dashboard 2026-09-17 (`3e40f28`, `dd2aac6`; H S395, K S388, C S354, **D S325**); hebrew_blend_generator 2026-09-17 (`04892f3`; G S392, A S385, **D S338 — D-next**); Hebrew_Font_Maker 2026-09-17 (measured; H S380, C S368); torah_trainer 2026-09-16 (`767064e`; I S394, N S389, **C S298 — C-next after the dictionary**); …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`selector-declared-twice-in-one-stylesheet`** (**registered 2026-09-16 (S394); swept 2026-09-17 (S395) — 3 hits fixed `04892f3`, detector 16 → 13 rows, 0 clean sweeps yet**): ACTIVE. One file declares the same selector twice in the same at-rule context with OVERLAPPING …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`physical-property-that-never-mirrors`** (**NEW, registered 2026-09-16 (S388 Pass K) — the shape has now surfaced in THREE sessions (S248, S376, S388), which is what makes it recurring rather than incidental**): ACTIVE. **S397 (Pass F): 2 carriers FIXED (`1348930`) — the `.skip-link` rule's `left:8px` on torah + trope, converged to `inset-inline-start:8px` as that rule was propagated to the 12 pages that had no skip link (measured on torah at 1280px: `he` logical start 1168px → 8px; `en` unchanged at 8px). The 12 new carriers were authored logical, so the sweep closed the shape rather than multiplying it. Hits 2, clean streak 0.** A physical horizontal CSS property …[full text: …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`light-literal-text-on-the-gold-token`** (**NEW, registered 2026-09-17 (S396) — 2 carriers in 2 sessions, both fixed; on 2 rather than 3 because the pair and the remedy are identical and the detection is mechanical**): `color:#fff` (or another light literal) painted on …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S397):** **BRANCH/PR: S396 CONTINUED `claude/loving-einstein-cdmbbz` — PR #246 was open, unmerged and draft at session start (read through the API, not assumed), so the skill's continue rule applied. 3 fix commits + close-out on top of S391–S395's 21. #246 was then MERGED (merge commit) on the maintainer's in-session authorization, after an adversarial review of the whole branch and its follow-up — 3 fixes (`b4e25b1`, `dbba0eb`, `4f7b1b6`) + a close-out — so S397 cuts a FRESH branch off latest `origin/main`; never stack on merged history.** **/!\ DRIFT CHECK: NONE, a FIFTH consecutive session — `origin/main` still `b304db0`, clone NOT shallow (3115 commits), migrations 0001–0003 recorded live (3 records = 3 files), advisors the one inert `auth_leaked_password_protection` lint, `delete-account` ACTIVE with `verify_jwt: true`, keep-alive's last SCHEDULED run green. CHECK, never assume either way (S391's clone arrived SHALLOW and read 65 commits stale).** After the review follow-up: `sw.js` **v772**, FM **5.45**, SDK pin 2.116.0. Never trust these numbers unread.

**⚑ /!\ RE-DERIVE THE POINTER'S REASON, DO NOT INHERIT IT. Two sessions running, this pointer has justified a defensible choice with a wrong fact** — S395's said the dashboard's last H was S46 (it was S107 → S278), and S396's offered `trope_tutor.html` as a tool C had never had (it has had three: S113 → S213 → S312). Both choices survived re-derivation, but only because it was done. Per-tool history lives in `docs/IMPROVEMENT_ARCHIVE.md`, not in the rotation row, which keeps only the last run.

**⚑ HARNESS. The `--sdk` fixture: `npm pack @supabase/supabase-js@2.116.0`, untar, pass `package/dist/umd/supabase.js` (**npm IS reachable; the CDNs are not**). smoke-account / smoke-saves / smoke-tools hardcode 8080, smoke-account-page 8083 — **kill your own server FIRST, by PID. Never `pkill -f` a server: it matches and kills this shell.** A backend-file change is verified by the matching smoke, never by the generic recipe. **Always `finally { ctx.close(); browser.close(); }`** (trap 54). `c396/lib.mjs` is a ready C harness on port 8096: the account-page smoke's fake cloud plus `openContext`/`openPage`/`CELLS`, with a `delayDelete` hook for in-flight states.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), F (S383), B (S384), A (S385), E (S386), L (S387), K (S388), N (S389), P (S390), M (S391), G (S392), D (S393), I (S394), H (S395), C (S396).** Unattended: take **F** (cross-tool consistency) — 13 sessions stale and now the longest unattended gap — and it has a **ready, measured affordance waiting: the keyboard bypass** (S396's census: 2 of 17 pages carry a working header skip link, the implementation and its CSV key already exist on both, and the dashboard's apparent third is a false positive). Then **B**, **A (its 2nd sweep of `selector-declared-twice-in-one-stylesheet` — run `i394/dupcss2.py`, do not re-derive; ALSO the first sweep of the new `light-literal-text-on-the-gold-token` row, which has never been swept beyond its 2 known carriers)**, E, L, K, N, P, M (next: a chrome page never M-audited — `account.html`), G (next: `flash_cards.html`, S279), D (next: the **generator**, S338), H (next: **`flash_cards.html`, S264, the stalest H tool**) and C (next: **`hebrew_dictionary.html`, S285**, the stalest C of the audited tools — or one of `resources`/`contact`/`privacy`/`terms`/`404`, which have never had a dedicated pass of any kind).

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1–34 are S384–S392's, 35–40 S393's, 41–47 S394's, 48–54 S395's, 55–61 S396's — all of them in full in loop-findings, read them there first.** The S396 seven in one line each: **(55)** `page.screenshot({clip})` is **viewport-relative**: bank a rect at one Tab stop, shoot it after the walk has scrolled, and you compare the wrong pixels — which always differ, so every stop falsely reports a focus ring; shoot rest at each stop (`blur()` → shot → `focus()`), and a clip below the fold throws outright. **(56)** a pixel-diff ring detector is **blind on text inputs** (the caret changes the pixels either way) — read the computed outline. **(57)** `getComputedStyle(n).display` does not see a `display:none` **ancestor** — use `n.checkVisibility()`. **(58)** grepping a module's **filename** over-counts its carriers: five pages name `js/ivrit-account.js` only in the gtag comment; **11** load it. **(59)** WCAG 2.5.8 has a **spacing exception** — without it a size census reports false failures (5 of them on account.html). **(60)** `a[href^="#"]` matches a bare `href="#"` and `querySelector('#')` **throws**. **(61)** `body > header` is not the universal page shape, so "stops before content" is not comparable across pages.

**⚑ TOP UNGATED CANDIDATES (S396): the bench is thin and that is the honest state** — the 13 `selector-declared-twice` carriers still open are every one DIVERGENT or a documented judgement call, so the next A there is a census and an intent decision, not a fix list. The **keyboard bypass** (above) is the strongest genuinely-shippable item and it is F's. Then **the tour-card h4 → h2 ×7** (breaks the 2-tool cap — take it as an A sweep rather than splitting it; CLAUDE.md UX rule 1 also wants the tour engine extracted into a shared block next time one is touched). Held back deliberately: **account.html's `#holdsList` list-semantics** (Safari/VoiceOver only — Chromium cannot reproduce it, so it needs a WebKit engine before it ships, NOT an assertion); **the sub-floor touch targets** (generator `.bingo-step`, FM `#rulerCorner` — gate 3, a visible size change); **"sign out everywhere"** (new control + copy, gate 1/2); **the Edge Function's production `localhost` origins** (backend boundary — maintainer redeploy). `hover-feedback-survives-the-disabled-state` still has NO open carrier (closed S394).

**⚑ GATE-2 BENCH (unchanged): the generator's Header Labels translates only Name/Date** (~10 render sites; authoring, not wiring). Then: the flash-card swatch labels; the generator's `restoreLastSetup` corrupt-drop; the dictionary drag-freeze + select-all cap; the corpus's split article tokens; the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the resources Taamim fallback; the FM's `nexthint_nikkud` copy; **the 5 unreferenced credit keys (S360)**. Gate 3/M: the FM stage annotations; **`account.html`'s dark `.btn.ghost`/`.btn.danger` (S386 diagnosed it as specificity, not a leak — an M call)**; **`.dash-edit-pencil`'s side in Hebrew**; **the hub's seven-size 2.4px type band**. Gate 4: the flash-card sticky-bar and landscape proposals. **Seed bench: 26.**

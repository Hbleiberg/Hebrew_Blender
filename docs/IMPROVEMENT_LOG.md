...
**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1-20 are S384-S388's, written out in full in loop-findings — read them there first.** Cues: (1) theme from a themed token; (2) catch-all abort route FIRST; (3) no `requestfailed` for a 404; (4) never hand-roll a name walker; (5) census a dialog OPEN; (6) seed `hebrewDashboard_setupSeen`; (7) an undetected plant can mean a warn-only rule; (8) `ivritRestore` blocks on Merge; (9) match the attribute quote's kind; (10) LOAD sites only; (11) `url(…)` also matches JS `URL(`; (12) specificity is a measurement; (13) an inline-script `const` is SCRIPT-SCOPED; (14) a junk axis needs a truthy non-string; (15) walk `@graph`; (16) `pkill -f` kills the shell; (17) read BOTH quote styles and ALL files; (18) concatenated fallbacks truncate at the first literal; (19) measure the CONTROL before anything that switches views; (20) `I18n.t()`'s missing-key warning is gated on `dictLoaded`. **(21) NEW S389 — a busy loop inside `page.evaluate()` is NOT reported to a `longtask` observer (it runs as a CDP task): drive the control with a REAL tap on a button whose handler blocks, or the arm is blind. (22) NEW S389 — seed a page's first-run gates before tapping its header: `hebrewDashboard_setupSeen` hid 6 real hits behind 8 VOIDs, and the FM's ungated wizard still hides its chip. (23) NEW S389 — `documentElement.scrollWidth` is BLIND to an abspos popup leaving the viewport, in BOTH directions; only a per-element rect census sees it.**

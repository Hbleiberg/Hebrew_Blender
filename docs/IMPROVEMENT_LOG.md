...
- [x] 2026-09-11 | `e452c8f` | classroom_dashboard.html | **The projector picker's close button follows the inline end (S376 M candidate, P4, logical CSS):** `.pick-close` `right:20px` → `inset-inline-end:20px`. | verified: `d378/pickclose.mjs`, 8 cells EN/HE × light/dark × 1280/800 via `openPickOverlay()`: LTR unchanged (20px from the right), RTL 20px from the left (was the right); centre …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
- 2026-09-11 | **S379** | iters: 1 pass (**I**) + 4 fixes = **5** (full budget) | tools: index + generator + flash + dashboard (`4f9a2e7`), generator + dictionary + torah + trope (`f4a102d`), Hebrew_Font_Maker (`0346a0b`), classroom_dashboard (`de49066`) | patterns fixed: …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S379 (2026-09-11):** hebrew_dictionary 2026-09-11 (`f4a102d`; D S378); classroom_dashboard **2026-09-11 (`de49066`; G S364)**; torah_trainer 2026-09-11 (`f4a102d`); trope_tutor 2026-09-11 (`f4a102d`; M S376); flash_cards 2026-09-11 (`4f9a2e7`; N S361); Hebrew_Font_Maker **2026-09-11 (`0346a0b`; C S368)**; resources 2026-09-11 (`b1c0655`; N S375); privacy 2026-09-11 (`f8f23e0`; S360); hebrew_blend_generator 2026-09-11 (`f4a102d`; H S363); index **2026-09-11 (`4f9a2e7`; S379)**; contact / terms / 404 2026-09-09 (S359–S361 chrome).
...
**Next session (S380):** **BRANCH/PR: S369–S379 ran on `claude/improve-loop-swgwqx` (cut at `66dd97b` = `origin/main`; 55 commits at this close-out) → draft PR #229. If it is still OPEN and unmerged, continue on it; if MERGED, cut a fresh `claude/*` off latest `origin/main` — never stack on merged history. Verify via the API** (0 check runs is correct — no workflows). **⚑ DRIFT CHECK: compare `origin/main` against `66dd97b`; anything beyond THIS loop's commits is an outside-loop landing. `sw.js` v704, `FONT_MAKER_VERSION` 5.39** — re-read both. A fresh container may start shallow — `git rev-parse --is-shallow-repository` before `update-sitemap.mjs`; the HTTP server on 8080 dies across a compaction — curl it first; the scratchpad keeps only `b370/`…`i379/` (older harness dirs are gone — rebuild from the receipts in loop-findings).

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), H (S367), C (S368), F (S369), B (S370), A (S371), E (S372), L (S373), K (S374), N (S375), M (S376), G (S377), D (S378), I (S379).** Unattended: take H (teacher walkthrough; its stalest tool is the Font Maker, S241 — role-play a real lesson end-to-end through the wizard on a fresh profile; frictions → P3 candidates, small affordances → Feature seeds; never re-log a struck seed). Then C (flash_cards, S271). **O next `torah_trainer.html`; F next the disabled-control look (the four-picker `.font-opt-attr` step SHIPPED S379); G next: re-derive per tool from the archive; M next `index.html` (S246) then torah; N next trope (S248) then index (S249); D next flash_cards or the Font Maker (re-derive from the archive); the next A sweeps `1d4c9cf..HEAD` with the range-setter arm on the Font Maker's 13 sliders + the torah/trope `_sliderNum` sites + the dashboard `applyZoom` (S379), the `vh-capped-sheet-without-dvh-twin` row (0 open; 3 clean sweeps retire it) and the `dark-print-shadow-slab` row; the next E re-runs the 17 arms over `408dbe4..HEAD`; the next L re-runs vs `4a1f186`; the next K re-runs over `4a1f186..HEAD` (seed the Infinity cell with the RAW `1e999` string — S379 measured the dashboard `zoomLevel` cell: "1e999" now → 100); the next I re-runs `i379/gates.mjs` + `arms.mjs` + `corrupt.mjs` (52 cells) over the then-delta.**

**⚑ Harness (standing): FONT MAKER probes pass `dismiss:false` to `b370/harness.mjs` `open()`; on a PHONE descriptor `#mobileWarnOverlay` opens INSTEAD of the wizard — call `dismissMobileWarn(false)` then wait `#wizardOverlay.open`; enter via `#wizName`/`#wizAuthor` → `#wizNext` ×2 → `wizardCreate()`; Load menu `#wizOpenProject`; anchors panel = a letter `.lchip` then `gotoAnchors('nikkud')` / `setAnchorClass(k)`, precomposed forms `setPrecompTarget(t)`; torah stubs Sefaria with the b370 `SEF` shape; hidden toggles → click `label.toggle:has(#id)`; dashboard: `frSkip()` BEFORE any header click, `openPickOverlay()` + `renderGroupCards([[…]])`, the week editor via `openWeekEditor()`; flash dialogs: `chooseProfileModal()` (never await it), `openWeaknessView(name)`, `openLadderMap()` after `createProfile(name)` + `renderProfiles()`; dictionary: ready = `filtered.length>0` + `#loadingMsg` hidden, search = `#searchInput.value` + `onFilter()`, bulk = `#bulkModeCb` (onchange), `wlAddList()` → `{ok,id}` → `wlLoadListAsFilter(id)`, `wlOpenManager()`; trope drill = `setMode('drill')` then `#drillStartBtn`; PRINT `g377/`; PERF `d378/lib.mjs`; a delay `page.route` goes AFTER the catch-all; `waitForFunction` polling is a number; `addInitScript` re-seeds on every navigation (seed via a prior page); contrast probes composite alpha; N `n375/mobile.mjs`; M `m376/audit.mjs`; screenshot paths are cwd-relative.**

**⚑ TOP UNGATED CANDIDATES (S379):** the tour-card h4 → h2 ×7 (it is the "next touch of a tour engine" — extract the shared block in the same session); the remaining sub-floor M calls (bingo stepper, scaled toggles, `#rulerCorner`); the `.rw-letter-tile .name` 7.2px grid (a layout decision). Gate 2: the dictionary drag-freeze and select-all preview-cap proposals (S378 D, numbers in loop-findings); the corpus's split article tokens (S377); the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on; the trope FAQ a7 copy; the four English-font picker headers; the resources preview stack's Taamim fallback (S353). Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. **Seed bench: 25 (unchanged; I is not an intake). Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

...
- [x] 2026-09-22 | `92dfe01` | trope_tutor.html + CSV + torah-and-trope.md | **After a Learn detour the Drill start screen repeats "N marks to review · Drill these marks"** (the S400 P3; maintainer gate 2 → the start-screen shape): `#drillStartReview` in the results box's style, filled by `updateDrillStart()` while `_lastMissed` holds marks; existing EN+HE strings (translator notes only), nothing …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S404 (2026-09-22):** all-page passes keep the *dedicated pass* the useful column. index (S403: `7552ba0` `6e28af3`; M S391, F S397); hebrew_blend_generator (S402 seeds; G S392, **D S338 — D-next**); flash_cards (D S393, **H S264 — the stalest H tool**, G S279); hebrew_dictionary (**C S285 — C-next**); classroom_dashboard (**S404: `4b7f9cc`**; S401 seeds; H S395); torah_trainer (**S404: `4b7f9cc`**; **O-next, never O-audited**; N S248); trope_tutor (**S404: `4b7f9cc` `92dfe01`**; S400 seeds; N S389, C S312); Hebrew_Font_Maker (S403: …[full text: IMPROVEMENT_ARCHIVE.md]
...
- L SEO & discoverability audit: 2026-09-22 (**S404 — 20th L, first since S387 (17 sessions); the stalest runnable pass (O blocked here). Delta `b5e1416..HEAD`: 150 commits, 15/17 pages moved, 1/17 crawler surfaces (trope's drill-length JSON-LD, accurate). 18 arm classes, every zero controlled: 0 hits. FOUND: 5 stale fallbacks (`4b7f9cc`, via the FAQ twin arm); torah/trope `og:image:alt` …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S405):** **BRANCH/PR: S403 + S404 sit on `claude/eager-bardeen-jdve4x` → draft PR #265 (S404 added `4b7f9cc` `92dfe01` `46959bf` `1d4943b` + its close-out). If #265 is open and unmerged, continue on it; if merged, cut a FRESH branch off latest `origin/main` and confirm that merge's Pages run concluded `success`.** **DRIFT vs S403: none** (`origin/main` still `041ddfa`). S404 closed at `sw.js` **v824**, FM **5.56**, SDK **2.116.0**; backend re-read LIVE in S404 (3 recorded migrations = 0001–0003, `delete-account` v3 `verify_jwt` on, advisors = the known leaked-password WARN only; keep-alive run #10 green). **Never trust these numbers unread.**

**⚑ STALEST PASS: O (S346 — BLOCKED here, S399 + S403), K (S388), N (S389), P (S390), M (S391), G (S392), D (S393), I (S394), H (S395), C (S396), F (S397), B (S398), A (S399), E (S403), L (S404).** Take **K** unless the maintainer says the O permission rule exists (then test ONE `node <detector> torah_trainer.html` first). K has two concrete targets: S403's 122 stale `leading-emoji` CSV notes, and S404's prose-glyph census — 201 `en` values carry a glyph mid-sentence, and prose that names a control still echoes its old emoji ("🔗 Copy link", "🖨️ Print", "🔁 Loop", "⚙ Settings") where the sweep dropped the "❓" from the Tour rows: decide one rule, then apply it (loop-findings `## S404` (2)). For a surface pass, the Font Maker's five glyph-set tabs are still the largest unaudited surface.

**⚑ READY TO SHIP, ungated: the two S404 candidates at the top.** The torah/trope `og:image:alt` → `IvritSuite — Free Hebrew Literacy Tools` (the maintainer answered gate 2 in S404: match the image), and the Trope Tutor's `🎯 Drill` tab (the icon sweep's one missed tab; the set has no target glyph, so pick one). Both touch `trope_tutor.html` — exactly its 2-per-session cap, so take them in one session or split them.

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1–73 in loop-findings; S404 adds 74–77 there** — **(74)** the visible FAQ NESTS one `<details>` per question inside a container `<details>`: a regex from `<details>`/`<summary>` to `</details>` swallows the first question (N−1 everywhere, met twice); **(75)** FAQ question equality is the wrong contract (short visible label vs keyword-rich JSON-LD); **(76)** a stale-fallback detector must read `data-i18n-html` and compare it as MARKUP (an href-only drift is invisible to text); **(77)** the dark key is `'1'`/`'0'` — a probe writing `'true'` leaves torah/trope light. **Also standing: `compact-ledger.mjs --apply` is NOT idempotent — run it ONCE, after writing the new pointer; the outgoing handoff is moved by hand.**

**⚑ AFTER THOSE:** S403's CSV notes (K's) and the My Fonts 24px icon buttons (C's), then the older bench — the flash/FM/trope heading-level trio (a structure call), account's `#holdsList` (needs WebKit). **Gate bench unchanged:** gate 2 — the generator's Header Labels translating only Name/Date; gate 3 — the FM stage annotations, account's dark `.btn.ghost`/`.btn.danger`, `.dash-edit-pencil`'s side in Hebrew, the hub's 2.4px type band; gate 4 — the flash-card sticky bar + landscape. **Seed bench: 12**, unchanged (dashboard's week-editor undo is the strongest).

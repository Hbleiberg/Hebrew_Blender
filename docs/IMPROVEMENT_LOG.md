...
- [x] 2026-09-08 | `7c6337a` | flash_cards.html | **Dark-mode selected `.pill-group .pill.active` gets `--navy-deep` + an inset 1px gold hairline (S346 Pass O candidate, P4, M's dark-mode parity).** One `body.dark` rule after the pill block, converging on the `.emoji-gender-btn.active` deeper-fill idiom. | verified: measured before/after at 1280/800 both themes, 0 pageerrors — light …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S347 (2026-09-08):** index **2026-09-08 (×1, `72104a2` last-backup line; S341 ×2)**; flash_cards **2026-09-08 (×1, `7c6337a` dark pill; S346 ×1, S344 ×1, S343 ×1)**; classroom_dashboard 2026-09-08 (S345 ×1, `4e25247`; S343 ×2); hebrew_blend_generator 2026-09-08 (S344 ×1, `608aa28`; S342 ×1); resources 2026-09-04 (S341 ×1, `39cf34a`); trope_tutor 2026-09-04 (S339 ×1, `fea54f5`); Hebrew_Font_Maker 2026-09-04 (S338 ×1, `fc73fd9`; outside-loop v5.39); torah_trainer 2026-09-04 (`f92ae1d`); hebrew_dictionary 2026-09-03 (`ecd0720`); contact / …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S348):** **BRANCH/PR: S347 CONTINUED `claude/improveloop-deslop-je9vs9` → draft PR #225 (base `origin/main` `aebb86b`). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v671, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS N (S334), THEN M (S335), H (S336), G (S337), D (S338), I (S339), C (S340), F (S341), B (S342), A (S343), E (S344), L (S345), O (S346), K (S347).** Take N (mobile & touch, one surface): re-derive the least-recently-N'd surface from the archive's N rows (torah_trainer was S334; the hub, dictionary, generator, dashboard, flash cards, Font Maker, Trope Tutor and the chrome pages each have an earlier N) — real device descriptors, SW blocked, the seven arms, and the phone contact sheet even when clean. **O is attended-only; its next surface is `torah_trainer.html`.** **K owes the next run its two skipped arms** (runtime HE census on the changed pages; whole-corpus plain-argument probe) — the S347 row says so. O harness (standing): the JS detector is the `skill-v4.1.3` tag; recipe in `docs/reference/deslop-detector.md`; harness notes in the archive under the S346 prior handoff. **Ledger lesson (standing):** write open candidates without the `NON_ACTIONABLE` phrases; run `compact-ledger --apply` ONCE.

**⚑ NO UNGATED CANDIDATE IS OPEN** (S347 took the S346 dark-pill fill). The S285 tooltip-binding divergence (P4) stays filed for F. Every other candidate is a gate or a decision (the picker terms, hub author URL, PDF paper choice, 7.2px grid, help tab strip, timer placeholder copy, Save-as-Word-List entry — each marked in its candidate). A session whose pass comes back clean ends early — precedent S340/S342/S343/S345/S346. **Seed bench:** the holiday share link (torah, S336 — ~15 lines, the smallest seed), the handout selection (torah, S336), the scroll-style handout (torah, S324), the erase-offers-backup and tool-card data-indicator seeds (index) — the last-backup date SHIPPED S347 (`72104a2`).

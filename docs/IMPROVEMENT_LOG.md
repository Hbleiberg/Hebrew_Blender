...
- [ ] P4 (**NEW S346 Pass O — filed to M: no detector rule id fired, a dark-mode parity refinement**) | flash_cards.html | **In dark mode the selected `.pill-group .pill.active` fill is nearly invisible: `var(--navy)` computes to rgb(15,22,38) on the group's rgb(20,25,42), so the selected state rides on gold text alone** (light: navy on cream, a strong cue; measured 2026-09-08 on "2 Letter"). A …[full text: IMPROVEMENT_ARCHIVE.md]
...
- 2026-09-08 | **S345** | iters: 1 pass (**L**) + 2 fixes = **3** (4th/5th unspent — L clean, the one ungated candidate taken, every other open candidate gated or a decision) | tools touched: classroom_dashboard ×1 (`4e25247`), docs ×1 (`101cf5b` loop-findings) | patterns fixed: …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S346 (2026-09-08):** flash_cards **2026-09-08 (×1, `66386bb` tour-card waiver; S344 ×1, S343 ×1)**; classroom_dashboard 2026-09-08 (S345 ×1, `4e25247`; S343 ×2); hebrew_blend_generator 2026-09-08 (S344 ×1, `608aa28`; S342 ×1); resources 2026-09-04 (S341 ×1, `39cf34a`); index 2026-09-04 (S341 ×2, `ccd4898` + `9068087`); trope_tutor 2026-09-04 (S339 ×1, `fea54f5`); Hebrew_Font_Maker 2026-09-04 (S338 ×1, `fc73fd9`; outside-loop v5.39); torah_trainer 2026-09-04 (`f92ae1d`); hebrew_dictionary 2026-09-03 (`ecd0720`); contact / privacy / terms / …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S347):** **BRANCH/PR: S346 opened NEW `claude/improveloop-deslop-je9vs9` → draft PR #225 (base `origin/main` `aebb86b` = the merged PR #224's head; PR #224 was merged before S346 started). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v670, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS K (S333), THEN N (S334), M (S335), H (S336), G (S337), D (S338), I (S339), C (S340), F (S341), B (S342), A (S343), E (S344), L (S345), O (S346).** Take K (i18n: the four `check-i18n` gates first, then the blind-spot arms on the static delta since the S333 close-out — grep the archive for the S333 K entry's three arms; the template-literal and `setAttribute`-ternary shapes escape the gate; the 18 baselined Check-A findings are the burndown). **O is attended-only; its next surface is `torah_trainer.html` (never O-audited; then trope_tutor, resources, the chrome pages).** O harness (standing): the JS detector is the `skill-v4.1.3` tag (upstream HEAD is a binary the sandbox cannot fetch — recipe in `docs/reference/deslop-detector.md`); hidden screens/modals need held-open copies + a Playwright open-assertion + a planted-tell control; the URL arm ignores comment waivers. **Harness notes (standing):** the full list is in the archive under the S346 prior handoff — CSP forbids string predicates; settle 900 ms after `classList.add('dark')`; a `cd` in a chained Bash persists (use absolute paths); the HTTP server does not survive a compaction. **Ledger lesson (standing):** write open candidates without the `NON_ACTIONABLE` phrases; run `compact-ledger --apply` ONCE.

**⚑ ONE UNGATED CANDIDATE IS OPEN: the S346 dark-mode selected-pill fill on flash cards (P4, M's — a one-rule `body.dark .pill-group .pill.active` change; verify light unchanged + dark cue visible at 1280/800).** The S285 tooltip-binding divergence (P4) stays filed for F. Every other candidate is a gate or a decision (the picker terms, hub author URL, PDF paper choice, 7.2px grid, help tab strip, timer placeholder copy, Save-as-Word-List entry — each marked in its candidate). A session whose pass comes back clean ends early — precedent S340/S342/S343/S345/S346. **Seed bench (unchanged):** the holiday share link (torah, S336 — ~15 lines, the smallest seed), the handout selection (torah, S336), the scroll-style handout (torah, S324), the last-backup date (index, S323).

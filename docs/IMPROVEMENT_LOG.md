...
- [ ] P4 (**NEW S376 Pass M — logical-CSS rule; one declaration**) | classroom_dashboard.html | **The projector overlay's `.pick-close` is pinned with `right:20px` (physical); the dictionary's `.wm-close` uses `inset-inline-end`.** Under the Hebrew UI the × stays at the right instead of the inline end; swap to `inset-inline-end:20px` and verify the × hit-testable in EN + HE. | found S376
...
- [x] 2026-09-11 | (S376 close-out) | branch/deploy note | **S376 CONTINUED `claude/improve-loop-swgwqx` (PR #229 open, unmerged; base `66dd97b` = `origin/main`), 4 commits + this close-out; a human merges and confirms the Pages run. Drift: none (`66dd97b`, sw v700, FM 5.39 — as the S375 close-out recorded). sw v700→v701 (trope, dictionary, dashboard); FM NOT bumped (untouched). Scripts: …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-11 | `16f2d18` | trope_tutor.html | **The footer FAQ wrapper's `text-align:left` → `start`** — under the Hebrew UI the seven questions sat at the LEFT edge of the 640px box and each answer's last line was ragged on the wrong side (logical-CSS rule; the S376 M hit) | `m376/verify-trope.mjs` with a BEFORE control: HE summary x 320→831 of a 320–960 box, last line now ends at 960 (was …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-11 | `b1c0655` | resources.html | **`.sg-modal` caps at `max-height:90vh;max-height:90dvh` (the hub's `.ie-modal` pair) — the Suggest and Preview sheets no longer overflow a phone backdrop with the URL bar expanded** (`vh-capped-sheet-without-dvh-twin`, registered S375) | `n375/dvh.mjs`: the cascade resolves to `90dvh` (CSSOM), computed max-height unchanged headless …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S377):** **BRANCH/PR: S369–S376 ran on `claude/improve-loop-swgwqx` (cut at `66dd97b` = `origin/main`; 40 commits) → draft PR #229. If it is still OPEN and unmerged, continue on it; if MERGED, cut a fresh `claude/*` off latest `origin/main` — never stack on merged history. Verify via the API** (0 check runs is correct — no workflows). **⚑ DRIFT CHECK: compare `origin/main` against `66dd97b`; anything beyond THIS loop's 40 commits is an outside-loop landing. `sw.js` v701, `FONT_MAKER_VERSION` 5.39** — re-read both. A fresh container starts shallow again — `git fetch --unshallow` before `update-sitemap.mjs`; the HTTP server on 8080 dies across a compaction — curl it first.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), G (S364), D (S365), I (S366), H (S367), C (S368), F (S369), B (S370), A (S371), E (S372), L (S373), K (S374), N (S375), M (S376).** Unattended: take G (print & export) over `hebrew_dictionary.html` (S252, its stalest surface; the word-list export, the bulk list, the print sheets and the `.ivrit` file all moved since) on Letter AND A4, both themes, inspecting every exported file; divergences are P2. Then D over the dictionary (S253). **O next `torah_trainer.html`; H next Font Maker (S241); C next flash_cards (S271); F next the disabled-control look, then the footer-nav header step (S376); M next `index.html` (S246) then torah (S334-era); N next trope (S248) then index (S249); the next A sweeps `1d4c9cf..HEAD` with the range-setter arm on the Font Maker's 13 sliders + the torah/trope `_sliderNum` sites + the `vh-capped-sheet-without-dvh-twin` row (7 open sites); the next I re-runs `i366/{gates,arms,corrupt,strshape}.mjs`; the next E re-runs the 17 arms over `408dbe4..HEAD`; the next L re-runs `l373/{audit,bytecheck}.mjs` vs `4a1f186`; the next K re-runs `k374/{static.py,he.mjs}` over `4a1f186..HEAD` (seed the Infinity cell with the RAW `1e999` string).**

**⚑ Harness (standing): FONT MAKER probes pass `dismiss:false` to `b370/harness.mjs` `open()` (its `.overlay.open` sweep closes the wizard); enter through the wizard (`#wizName`/`#wizAuthor` → `#wizNext` ×2 → `wizardCreate()`), Load menu = `#wizOpenProject` / the toolbar `toggleLoadMenu(event)`, seed recents with `pushRecentProject(project)`; a project file = `JSON.stringify(project)` + `format:'hebrew-font-maker-project'` via `page.setInputFiles('#projFile', {buffer})`; torah stubs Sefaria by fulfilling `**/*sefaria.org/**` (`harness.mjs` `SEF`); hidden toggle inputs → click `label.toggle:has(#id)`; dashboard: `frSkip()` BEFORE any header click, `openWeekEditor()`, `openSettings()`, `openPickOverlay()` + `renderGroupCards([[…]])` for the Group Maker; the generator's sidebar is a drawer at 390; the dictionary's collapsed bodies need a REAL click on Expand-all-menus, a `.word-card` click opens `#wordModal`; flash: the practice screen is `#screenCard.active`; trope: reset `drill.active=false` + `setMode('learn')` between views, the tour's `_tourActive` is not on `window`; probe screenshot paths are cwd-relative; junk-store cells: `context.addInitScript` writes the RAW key string; CSSOM walks must skip `CSSStyleRule.cssRules` (nesting) or every rule reads empty; a contrast probe MUST composite alpha up the ancestor chain; N harness: `n375/mobile.mjs` + `sheet.mjs`; M harness: `m376/audit.mjs` (type/radius/row/colour/contrast census × views) + `sheet.mjs`.**

**⚑ TOP UNGATED CANDIDATES (S376):** the 7 vh-capped sheets (flash ×3, FM ×4 — one declaration pair per site, ≤2 tools per session); the ~30 inline Font Maker hints under 11px; the tour-card h4 → h2 ×7; the four-picker `.font-opt-attr` step (one F change or a waiver); the footer-nav header step ×3 (F); the dashboard `.pick-close` physical pin; the remaining sub-floor M calls (bingo stepper, scaled toggles, `#rulerCorner`). Gate 2: the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on; the trope FAQ a7 copy; the four English-font picker headers. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. **Seed bench: 25 (unchanged; M is not an intake). Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

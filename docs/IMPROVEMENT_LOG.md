...
- [ ] P4 (**NEW S368 Pass C — pattern `sub-floor touch target`; the file is at the 2/tool cap this session**) | Hebrew_Font_Maker.html | **Two `.ctl-btn` buttons on the Spacing tab measure 41×22 and 37×22 (22px tall, the floor is 24) with no pseudo-element vehicle; the `.ctl-btn` class has no `min-height`.** Fix = `min-height:24px` on the class (check the draw-chips and node-panel pairs keep …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P4 (**NEW S368 Pass C — cross-carrier: a SHARED-BLOCK change, so it is A/F work, not a single-page fix**) | the `test-phrases` + `hebrew-keyboard` CSS blocks (font pages / 4 keyboard carriers) | **`.tp-label` (0.62rem = 9.92px, "Samples") and `.hk-subhead` (0.62rem) sit under the 11px floor inside byte-identical shared blocks.** Raise both to 0.7rem on EVERY carrier in one commit and …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-10 | (S368 close-out) | branch/deploy note | **PR #228 open (draft, clean, head `19a10ab`) → S368 CONTINUED `claude/awesome-bardeen-jpi6sn`, 4 commits; a human merges + confirms Pages. Drift: none (`0ff0989`, sw v692, FM 5.39). sw v692→v693 (2 pages); FM NOT bumped (a11y CSS fixes, no feature). check-i18n / check-inline-js / compact / check clean; no CSV change; no sitemap. …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-10 | `880c664` | Hebrew_Font_Maker.html | **The last sub-11px class rules for functional text → 0.7rem: fullscreen captions 8.32px, punctuation band labels 9.28px, the kern built-in tag 9.6px, the Allowed line 10.4px at 0.8 opacity (3.23:1), metric sub-labels, the metric hint, QA table heads, node-panel delete rows, brush-tile labels, guide rename, Load-menu heads, the Tricky-combos …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S368 (2026-09-10):** Hebrew_Font_Maker **2026-09-10 (`880c664`; C-audited S368)**; hebrew_blend_generator **2026-09-10 (`9fc75c7`; H-audited S363)**; trope_tutor 2026-09-10 (`92eadd0`; H-audited S367); classroom_dashboard 2026-09-10 (`5c7c796`; G-audited S364); hebrew_dictionary 2026-09-09 (`10be341`; M-audited S362); index 2026-09-09 (S360); contact / privacy / terms / 404 2026-09-09 (S360); flash_cards 2026-09-10 (`e54dd1a`; N-audited S361); resources 2026-09-09 (S356); torah_trainer 2026-09-09 (S355).
...
- **`sub-floor touch target`**: ACTIVE. **S368 (Pass C, Font Maker census): 2 NEW carriers logged (`.ctl-btn` 41×22 / 37×22 on the Spacing tab — cap-blocked), `.fld-help` 15×15 is CLEAN via its `::after` 24px box (measure pseudo-aware), `#rulerCorner` stays gate 3. S365: 2 FIXED …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S369):** **BRANCH/PR: S368 CONTINUED `claude/awesome-bardeen-jpi6sn` → draft PR #228 (base `origin/main` `0ff0989`; S367–S368 = 10 commits). Continue it if open; if merged, cut a fresh `claude/*` off latest `origin/main`. Verify via the API** (0 check runs is correct — no workflows). **Drift note: `sw.js` v693, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), F (S355), B (S356), A (S357), E (S358), L (S359), K (S360), N (S361), M (S362), G (S364), D (S365), I (S366), H (S367), C (S368).** Unattended: take F (cross-tool consistency) — affordance candidates never F-swept: the TEXT-SIZE FLOOR for functional labels across all 7 tools + chrome (the S309 O census listed generator ×14, flash ×9, dashboard ×9, dictionary ×9, resources ×5, torah ×3 sites; S316/S368 closed the Font Maker's class rules — converge on 0.7rem), or the disabled-control look (opacity 0.4/0.45 vs `aria-disabled` styles). **O next `torah_trainer.html`; D next dictionary (S253); G next dictionary (S252); H next Font Maker (S241); N next resources (S246); M next trope_tutor (S245); C next flash_cards (S271); the next I re-runs `i366/{gates,arms,corrupt,strshape}.mjs` and weights the S366–S368 fixes.**

**⚑ Harness (standing): torah stubs Sefaria by fulfilling `**/*sefaria.org/**` (`b356/harness.mjs`); hidden toggle inputs → click `label.toggle:has(#id)`; the HTTP server dies across a compaction — curl it first; pypdf needs the stub `cryptography` package (`d352/stub`); `page.pdf()` fires `afterprint` — re-wear a print-state class before EACH pdf; PyMuPDF is absent (`g364/paper.mjs` CENSUS is the slab detector); a hover read needs a REAL `page.mouse.move` + 400 ms, a focus read a real Tab (never `page.focus()`); open a probe's host by ITS path (dashboard: `openWeekEditor()`, dismiss `#frBackdrop` via `frSkip()`, expand collapsed panels by `.panel-title`; the generator's live-preview toggle is in the collapsed Advanced panel, presets CARRY `livePreview`, and `applySettings` rebuilds `#letterGrid` — re-query tile handles; FM: enter the workspace through the wizard (`#wizName`/`#wizAuthor` → `#wizNext` ×2 → `wizardCreate()`), Save Project ▾ = `[data-i18n="fontmaker.toolbar.save_project_menu"]`); an `elementFromPoint` receipt precedes a real click; modal probes read the LAST `[aria-modal]` whose computed `display` is not none; a `let` page global is not on `window` but IS reachable by bare name in `page.evaluate`; the harness auto-accepts dialogs — a confirm probe installs a dismissing handler; `page.addInitScript` re-runs on every navigation — seed a garbage key ONCE (`sessionStorage` flag); probe screenshot paths are cwd-relative (run from the scratchpad).**

**⚑ TOP UNGATED CANDIDATES (S368):** the Font Maker `.ctl-btn` 22px pair (sub-floor, one `min-height`); the shared `.tp-label`/`.hk-subhead` 0.62rem (an A/F shared-block commit across carriers, sha-verified); the ~30 inline Font Maker hints under 11px (one batch); the tour-card h4 → h2 ×7 (not small); the remaining sub-floor M calls (the generator's 33 `scale(0.75)` sub-option toggles, gate 3; the bingo ▲/▼ stepper; FM `#rulerCorner`). Gate 2: the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on (`g364/out/board-print-en-dark.png`); the trope FAQ a7 copy. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. The next A sweeps the S357–S368 delta with the print-hide-list arm, the wrong-shape arm (the dictionary's `wlReadStore` is now RECORDED as guarded — no store left unprobed), a `wired-then-clobbered label` re-sweep, and the sub-floor arm measured pseudo-aware; the next K greps every authored key literally. **Seed bench: 25 (unchanged; C is not an intake). Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

...
- **S349 (2026-09-08):** torah_trainer **2026-09-08 (×1, `c3d2e11` handout selection; S348 ×1, S334 ×3)**; 404 **2026-09-08 (×1, `5e64c95` bidi)**; index 2026-09-08 (S347 ×1, `72104a2`; S341 ×2); flash_cards 2026-09-08 (S347 ×1, `7c6337a`; S346 ×1); classroom_dashboard 2026-09-08 (S345 ×1, `4e25247`; S343 ×2); hebrew_blend_generator 2026-09-08 (S344 ×1, `608aa28`); resources 2026-09-04 (S341 ×1, `39cf34a`); trope_tutor 2026-09-04 (S339 ×1, `fea54f5`); Hebrew_Font_Maker 2026-09-04 (S338 ×1, `fc73fd9`; outside-loop v5.39); hebrew_dictionary …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S350):** **BRANCH/PR: S349 CONTINUED `claude/improveloop-deslop-je9vs9` → draft PR #225 (base `origin/main` `aebb86b`; S346–S349 on it). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v673, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS H (S336), THEN G (S337), D (S338), I (S339), C (S340), F (S341), B (S342), A (S343), E (S344), L (S345), O (S346), K (S347), N (S348), M (S349).** Take H (teacher walkthrough, one tool): re-derive the least-recently-H'd tool from the archive's H rows (torah was S336; the S336 row said the generator's last dedicated H was S251 — check dashboard, flash cards, dictionary, Font Maker, Trope Tutor and the hub against it) — fresh profile, role-play a real lesson end to end, frictions → P3 candidates, missing affordances → Feature seeds. **O is attended-only; its next surface is `torah_trainer.html`.** **K owes its next run two arms** (runtime HE census; whole-corpus plain-argument probe). **N's next surface is `flash_cards.html` (S244).** Harness (standing): O detector = the `skill-v4.1.3` tag; torah verification stubs Sefaria by fulfilling `**/*sefaria.org/**` with `{versions:[{language:'he',text:[[…]]}],sections:['c','v']}`; the page's toggle inputs are visually hidden — click `label.toggle:has(#id)`, not the input; the HTTP server does not survive a compaction. **Ledger lesson (standing):** write open candidates without the `NON_ACTIONABLE` phrases; run `compact-ledger --apply` ONCE.

**⚑ NO UNGATED CANDIDATE IS OPEN.** The S285 tooltip-binding divergence (P4) stays filed for F. Every other candidate is a gate or a decision (the picker terms, hub author URL, PDF paper choice, 7.2px grid, help tab strip, timer placeholder copy, Save-as-Word-List entry — each marked in its candidate). A session whose pass comes back clean ends early — precedent S340/S342/S343/S345/S346/S348. **Seed bench:** the scroll-style handout (torah, S324 — one more handout checkbox wired like `handoutHideNikkud`), the erase-offers-backup and tool-card data-indicator seeds (index), the submit-a-font form (resources, M), the dashboard per-day overrides (M) — SHIPPED: handout selection S349 (`c3d2e11`), holiday share link S348 (`4329006`), last-backup date S347 (`72104a2`).

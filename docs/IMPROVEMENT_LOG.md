...
- [ ] P3 (**NEW S392 Pass G — MEASURED with a positive AND a negative control; not shipped only because the variety governor had the generator at its 2-touch cap**) | hebrew_blend_generator.html | **With the print dialog's default "Background graphics" OFF, `.answer-key-banner` prints as 1.5:1 gold-on-white on all 9 answer-key shapes; bingo's `.bingo-card-header span` column letters print 1:1 …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P3 (**NEW S392 Pass G — measured on the stashed pre-fix tree, so it is NOT a regression from `bf5126d`**) | hebrew_blend_generator.html | **Header Labels = עברית does nothing to the Word Search and Crossword sheets — Name/Date stay English (`he:false en:true` before the click and after), 288 cells either way — while the other 16 printable shapes flip.** Those two builders never read …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-16 | `95fe5ec` | hebrew_blend_generator.html | **The locked Print and Save buttons stop lighting up** — 2 of the 3 carriers S388 left open for `hover-feedback-survives-the-disabled-state`, runtime-confirmed and closed. `#printBtn` (`.btn-secondary`, native `disabled`) was dimmed by `.btn[disabled]`, but that rule declares NO `background`, so the hover painted through it (light only …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-16 | (S389 close-out) | branch/deploy note | **S389 CONTINUED `claude/wizardly-tesla-xu0ikd` (PR #242 open, unmerged, draft; base `4d6c02d` = `origin/main`), 2 fix commits + this close-out on top of S382-S388's 31; a human merges and confirms the Pages run.** Drift: **none for a seventh consecutive session** — `origin/main` still `4d6c02d`, 0 commits this branch lacks, `sw.js` v755, …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | `ba34a28` | js/ivrit-saves.js | **The account screen's Backups note keeps its second half when the dictionary is missing** — the 4th and last of S388's fallback-drift axis. The hardcoded fallback stopped at "...on the home page." while the CSV `en` continues ": it is merged into what is on that device (per-device settings such as zoom and panel layout are not in it)" — the half …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | (S389 Pass N — mobile & touch-device, **17th-ever run**, `trope_tutor.html`, its 2nd since S248) | **N was the stalest unattended pass (O attended-only since S346) and the S389 pointer named both the pass and the surface.** Delta since S248: 56 commits, +518/-74 — the print chart, the drawer resize block, roving tabindex, the mastery grid, **and the whole accounts layer, which …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | (S388 close-out) | branch/deploy note | **S388 CONTINUED `claude/wizardly-tesla-xu0ikd` (PR #242 open, unmerged, draft; base `4d6c02d` = `origin/main`), 3 fix commits + this close-out on top of S382–S387's 27; a human merges and confirms the Pages run.** Drift: **none** — `origin/main` still `4d6c02d` with nothing on it this branch lacks, `sw.js` v754, FM 5.44, migrations …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | pass | suite-wide | **S388 Pass K (i18n audit), the 26th K and the first since S374 (14 sessions) — K's LARGEST delta ever (146 commits, 66 files, 25,891 insertions) and its FIRST sight of the accounts layer: 4 `js/` modules, `account.html`, 2 harnesses, none of which any prior K had read.** 5 gates clean (5,516 keys) — **and the zero is measured: 12 planted defects fired 12 of …[full text: IMPROVEMENT_ARCHIVE.md]

## Metrics

### Per-session log (one line per session)

- 2026-09-16 | **S392** | iters: 1 pass (**G**) + 2 fixes = **3** (4th/5th unspent: all 4 of G's defects are in one tool and the governor caps it at 2; the torah hover carrier stayed a VOID rather than ship half-verified) | tools: generator (`bf5126d`, `a05e1cf`) | patterns fixed: — …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S392 (2026-09-16):** hebrew_blend_generator **2026-09-16 (`bf5126d`, `a05e1cf`; G S392, A S385, P S382)**; index 2026-09-16 (`fa001b0`; M S391, D S352, C S340); Hebrew_Font_Maker 2026-09-16 (`d5ee54c`; H S380); scripts/smoke-account + account-test 2026-09-16 (`bfcdc15`); account 2026-09-16 (P S390, L S387, E S386); all 14 root pages 2026-09-16 (`c0659f3`); js/ivrit-account + js/ivrit-saves 2026-09-16 (P S390, N S389, K S388); trope_tutor 2026-09-16 (N S389, M S376); flash_cards 2026-09-16 (C S381, **D-next**); hebrew_dictionary 2026-09-15 …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`hover-feedback-survives-the-disabled-state`** (registered 2026-09-15, S383 Pass F): ACTIVE — consequence-adjacent (a locked destructive control that still lights up invites the click), so it never retires on a clean streak. **S392: no carrier fixed; the LAST open one — …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S393):** **BRANCH/PR: S392 CONTINUED `claude/loving-einstein-cdmbbz` — PR #246 was open, unmerged and draft at session start (verified through the API, not assumed), so the skill's continue rule applied. 2 fix commits + this close-out on top of S391's 5. If #246 is STILL open and unmerged, continue on it; if merged, cut fresh off latest `origin/main` — never stack on merged history.** **⚑ DRIFT CHECK: NONE this session — `origin/main` still `b304db0`, nothing on it this branch lacks. The clone arrived COMPLETE (`git rev-parse --is-shallow-repository` = false, so `--unshallow` errored harmlessly); S391's arrived SHALLOW and read 65 commits stale, so CHECK, never assume either way.** After S392: `sw.js` **v767**, FM **5.45**, migrations 0001–0003 all `Live? = yes`, SDK pin 2.116.0, keep-alive run #4 (scheduled, 2026-09-16) green. Never trust these numbers unread. The `--sdk` fixture: `npm pack @supabase/supabase-js@2.116.0` (**npm IS reachable; the CDNs are not** — the same trick serves html2canvas 1.4.1 + jspdf 2.5.1 for the generator's PDF export, and pypi timed out where npm did not). The smokes hardcode port 8080 — kill your own server first, by PID. **Never `pkill -f` a server: it matches and kills this shell.**

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), D (S378), I (S379), H (S380), C (S381), F (S383), B (S384), A (S385), E (S386), L (S387), K (S388), N (S389), P (S390), M (S391), G (S392).** Unattended: take **D** (performance, one tool) — 14 sessions stale and now the longest unattended gap. Surface by staleness: **`flash_cards.html`** (the coverage row has carried **D-next** on it for several sessions; the generator is S338, the dictionary S378, index S352). Then I (re-run `i379/gates.mjs`), **H next the dashboard (S46 — still the stalest tool pairing in the table)**, C, F, B, A, E, L, K, N, P, M (next: a chrome page never M-audited — `account.html` has never had one), and G (next: `flash_cards.html` at S279, or the generator again for the two P3s this session could not ship).

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1–30 are S384–S391's, in full in loop-findings — read them there first.** **(31) NEW S392 — a build-readiness gate that counts NODES returns a HALF-BUILT sheet. The real-words drills fetch their corpus, so `nodes > 20` matched a 227-char skeleton with zero cards and every later measurement was of an empty artifact. Gate on CONTENT selectors, and widen them per mode — Inverse builds `.blend-cell-inv`, which a `.blend-cell` gate reads as "never built".** **(32) NEW S392 — this page has TWO `@page` rules: a static one in the stylesheet and an injected `#pageSizeStyle`. A probe that regex-matches the first `@page{...}` across all `<style>` text reads the static one and reports "NOT-A4" for a page that is correctly A4 — 17 false FAILs. Read the injected element by id.** **(33) NEW S392 — a probe can TRIP the very defect it is measuring: `paper.mjs` clicked Page Size to set each cell up, and Page Size was the wiping control, so every real-words PDF read 89 chars. That looks exactly like a probe artifact. Diagnose before discarding AND before believing.** **(34) NEW S392 — the print dialog ships "Background graphics" OFF, so a contrast census with backgrounds ON reads clean on text that is invisible on paper. Model it as `*,*::before,*::after{background-color:transparent!important;background-image:none!important}` under print media, with backgrounds-ON as the same-cell control.**

**⚑ TOP UNGATED CANDIDATES (S392): the generator's plate-dependent print ink** (P3, NEW — the answer-key banner at 1.5:1 on all 9 answer keys and bingo's header letters at 1:1 with the dialog's default backgrounds-off; converge on flash_cards' print block, which measures clean both ways); **Header Labels doing nothing on word search/crossword** (P3, NEW); **the Edge Function's production `localhost` origins** (P4, backend boundary — maintainer redeploy); **"sign out everywhere"** (P4); **`clampMenu()` on resize/orientationchange** (P4); **the FM chip's unmeasured phone geometry** (P4); **torah `.btn`, the LAST open carrier of the hover pattern — its recipe is now most of the way there (loop-findings), the one missing step is the bulk-select toggle inside the Copy bar**; **the tour-card h4 → h2 ×7**; **the sub-floor touch targets**. Gate 2: the flash-card swatch labels; the generator's `restoreLastSetup` corrupt-drop; the dictionary drag-freeze + select-all cap; the corpus's split article tokens; the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the resources Taamim fallback; the FM's `nexthint_nikkud` copy; **the 5 unreferenced credit keys (S360)**. Gate 3/M: the FM stage annotations; `account.html`'s dark `.btn.ghost`/`.btn.danger`; **`.dash-edit-pencil`'s side in Hebrew**; **the hub's seven-size 2.4px type band**. Gate 4: the flash-card sticky-bar and landscape proposals. **Seed bench: 26.**

...
- **S390 (2026-09-16):** account **2026-09-16 (`db0c149`; P S390 security, L S387, E S386)**; all 14 root pages 2026-09-16 (`c0659f3`); js/ivrit-account + js/ivrit-saves 2026-09-16 (P S390 read-only audit; `f5ccea8`/`ba34a28`, N S389, K S388); trope_tutor 2026-09-16 (N S389, M S376); flash_cards 2026-09-16 (C S381, D-next); hebrew_blend_generator 2026-09-16 (A S385, P S382); hebrew_dictionary 2026-09-15 (G S377 + D S378); classroom_dashboard 2026-09-15 (A S385 + B S384, **H stalest since S46**); Hebrew_Font_Maker 2026-09-16 (H S380)
...
- P accounts & cloud (one surface): 2026-09-16 (**S390 — 2nd-ever P, and the first run steered by explicit user direction ("focus on security with the new backend") rather than staleness; M stays the stalest unattended pass.** Surface: the whole layer, security-first. **2 REAL FINDINGS, both FIXED:** the PKCE `?code=` and `?error_description=` reaching Google Analytics (`c0659f3`, all 14 …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S391):** **BRANCH/PR: PR #242 was MERGED (`af96827`). S390 therefore cut a FRESH branch, `claude/modest-edison-bgpu71`, off `origin/main` `44963bd`, with 2 fix commits + a close-out and a new draft PR. If that PR is open and unmerged, CONTINUE on it; if merged, cut fresh off latest `origin/main` again — never stack on merged history.** **⚑ DRIFT CHECK: S390 found the FIRST drift in eight sessions — `sw.js` v756→v758 and one outside-loop commit (`44963bd`) on `js/ivrit-account.js`. Re-check against `44963bd` + this loop's commits; never trust these numbers unread.** After S390: `sw.js` **v759**, FM 5.44, migrations 0001–0003 all `Live? = yes`. A fresh container starts SHALLOW — `git fetch --unshallow` before `update-sitemap.mjs` and any `git log`-dated arm. The `--sdk` fixture the smokes need: `npm pack @supabase/supabase-js@2.116.0` (npm IS reachable; the CDNs are not), untar, pass `package/dist/umd/supabase.js` — **pass it to `smoke-tools.mjs` too or its signed-in arms silently SKIP**. The smokes hardcode port 8080 — one at a time, and background them. **Never `pkill -f` a server: it matches and kills this shell. Kill by PID.**

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), M (S376), G (S377), D (S378), I (S379), H (S380), C (S381), F (S383), B (S384), A (S385), E (S386), L (S387), K (S388), N (S389), P (S390).** Unattended: take **M** (aesthetics) — now 14 sessions stale and the longest-standing unattended gap for a second session running; its surface is **`index.html`** (S246, the pointer's standing choice). M ships BEFORE and AFTER screenshots light + dark at the affected breakpoints; anything a returning teacher would notice is gate 3, unattended-deferred. Then G (S377), D (S378 — next flash_cards or the FM), I (re-run `i379/gates.mjs`), **H next the dashboard (S46, the stalest tool pairing in the table)**, C, F, B, **A (must use S388's CORRECTED arm-(b) definition: three disabled forms, ID-keyed guards count, dimming does NOT discharge hover-cancelling; A also now owns `auth-param-read-before-its-cleanup-runs`, consequence-critical so it never retires)**, E, L, K, N, P.

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1–23 are S384–S389's, in full in loop-findings — read them there first.** **(24) NEW S390 — seeding a language/theme with the WRONG localStorage key makes every HE and dark cell silently an EN light cell, and all six still "PASS". The real keys are `hebrewBlender_lang` and `hebrewBlender_darkMode`; assert the axis actually switched (`documentElement.lang`, computed `direction`, the dark class) or the cell is VOID, not a pass.** **(25) NEW S390 — the standard recipe's catch-all abort kills the analytics request too, so any arm asking "what does a third party receive?" must COUNT the aborted requests as its side effect; a zero with a zero counter is blind.**

**⚑ TOP UNGATED CANDIDATES (S390): the Edge Function's production `localhost` origins** (P4, backend boundary — maintainer redeploy); **"sign out everywhere"** (P4, `scope: 'global'` + one account-page control); **widen the smokes' viewport check** (P3 — `smoke-account`'s one 800×700 cell passed clean through 17 real phone overflows); **`clampMenu()` on resize/orientationchange** (P4); **the FM chip's unmeasured phone geometry** (P4, behind its wizard); **the three unconfirmed arm-(b) carriers**; **the tour-card h4 → h2 ×7**; **the sub-floor touch targets**. Gate 2: the flash-card swatch labels; the generator's `restoreLastSetup` corrupt-drop; the dictionary drag-freeze + select-all cap; the corpus's split article tokens; the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the resources Taamim fallback; the FM's `nexthint_nikkud` copy; **the 5 unreferenced credit keys (S360)**. Gate 3/M: the FM stage annotations; `account.html`'s dark `.btn.ghost`/`.btn.danger`; **`.dash-edit-pencil`'s side in Hebrew**. Gate 4: the flash-card sticky-bar and landscape proposals. **Seed bench: 26.** **⚑ PROTOCOL QUESTION STILL OPEN: the shared-`js/`-module cap — S388 read it as one such fix per session, S389 arithmetically. S390 did not need to adjudicate it (its two fixes were an inline-snippet sweep and a single chrome page), so one reading still needs to win.**

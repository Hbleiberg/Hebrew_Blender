...
- [ ] P4 (**NEW S366 Pass I — pattern `stored-json-of-the-wrong-shape-trusted`, the 3 carriers the cap left; `i366/strshape.mjs`**) | hebrew_blend_generator.html + classroom_dashboard.html | **A stored STRING where a preset map is expected lists its characters as presets: `hebrewBlender_presets`, `hebrewDashboard_presets` and `hebrewDashboard_schedules` = `"hello"` each render 5 phantom rows …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S366 (2026-09-10):** classroom_dashboard **2026-09-10 (`b98d7ae`; G-audited S364)**; G-audited S364)**; hebrew_blend_generator **2026-09-10 (`8bf361c`; H-audited S363)**; H-audited S363)**; trope_tutor **2026-09-10 (`772068b`; D-audited S365)**; hebrew_dictionary 2026-09-09 (`10be341`; M-audited S362); Hebrew_Font_Maker 2026-09-09 (`26fd826`); index 2026-09-09 (S360); contact / privacy / terms / 404 2026-09-09 (S360); flash_cards **2026-09-10 (`e54dd1a`)**; N-audited S361); resources 2026-09-09 (S356); torah_trainer 2026-09-09 (S355).

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`stored-json-of-the-wrong-shape-trusted`**: ACTIVE — **REGISTERED S366 (Pass I): a reader that parses a localStorage JSON key and trusts a plain object (`Object.keys`, member access, `Object.assign`) — a stored `null`/string/array/number (a hand-edited key, a corrupted blob, …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S367):** **BRANCH/PR: S366 CONTINUED `claude/inspiring-meitner-rhqskk` → draft PR #227 (base `origin/main` `f2384a7`; S352–S366 = 67 commits). Continue it if open; if merged, cut a fresh `claude/*` off latest `origin/main`. Verify via the API** (0 check runs is correct — no workflows). **Drift note: `sw.js` v691, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), C (S354), F (S355), B (S356), A (S357), E (S358), L (S359), K (S360), N (S361), M (S362), H (S363), G (S364), D (S365), I (S366).** Unattended: take C (accessibility, one tool) on `Hebrew_Font_Maker.html` — its last dedicated C was S257 (the S340 ordering carried since S365: FM S257 < the rest); the wizard, the category tabs, the glyph stage (`aria-` on the SVG/canvas), every modal's Escape/Enter, focus visibility in both themes, reduced motion, the touch-floor census (`#rulerCorner` 22×22 is a known gate-3 item — census it, don't re-file it), contrast light + dark. **O next `torah_trainer.html`; D next dictionary (S253); G next dictionary (S252); H next trope_tutor (S228); N next resources (S246); M next trope_tutor (S245); the next I re-runs `i366/{gates,arms,corrupt,strshape}.mjs` and weights the S366 fixes.**

**⚑ Harness (standing): torah stubs Sefaria by fulfilling `**/*sefaria.org/**` (`b356/harness.mjs`); hidden toggle inputs → click `label.toggle:has(#id)`; the HTTP server dies across a compaction — curl it first; pypdf needs the stub `cryptography` package (`d352/stub`); `page.pdf()` fires `afterprint` — re-wear a print-state class before EACH pdf; PyMuPDF is absent — `g364/paper.mjs` CENSUS is the slab detector; a hover read needs a REAL `page.mouse.move` + 400 ms, a focus read a real Tab, never `page.focus()`; open a probe's host by ITS path (dashboard schedules `openWeekEditor()`; the generator's live-preview toggle is in the collapsed Advanced panel, live sync speaks only after a first Generate, 500 ms debounced; FM Save Project ▾ = `[data-i18n="fontmaker.toolbar.save_project_menu"]`); an `elementFromPoint` receipt precedes a real click; modal probes read the LAST `[aria-modal]` whose computed `display` is not none (an off-canvas drawer has width > 0 while closed); a disabled read checks `aria-disabled` too; a `let` page global is not on `window` — wait on a DOM receipt; `ivritAskMode()` opens by calling it inside `page.evaluate`, never awaited; the harness auto-accepts dialogs — a confirm probe installs a handler that dismisses; K's strongest arm is `k360/hecensus.mjs`; probe screenshot paths are cwd-relative — run probes from the scratchpad.**

**⚑ TOP UNGATED CANDIDATES (S366):** the 3 string-shape preset readers (generator presets, dashboard presets + schedules — the flash `readPresets` idiom, one commit; cap-blocked this session, top for S367); the tour-card h4 → h2 ×7 (CLAUDE.md: the next touch of a tour engine extracts it into a shared block — not small); the remaining sub-floor M calls (the generator's 33 `scale(0.75)` sub-option toggles — a visible restyle, gate 3; the bingo ▲/▼ stepper; FM `#rulerCorner`). Gate 2: the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on (`g364/out/board-print-en-dark.png`); the trope FAQ a7 "one reference sheet" copy. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. The next A sweeps the S357–S366 delta with the `fixed-surface-missing-from-the-print-hide-list` arm on all 7 tools AND the new `stored-json-of-the-wrong-shape-trusted` arm (`i366/corrupt.mjs` + the static reader grep); the next K greps every authored key literally. **Seed bench:** unchanged (4 seeds: resources font-submit form, hub saved-work badges, dashboard per-day overrides, dashboard week cycles).

...
- [ ] P3 (**S383 Pass F, 5 carriers — 3 SHIPPED S384 (`a585608` FM, `df9ff3b` contact + resources); the 2 left are a DIFFERENT SHAPE and need per-variant rules, analysis below so nobody re-derives it**) | trope_tutor.html, account.html | **Both key off `[aria-disabled="true"]`, not `:disabled`, and both carry VARIANT classes with their own rest colours — trope `.btn btn-secondary` (its own …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P3 (**NEW S384 Pass B arm 4 — the sibling shape of the shipped `9614a80`, on the pages the probe did not reach**) | suite-wide (`applySettings` carriers) | **The dashboard's 7 unguarded interpolated `querySelector(...).checked` sites were the only carriers of that exact shape, but the arm only drove `dashboardSettings`: the same wrong-shape `.ivrit` axis is UNSWEPT for the generator, flash …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P4 (**NEW S383 Pass F — the one page that hand-rolls the disabled look instead of using a rule**) | hebrew_dictionary.html | **Three controls are dimmed with inline `style.opacity` rather than a `:disabled` rule — `wlSaveBtn` 0.5 and the word-list `wm-mini` ↑/↓ at 0.3, the lowest disabled opacity in the suite (every rule-based one is 0.35–0.55) — and none of them changes cursor, so a …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-15 | `a585608` | Hebrew_Font_Maker.html | **A disabled control stops lighting up under the cursor: `.ctl-btn:disabled` dims to 0.4/`cursor:default` but both `:hover` rules were unguarded, so undo, redo, the node-bar twins and the mark-editor pair — which ship disabled — kept lighting up (S384, P3, pass B, carrier 1 of 5, `hover-feedback-survives-the-disabled-state`)** | …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-15 | `9614a80` | classroom_dashboard.html | **A hand-edited settings file no longer bricks the board: `syncFormToSettings()` set 7 radio groups with a bare interpolated `querySelector(...).checked = true`, so a stored value matching no radio returned null, the throw aborted the whole sync before the board rendered — and the junk was ALREADY SAVED, so every later load died the same …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-15 | (S382 close-out) | branch/deploy note | **S382 opened a FRESH `claude/wizardly-tesla-xu0ikd` off latest `origin/main` (PR #229 merged at S381); 3 fix commits + this close-out; a human merges and confirms the Pages run. DRIFT — the largest yet recorded: `origin/main` `7f94f28` → `4d6c02d` (21+ commits), `sw.js` v706 → **v748**, `FONT_MAKER_VERSION` 5.39 → **5.44**, migrations …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-11 | (S381 close-out) | branch/deploy note | **S381 CONTINUED `claude/improve-loop-swgwqx` (PR #229 open, draft, mergeable clean at start; base `66dd97b` = `origin/main`), 3 fix commits + this close-out (62 on the branch). After the close-out the maintainer authorized "push to main" in-session: main fast-forwarded `66dd97b..7f94f28`, PR #229 auto-closed as merged, the Pages run for …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-11 | pass | flash_cards.html | **S381 Pass C (accessibility), flash cards' 5th dedicated C (S167 → S271 → S381; 46 commits to the file since S271), stalest unattended pass + tool. 16 arms in EN light 1280 + HE dark 800 (`c381/pass.mjs`, a seeded 2-result profile, injected-throw control counted in every cell): name census 173/173 visible controls named (setup expanded; 0 …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]
...
- 2026-09-15 | **S384** | iters: 1 pass (**B**) + 3 fixes = **4** (the hover-vs-disabled class hit its 2-per-session cap; every other open candidate is gated or P4) | tools: classroom_dashboard (`9614a80`), Hebrew_Font_Maker (`a585608`), contact + resources (`df9ff3b`) | patterns fixed: …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S384 (2026-09-15):** classroom_dashboard **2026-09-15 (`9614a80`; B S384)**; Hebrew_Font_Maker **2026-09-15 (`a585608`; B S384, H-audited S380)**; contact **2026-09-15 (`df9ff3b`; B S384)**; resources **2026-09-15 (`df9ff3b`; B S384, N-audited S375)**; flash_cards 2026-09-15 (`a234f9f`; F S383, C-audited S381); hebrew_dictionary 2026-09-15 (`3674d6e`; F S383, D/G-audited S377–S378); the 9 cloud-panel pages 2026-09-15 (`5cd1b90`, shared modules; F S383); hebrew_blend_generator 2026-09-15 (`b5f06f5`; P S382); torah_trainer 2026-09-11 …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`hover-feedback-survives-the-disabled-state`** (registered 2026-09-15, S383 Pass F — 7 carriers found, 5 now fixed): ACTIVE. **S384 (Pass B): 3 MORE FIXED — FM `.ctl-btn` (`a585608`), contact `.btn-primary` + its `:active` dip and resources `.sg-btn-primary` (`df9ff3b`); 2 …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`apply-settings-trusts-collection-members`**: ACTIVE (consequence-critical: garbage is applied AND SAVED — never retires). **S384 (Pass B arm 4): a NEW shape FIXED — an ENUM value interpolated into a selector, where the miss is a NULL DEREFERENCE rather than a wrong value: …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S385):** **BRANCH/PR: S382+S383+S384 all ran on `claude/wizardly-tesla-xu0ikd` (PR #242, draft, open, unmerged; base `4d6c02d`), now 12 commits + 3 close-outs. If that PR is still open and unmerged, CONTINUE on it; if merged, cut a fresh `claude/*` off latest `origin/main` and open a new draft PR at close-out. Verify via the API** (no workflow triggers on `pull_request`, so `get_status` reads `pending` with `total_count: 0` — GitHub's default for a commit with no legacy statuses, NOT a queued check. `get_check_runs` is the one that answers: exactly 1 run, the Supabase GitHub App's **`Supabase Preview`**, concluded **`skipped`** — correct, because migrations live in `db/` and not `supabase/` precisely so the integration never spins up a paid preview branch. A `skipped` Supabase Preview is a green PR, not a failure to chase.) **⚑ DRIFT CHECK: compare `origin/main` against `4d6c02d` + this loop's commits; S383 and S384 both found NONE, S382 the largest yet — CHECK IT, never trust this unread. `sw.js` v751, FM 5.44, migrations 0001–0003 all `Live? = yes`.** A fresh container starts SHALLOW — `git fetch --unshallow` before `update-sitemap.mjs`. The `--sdk` fixture the required smokes need: `npm pack @supabase/supabase-js@2.116.0` (the npm registry IS reachable; the CDNs are not), untar, pass `package/dist/umd/supabase.js`; **pass `--sdk` to `smoke-tools.mjs` too or its signed-in B arms silently SKIP** (114 checks without it, 135 with).

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), A (S371), E (S372), L (S373), K (S374), N (S375), M (S376), G (S377), D (S378), I (S379), H (S380), C (S381), P (S382), F (S383), B (S384).** Unattended: take **A** (recurring-pattern sweep) — it is 13 sessions stale and its arms are already written below; then E. **The next A sweeps `1d4c9cf..HEAD` and must add the S383/S384 rows `hover-feedback-survives-the-disabled-state` (2 carriers still open) plus `synced-key-wiped-without-forgetRow`, `tile-grid-built-hidden-never-roved`, `vh-capped-sheet-without-dvh-twin` and `dark-print-shadow-slab`, and re-sweep `apply-settings-trusts-collection-members` with its NEW S384 shape (a stored value interpolated into a selector and dereferenced unchecked).** O next `torah_trainer.html`; C next torah (S298) then dictionary (S285); M next `index.html` (S246) then torah; N next trope (S248) then index (S249); D next flash_cards or the Font Maker; H next the dashboard (S46) or the generator (S56); P next the dashboard or the dictionary; F next affordance: the disabled CURSOR idiom (S383 measured `not-allowed` 9 / `default` 17 but did not converge) or the empty-state re-sweep; the next E re-runs its 17 arms over `408dbe4..HEAD` — **E has still never seen the backend landing**; the next L re-runs vs `4a1f186`; the next K over `4a1f186..HEAD`; the next I re-runs `i379/gates.mjs`.

**⚑ B-PASS HARNESS (standing, NEW S384): three probe traps, all hit this session and all recorded in loop-findings.** (1) **`hebrewBlender_darkMode` stores `'1'`/`'0'`, NOT `'true'`** — a probe writing `'true'` runs LIGHT while reporting a dark sweep; assert `body.dark` per cell and void the cell otherwise (`i18n-test.html` legitimately has no dark). (2) **Playwright matches routes in REVERSE registration order** — register the catch-all abort FIRST and specific stubs (Sefaria) AFTER, or the stub never fires and torah reports a `Failed to fetch` that is pure harness. (3) **`requestfailed` never fires for a 404** (a 404 is a successful response) — control that detector with a routed `abort()` on a same-origin URL. Also: `ivritRestore` BLOCKS on the Merge/Replace modal, so fire it WITHOUT awaiting inside `page.evaluate` and then click `.ivrit-ask-btn[data-act="merge"]` for real; and give every wrong-shape import a FRESH CONTEXT — a shared one persists the junk, so shape N+1 inherits N's damage and the last dozen shapes all read as broken.

**⚑ TOP UNGATED CANDIDATES (S384): the 2 remaining hover-vs-disabled carriers (trope `.btn`, account `.btn` — both `[aria-disabled]` + variant classes needing per-variant restate rules; account's `#delGo` is `class="btn danger" aria-disabled="true"`, a locked DESTRUCTIVE control that still lights red — the analysis is in the candidate, do not re-derive it); the wrong-shape `.ivrit` axis on the five liveState blobs arm 4 never drove (`b384/arm4b.mjs` takes a page + key); the dictionary's inline `style.opacity` disabled hack; the tour-card h4 → h2 ×7 (the "next touch of a tour engine" — extract the shared block in the same session); the remaining sub-floor M calls (bingo stepper, scaled toggles, `#rulerCorner`). Decide-then-sweep: whether an action TOOLBAR should rove like a selection grid; whether the disabled CURSOR should converge. Gate 2: the flash-card swatch labels; the generator's `restoreLastSetup` corrupt-drop; the dictionary drag-freeze + select-all cap; the corpus's split article tokens; the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the resources preview Taamim fallback. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals. **Seed bench: 26. Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

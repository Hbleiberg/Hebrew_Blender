...
- [ ] P3 (**NEW S388 Pass K — the 4th and last of the fallback-drift axis; blocked by the variety governor, not by doubt**) | js/ivrit-saves.js | **`shared.cloud.acct_download_note`'s hardcoded fallback stops at "…on the home page." while the CSV `en` continues ": it is merged into what is on that device (per-device settings such as zoom and pan…)".** A teacher whose dictionary never loaded …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S388 (2026-09-16):** js/ivrit-account **2026-09-16 (`cfb1ce0`; K-audited S388, the accounts layer's FIRST K)**; flash_cards **2026-09-16 (`b4afd93`; C S381, D-next)**; hebrew_blend_generator **2026-09-16 (`60cd7ae`; A S385, P S382)**; trope_tutor 2026-09-15 (`f77b6da`; L S387, M S376); hebrew_dictionary 2026-09-15 (`831b414`; G S377 + D S378); classroom_dashboard 2026-09-15 (`68552b3`; A S385 + B S384, **H stalest since S46**); account 2026-09-15 (`fba22e1`; L S387, E S386, P S382); repo metadata — README, THIRD_PARTY_LICENSES 2026-09-15 …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`hover-feedback-survives-the-disabled-state`** (registered 2026-09-15, S383 Pass F): ACTIVE. **S388 (Pass K session, ×1): the NINTH carrier FIXED (`b4afd93`, flash cards) and the FIRST ever found by ARM (b)** — two controls, two idioms, two different failures: …[full text: IMPROVEMENT_ARCHIVE.md]

- **`physical-property-that-never-mirrors`** (**NEW, registered 2026-09-16 (S388 Pass K) — the shape has now surfaced in THREE sessions (S248, S376, S388), which is what makes it recurring rather than incidental**): ACTIVE. A physical horizontal CSS property …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S389):** **BRANCH/PR: S382–S388 all ran on `claude/wizardly-tesla-xu0ikd` (PR #242, draft, open, unmerged; base `4d6c02d`), now 30 commits + 7 close-outs. If that PR is still open and unmerged, CONTINUE on it; if merged, cut a fresh `claude/*` off latest `origin/main` and open a new draft PR at close-out. Verify via the API** (`get_status` reads `pending`/`total_count: 0` — no workflow runs on `pull_request`, so that is GitHub's default, NOT a queued check. `get_check_runs` answers: 1 run, `Supabase Preview`, concluded **`skipped`** — correct, migrations live in `db/`. A GREEN PR, not a failure to chase.) **⚑ DRIFT CHECK: compare `origin/main` against `4d6c02d` + this loop's commits; S383–S388 all found NONE — CHECK IT, never trust this unread. `sw.js` v755, FM 5.44, migrations 0001–0003 all `Live? = yes`.** A fresh container starts SHALLOW — `git fetch --unshallow` before `update-sitemap.mjs` and before any `git log`-dated arm (S388's clone was full at 3,071). The `--sdk` fixture the backend smokes need: `npm pack @supabase/supabase-js@2.116.0` (the npm registry IS reachable; the CDNs are not), untar, pass `package/dist/umd/supabase.js`; **pass `--sdk` to `smoke-tools.mjs` too or its signed-in arms silently SKIP** (S388 measured 135/135 with it). `smoke-account`/`smoke-saves`/`smoke-tools` all hardcode port 8080 — run them ONE AT A TIME; `smoke-tools` + `smoke-account-page` together take over 2 minutes, so background them.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), N (S375), M (S376), G (S377), D (S378), I (S379), H (S380), C (S381), P (S382), F (S383), B (S384), A (S385), E (S386), L (S387), K (S388).** Unattended: take **N** (mobile & touch) — 13 sessions stale and the longest-standing unattended gap; its surface is **trope_tutor** (S248, the pointer's standing choice). N needs a REAL device descriptor (`browser.newContext({ ...devices['iPhone 13'], serviceWorkers: 'block' })`), ships a phone contact sheet even when clean, and gate 4 is unattended-deferred. M next `index.html` (S246); G next; D next flash_cards or the FM; H next the dashboard (**S46, the stalest tool pairing in the table**); P next the dashboard or dictionary; F next the disabled CURSOR idiom (the suite now runs `not-allowed` on trope/flash/torah and `default` on the dictionary/FM/account — F's call) or the empty-state re-sweep; the next I re-runs `i379/gates.mjs`; **the next A sweeps `b5e1416..HEAD` and must use S388's CORRECTED arm-(b) definition (three disabled forms, ID-keyed guards count, and dimming does NOT discharge hover-cancelling)**; the next K re-runs `k388/{plant,keys,dyn,blind,acct,fallback,rtl,armb}.mjs` against this session's endpoint.

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1–16 are S384–S387's, written out in full in loop-findings — read them there first.** Cues: (1) theme from a themed token, not the stored flag; (2) catch-all abort route FIRST; (3) no `requestfailed` for a 404; (4) never hand-roll a name walker; (5) census a dialog OPEN; (6) seed `hebrewDashboard_setupSeen`; (7) an undetected plant can mean immunity or a warn-only rule; (8) `ivritRestore` blocks on Merge; (9) match the attribute quote's kind; (10) LOAD sites only; (11) `url(…)` also matches JS `URL(`; (12) specificity is a measurement — measure dark too unless the rest colour is a themed token; (13) a top-level `const` in an inline script is SCRIPT-SCOPED, so use the bare identifier and VOID if undefined; (14) a junk-shape axis needs a truthy non-string; (15) walk `@graph`; (16) `pkill -f` kills the shell. **(17) NEW S388 — a literal scanner must read BOTH quote styles and ALL target files: single-quote-only hid 30 sites, a 6-file scan 29 more (360→390→419). (18) NEW S388 — a fallback scanner truncates CONCATENATED fallbacks at the first literal; 20 of S388's 24 drifts were that artefact. (19) NEW S388 — measure the CONTROL before anything that switches views: `showResults()` and `_syncCopyAllEnabled()` hid it and VOIDed two good runs. (20) NEW S388 — `I18n.t()` warns `[i18n] missing key:` and returns the raw key, but that warning is gated on `dictLoaded` — fire a control miss or a silent cell reads clean.**

**⚑ TOP UNGATED CANDIDATES (S388): `js/ivrit-saves.js`'s `acct_download_note` fallback** (the 4th of the drift axis, P3, blocked only by S388 spending the shared-`js/` allowance — take it FIRST next session); **the three unconfirmed arm-(b) carriers** (generator `.btn-secondary`/`.btn-xs`, FM `.help-tab`, torah `.btn` — torah needs a loaded parsha to drive, and its guard is ID-keyed); **the tour-card h4 → h2 ×7** (the "next touch of a tour engine" — extract the shared block in the same session); **the remaining sub-floor touch targets** (bingo stepper, scaled toggles, `#rulerCorner`). Gate 2: the flash-card swatch labels; the generator's `restoreLastSetup` corrupt-drop; the dictionary drag-freeze + select-all cap; the corpus's split article tokens; the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the resources preview Taamim fallback; the FM's `nexthint_nikkud` dead copy; **the 5 authored-but-unreferenced credit keys (S360)**. Gate 3/M: the Font Maker stage annotations; `account.html`'s dark `.btn.ghost`/`.btn.danger`; **`.dash-edit-pencil`'s side in Hebrew (S388 measured it; S248 and S376 logged the same shape)**. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals. **Seed bench: 26. Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

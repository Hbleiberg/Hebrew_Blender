...
- **S365 (2026-09-10):** classroom_dashboard **2026-09-10 (`095bc47`; G-audited S364)**; hebrew_blend_generator **2026-09-10 (`d86e728`; H-audited S363)**; trope_tutor **2026-09-10 (`772068b`; D-audited S365)**; hebrew_dictionary 2026-09-09 (`10be341`; M-audited S362); Hebrew_Font_Maker 2026-09-09 (`26fd826`); index 2026-09-09 (S360); contact / privacy / terms / 404 2026-09-09 (S360); flash_cards 2026-09-09 (S360; N-audited S361); resources 2026-09-09 (S356); torah_trainer 2026-09-09 (S355).
...
**Next session (S366):** **BRANCH/PR: S365 CONTINUED `claude/inspiring-meitner-rhqskk` → draft PR #227 (base `origin/main` `f2384a7`; S352–S365 = 63 commits). Continue it if open; if merged, cut a fresh `claude/*` off latest `origin/main`. Verify via the API** (0 check runs is correct — no workflows). **Drift note: `sw.js` v690, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), I (S353), C (S354), F (S355), B (S356), A (S357), E (S358), L (S359), K (S360), N (S361), M (S362), H (S363), G (S364), D (S365).** Unattended: take I (first-load & empty-state), its 28th run — every page in a fresh context (empty localStorage AND IndexedDB, SW blocked, foreign origins aborted), EN + HE, weighting the S353–S365 surfaces: the generator's 24px toggles / kept-renderer notice / bingo stepper, the dashboard week editor + its print block, the Trope Tutor's Geresh-family empty-state line and offline retry, the hub / flash / FM UI-language dates, the five landing footers. **O next `torah_trainer.html`; D next dictionary (S253); G next dictionary (S252); H next trope_tutor (S228); C the Font Maker (S257); N next resources (S246); M next trope_tutor (S245).**

**⚑ Harness (standing): torah stubs Sefaria by fulfilling `**/*sefaria.org/**` (`b356/harness.mjs` carries the stubs); hidden toggle inputs → click `label.toggle:has(#id)`; the HTTP server dies across a compaction — curl it first; pypdf needs the stub `cryptography` package (`d352/stub`) and prints `<input type=time>` as 12-hour text; `page.pdf()` fires `afterprint` — re-wear a print-state class before EACH pdf; PyMuPDF is absent — `g364/paper.mjs` CENSUS (fixed elements + dark slabs under print media) is the slab detector; a hover read needs a REAL `page.mouse.move` + 400 ms, a focus read a real Tab, never `page.focus()`; open a probe's host by ITS path (dashboard schedules `openWeekEditor()`; the backup box ⚙ → Presets → `#ivritModeManual`; `#dashEditor` is INSIDE the settings modal; the generator's live-preview toggle is in the collapsed Advanced panel and live sync speaks only after a first Generate, 500 ms debounced; the FM Custom glyphs tab: Advanced → `label.chk:has(#addCustomGlyphsChk)`); an `elementFromPoint` receipt precedes a real click; modal probes read the LAST `[aria-modal]`; `ivritAskMode()` opens by calling it inside `page.evaluate`, never awaited; the harness auto-accepts dialogs — a confirm probe installs a handler that dismisses; K's strongest arm is `k360/hecensus.mjs`; probe screenshot paths are cwd-relative — run probes from the scratchpad.**

**⚑ Harness (S365, performance): `d365/lib.mjs` is the D kit (longtask + keyed MutationObserver + rAF/timer counters, CPU throttle, real-click busy-loop control); `page.focus()` on a control inside a COLLAPSED panel silently leaves focus where it was — expand by a real title click and assert `document.activeElement` before any key; Playwright's `waitForFunction` polls by rAF, so a page-side rAF counter reads ~60/s while it waits (use `polling:'mutation'` or a stack histogram); CDP `Performance.getMetrics` LayoutCount / RecalcStyleCount deltas are the layout-thrash meter; `Runtime.getHeapUsage` after `HeapProfiler.collectGarbage` is the leak meter (`performance.memory` is quantised); the shared language switcher stretches to its flex LINE (measure `[data-i18n-switcher] > *`).**

**⚑ TOP UNGATED CANDIDATES (S365):** the tour-card h4 → h2 ×7 (CLAUDE.md: the next touch of a tour engine extracts it into a shared block — not small); the remaining sub-floor M calls (the generator's 3 `scale(0.75)` toggles, the bingo ▲/▼ stepper, FM `#rulerCorner`). Gate 2: the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on (`g364/out/board-print-en-dark.png`); the trope FAQ a7 "one reference sheet" copy. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. The next A sweeps the S357–S365 delta with the NEW `fixed-surface-missing-from-the-print-hide-list` arm on all 7 tools; the next K greps every authored key literally (the S365 trope leak hid behind a `'trope.learn.' + x` prefix). **Seed bench:** unchanged (4 seeds: resources font-submit form, hub saved-work badges, dashboard per-day overrides, dashboard week cycles).

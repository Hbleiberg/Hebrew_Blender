...
- [ ] P3 (**NEW S371 Pass A — pattern `stored-json-of-the-wrong-shape-trusted`; the definition's claim that this key checks `Array.isArray` was wrong**) | Hebrew_Font_Maker.html | **`getRecentProjects()` returns whatever `hebrewFontMaker_recentProjects` parses to: a stored `"abc"`, `null` or an array with a null member throws uncaught (`recents.map` / `null.length` / `null.name`) inside …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S371 (2026-09-10):** Hebrew_Font_Maker 2026-09-10 (`df7f98e` S370; A-probed S371: recents key + `.guide-rename` logged; C-audited S368); hebrew_blend_generator **2026-09-10 (`e4c8d75`; H-audited S363)**; trope_tutor 2026-09-10 (`869ee1c` S369; A-probed S371: settings print + Infinity labels logged; H-audited S367); classroom_dashboard **2026-09-10 (`b85b579`, `1568296`; G-audited S364)**; hebrew_dictionary **2026-09-10 (`e5dac20`; M-audited S362)**; index 2026-09-10 (`edf4b2e` S370; S360); contact / privacy / terms / 404 2026-09-09 (S360; …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`stored-json-of-the-wrong-shape-trusted`**: ACTIVE. **S371 (Pass A): 1 NEW carrier found, logged P3 (budget spent) — the Font Maker's `hebrewFontMaker_recentProjects` reader has NO `Array.isArray` check (this row's definition said it did — corrected): `"abc"`/`null`/`[null]` …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S372):** **BRANCH/PR: S369 + S370 + S371 ran on `claude/improve-loop-swgwqx` (cut at `66dd97b` = `origin/main`; 15 commits) → draft PR #229. If it is still OPEN and unmerged, continue on it; if MERGED, cut a fresh `claude/*` off latest `origin/main` — never stack on merged history. Verify via the API** (0 check runs is correct — no workflows). **⚑ DRIFT CHECK: compare `origin/main` against `66dd97b`; anything beyond THIS loop's 15 commits is an outside-loop landing. `sw.js` v696, `FONT_MAKER_VERSION` 5.39** — re-read both. The clone is shallow (deepened to ~85 commits in S371); `git fetch --deepen=N origin main` before any range older than that.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), E (S358), L (S359), K (S360), N (S361), M (S362), G (S364), D (S365), I (S366), H (S367), C (S368), F (S369), B (S370), A (S371).** Unattended: take E (freshness & site health) over the S358–S371 delta `b418cf9..HEAD` — precache list vs files both ways, sitemap `lastmod` vs git (the loop branch has NOT run `update-sitemap.mjs` since S357 — check whether merged pages moved), `docs/reference/*` accuracy against the S363–S371 fixes (dashboard.md: the size setters clamp via `_sizeNum`; storage-and-backup.md: `hebrewBlender_hebFontSize` is clamped on BOTH readers; generator.md: the subtitle keys; the `.ivrit` engine's `ivritAskMode` hover rules), THIRD_PARTY_LICENSES, robots/CNAME; then L. **O next `torah_trainer.html`; D next dictionary (S253); G next dictionary (S252); H next Font Maker (S241); N next resources (S246); M next trope_tutor (S245); C next flash_cards (S271); F next the disabled-control look; the next A sweeps `1d4c9cf..HEAD` with the range-setter arm on the Font Maker's 13 sliders + torah/trope (`a371/junk.mjs` takes a STORES entry per page) and re-runs `fmrecents.mjs` after the P3 lands; the next I re-runs `i366/{gates,arms,corrupt,strshape}.mjs`.**

**⚑ Harness (standing): FONT MAKER probes pass `dismiss:false` to `b370/harness.mjs` `open()` — its `.overlay.open` sweep closes the wizard and `#wizName`/`#wizOpenProject` then time out; enter through the wizard (`#wizName`/`#wizAuthor` → `#wizNext` ×2 → `wizardCreate()`), Load menu = `#wizOpenProject` / the toolbar `toggleLoadMenu(event)` button, `#exportFormat` offers ttf/woff2/ttfwoff2/ufo, a project file = `JSON.stringify(project)` + `format:'hebrew-font-maker-project'` through `page.setInputFiles('#projFile', {buffer})`, Spacing = `setCatTab('spacing')` + drop `.collapsed`; torah stubs Sefaria by fulfilling `**/*sefaria.org/**` (`harness.mjs` `SEF`); hidden toggle inputs → click `label.toggle:has(#id)`; the HTTP server dies across a compaction — curl it first; hover = a REAL `page.mouse.move` + 400 ms, focus = a real Tab; dashboard: `frSkip()` BEFORE any header click, `openWeekEditor()` (+ `#swmAddPeriod`, `#swmSaveName`, `#swmSaveBtn`), `openSettings()`; the generator's sidebar is a drawer at 390; the dictionary's collapsed bodies need a REAL click on Expand-all-menus; flash: the practice screen is `#screenCard.active`; an `elementFromPoint` receipt precedes a real click; a nowrap width clone copies `getComputedStyle(n).font`; probe screenshot paths are cwd-relative; a junk-store cell = `context.addInitScript` writing the key, then a NaN/Infinity/undefined/[object Object] scan of `*Val` labels, inline styles and root custom props.**

**⚑ TOP UNGATED CANDIDATES (S371):** the Font Maker recents guard (P3, 1 line, `fmrecents.mjs` is the receipt); the dashboard swatch hover (1 rule); FM `.guide-rename` hit box (the `::after` idiom); the trope settings pair in the print block (1 selector pair); torah/trope `Number.isFinite` at 2 restore sites; the flash "Listening" badge space (1 line); the per-page sub-11px batches (resources ×45, dictionary ×21, generator, flash, privacy — `f369/census-en.txt`); the ~30 inline Font Maker hints under 11px; the tour-card h4 → h2 ×7; the remaining sub-floor M calls. Gate 2: the generator's two-subtitle slot (S371); the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on; the trope FAQ a7 copy. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. **Seed bench: 25 (unchanged; A is not an intake). Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

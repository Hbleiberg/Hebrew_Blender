...
- [ ] M | resources.html | **"Submit a font" is a `mailto:` while "Suggest a Resource" is a real form.** Measured 2026-09-01: `openSubmitFont` builds a `mailto:` with a pre-filled subject and body and sets `window.location.href`; the sibling flow one view away is a Web3Forms POST with 5 required fields, 18 choice pills and hCaptcha. So the contribution path that actually needs a **file …[full text: IMPROVEMENT_ARCHIVE.md] **Partially addressed outside the loop (FM v5.39, `4638e7b`):** the Font Maker end now carries the files itself (share sheet with the .ttf + LICENSE attached on Safari/iOS; in-page attach checklist elsewhere) and no longer routes through `?submit=`; the resources.html half stays a `mailto:` by maintainer decision (no server, no form) and the deep link is kept for external links.
...
- [x] 2026-09-03 | 4638e7b | Hebrew_Font_Maker.html, locales/ui-strings.csv, sw.js, docs/reference/font-maker.md | **Outside-loop feature (maintainer request), FM v5.39:** Exported ✓ → 📤 Send to IvritSuite builds the submission email in-page; share sheet with .ttf + LICENSE attached where `canShare({files})` allows, else mailto + persistent attach checklist (Download again / Save as… / LICENSE / Copy address); Exported ✓ also gains Share / Save to Files + Chromium Save as… picker; resources.html untouched. | Playwright: mailto, share resolve/Abort/NotAllowed, picker ×2, stale rebuild, no-file, EN+HE, light+dark, 1280+800, help/about, resources regression — 0 pageerrors. sw v647→v648.

...
- [x] 2026-09-18 | `7d7b89a` | classroom_dashboard.html + CSV + sw.js + reference | **"+1 min" on the timer — shipped (S401, seed S367).** `timerAddMinute()` adds 60 s to `timerRemaining` ONLY (Reset returns to the preset), keeps a running clock running, resumes a just-finished one, leaves a paused/idle one stopped; the "Time!" hold is now `timerFinishTimeout`, cleared by …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-18 | `92217cf` | classroom_dashboard.html + CSV | **"Everyone's here" clears the class's absence marks in one tap — shipped (S401, seed S367).** `clearAbsences()` beside `resetPickerCycle()`; one `.btn-xs` in `#pickerProgress` while `absent.length`, fair cycle on or off, gone when all-present; active class only, `picked` untouched, no `confirm()`, no new state. Seed premise …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S401 (2026-09-18):** all-page passes keep the *dedicated pass* the useful column. index (M S391, F S397); hebrew_blend_generator (G S392, **D S338 — D-next**); flash_cards (D S393, **H S264 — the stalest H tool**, G S279); hebrew_dictionary (`273f449`; **C S285 — C-next**); classroom_dashboard (**S401: 2 seeds shipped** `92217cf` `7d7b89a`; H S395); torah_trainer (**O-next — the ledgered O target, never O-audited**; N S248); trope_tutor (**S400: 3 seeds shipped** `4ce56bb` `3249dba` `c57eb53`; N S389, C S312); Hebrew_Font_Maker …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S402):** **BRANCH/PR: S401 shipped the two S-sized dashboard seeds (`92217cf` `7d7b89a`) + its close-out on `claude/improveloop-feature-seeds-mwdbk4` / draft PR #251, cut FRESH off `origin/main` `5f1941d` (PR #250 confirmed merged through the API). The branch was then FAST-FORWARDED ONTO MAIN at the maintainer's in-session word (so #251 reads merged, and the Pages run for that push is the deploy — confirm it concluded `success`); cut a FRESH branch off latest `origin/main` and never stack on merged history.** **DRIFT vs S400: NONE** — `origin/main` stayed `5f1941d` for the whole session. S401 closed at `sw.js` **v778**, FM **5.46** (untouched), SDK **2.116.0** (untouched); the backend numbers (migrations 0001–0003 `Live?` yes, `delete-account` untouched since `f141b7d`, keep-alive cron `41 4 * * *`) are STILL S399's readings, NOT re-read in S400 or S401. **Never trust these numbers unread.** S401 ran no pass, so the stalest-pass order below is unchanged.

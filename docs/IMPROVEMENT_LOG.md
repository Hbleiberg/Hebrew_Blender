...
- [ ] P4 (**NEW S340 Pass C — `sub-floor touch target`, the only sub-floor control on the hub in 8 cells + 2 phone cells**) | index.html | **The Resources card's "🔤 Hebrew Fonts →" `.card-attr` link paints 141×23 (12.8px text + 4px padding-block, `display:block` as a flex item)**, one pixel under the 24px WCAG 2.5.8 floor; its Sefaria sibling is 36px. `min-block-size:24px` on `.card-attr` lifts …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-03 | 36f2a5a | torah_trainer.html | **The custom picker's chapter box follows the chosen book** (the S336 Pass H paper-cut). Nothing listened to the Book select, so after switching to Deuteronomy the box still accepted chapter 45 as valid (Genesis's 50) and the teacher met Sefaria's error. A `change` listener sets `max` from `TORAH_BOOK_CHAPTERS` and clamps; Go clamps once more and …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-03 | fa4dce9 | torah_trainer.html | **Narrowing a holiday reading keeps the holiday's name** (the S336 Pass H paper-cut). `applyCustomRange` nulled `customLabel` and `holidayKey`, so "Rosh Hashanah — Day 1 — Genesis 21:1-34" narrowed to verses 1–8 rendered a bare "Genesis 21:1-8" in the tab, the print band, the strip chip and the saved reading. When the new ref lies inside the …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S340 (2026-09-04):** index **2026-09-04 (×2, `b46648b` Merge/Replace dialog + `dc9bb62` bookmark live region; S323 ×2)**; trope_tutor 2026-09-04 (S339 ×1, `fea54f5`); flash_cards 2026-09-04 (S339 ×1, `f001815`); hebrew_blend_generator 2026-09-04 (S338 ×1, `5b55d19`); Hebrew_Font_Maker 2026-09-04 (S338 ×1, `fc73fd9`; outside-loop v5.39); torah_trainer 2026-09-04 (S338 ×1, `f92ae1d`); classroom_dashboard 2026-09-03 (S336 ×1, `009611b`); locales/ui-strings.csv 2026-09-03 (S334 `fa4dce9`); hebrew_dictionary 2026-09-03 (S333 ×1); resources …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`sub-floor touch target`**: **1 new carrier found 2026-09-04 (S340 Pass C runtime census on the hub, 8 cells + 2 phone cells): the Resources card's `.card-attr` Fonts link 141×23 (12.8px text + 4px padding-block) — logged P4 with the one-line fix (`min-block-size:24px`), the …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S341):** **BRANCH/PR: S340 CONTINUED `claude/improve-loop-0mbq58` → draft PR #223 (head after this close-out; base `origin/main` `4b66a64`). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v664, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS O (S315, attended only), THEN F (S328), B (S329), A (S330), E (S331), L (S332), K (S333), N (S334), M (S335), H (S336), G (S337), D (S338), I (S339), C (S340).** **Unattended: skip O and take F** (cross-tool consistency, one affordance). **Affordance suggested by S340's headline:** the hand-rolled dialog contract — every page's non-`<dialog>` overlay (the `.ivrit` Merge/Replace prompt on its 4 carriers, each tool's tour card, the FM `askModal`, the dashboard/flash-cards modals, the generator's `.overlay`s): role + label, focus in, Tab trap, Escape, focus restore — the hub's copy had silently missed the engine's clause; measure the others the S340 way (open each by keyboard, Tab ×12, Escape, `DESC` of the focus). O's surface is still flash_cards.html. **Harness notes (standing):** CSP forbids string predicates — pass functions to `waitForFunction`; the raw-i18n-key detector must skip SCRIPT/STYLE text nodes; never run the FM wizard arm beside another harness process; Playwright refusing an `aria-disabled="true"` click on a primary action the teacher has just fixed is a FINDING; a harness module that runs an arm at import time re-runs it in every importer — gate the arm on `process.argv`. **Ledger lesson (standing):** write open candidates without the `NON_ACTIONABLE` phrases or `compact-ledger` files them.

**⚑ THE STRONGEST UNTAKEN CANDIDATE IS THE HUB'S 23px FONTS LINK (P4, `sub-floor touch target`, one CSS line — take it first at S341 while the hub is uncapped);** the rest are gated or decisions — the four unauthored picker terms (gate 2); the staging author URL on the hub (gate 2); the specimen-A4-vs-template-Letter paper choice (gate 2); the real-words 7.2px grid (layout decision); the help tab strip (gate 3); the S321 timer placeholder copy (gate 2). **For the next A sweep:** `stale-validation-note-after-its-input-changes` wants its first suite-wide runtime sweep; `uncompressed-jspdf-raster` continues its retirement streak; `blocking-alert-for-a-routine-path` still wants its dashboard click census; the S335 runtime arm for `background:#fff` / `color:#1a2744` on the 8 other pages still stands. **Seed bench (unchanged):** the holiday share link (torah, S336 — ~15 lines, the smallest seed), the handout selection (torah, S336), the scroll-style handout (torah, S324), the last-backup date (index, S323).

...
- [ ] P4 (**NEW S335 — found under Pass M; K's shape (English passed as a plain argument); not taken because the Font Maker hit the per-tool cap**) | Hebrew_Font_Maker.html | **Letter names reach the Hebrew UI in English wherever `.name` is passed raw while the tiles use `gName()`:** the "Editing: א — Alef" header (`updateCurrentGlyphLabel`), the drop zone "Drop an image for Alef", the mark …[full text: IMPROVEMENT_ARCHIVE.md]
...
- 2026-09-03 | **S335** | iters: 1 pass (**M**) + 2 fixes = **3** (4th/5th unspent — Hebrew_Font_Maker AT CAP after two M fixes; the pass's third finding is K's shape on the same file; every other open candidate is gate-2/3, a layout decision or an F/C convergence question) | tools touched: …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S335 (2026-09-03):** Hebrew_Font_Maker **2026-09-03 (×2, `3f43d90` mark-editor pills, `cea6479` stacked seam; S333 ×2; outside-loop v5.39)**; torah_trainer 2026-09-03 (S334 ×2); locales/ui-strings.csv 2026-09-03 (S334 `fa4dce9`); classroom_dashboard 2026-09-03 (S333 ×2); flash_cards + hebrew_blend_generator + trope_tutor 2026-09-03 (S333 ×2 each); hebrew_dictionary 2026-09-03 (S333 ×1); resources 2026-09-03 (S333 ×1); contact/privacy/terms 2026-09-03 (S332 ×1 each); index 2026-09-03 (S330 ×1); 404 untouched since its last E/L fix.
...
**Next session (S336):** **BRANCH/PR: S335 CONTINUED `claude/improve-loop-0mbq58` → draft PR #223 (head after this close-out; base `origin/main` `4b66a64`). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v659, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS O (S315, attended only), THEN H (S323), G (S324), D (S325), I (S326), C (S327), F (S328), B (S329), A (S330), E (S331), L (S332), K (S333), N (S334), M (S335).** **Unattended: skip O and take H** (teacher walkthrough, one tool: role-play a real lesson end-to-end; frictions → P3 candidates, missing small affordances → Feature seeds; never re-log a seed the maintainer struck). H's per-tool history as the ARCHIVE's Pass H filings record it (re-derive with `grep -o 'S[0-9]* Pass H[^|]*| [A-Za-z_]*\.html'`): generator **S130**, torah S217, trope S228, FM S241, flash_cards S264, dashboard S278, dictionary S292, resources S305, index S323. **`hebrew_blend_generator.html` (S130, ~205 sessions — the suite's most-changed tool since: word search, real words, the S327 C work, the S330/S332 share-link gates) is the pick unless the re-derivation finds a later generator H.** O's surface is still flash_cards.html.

**⚑ THE STRONGEST UNTAKEN CANDIDATES:** the S335 K shape (raw `.name` letter names in the FM's Hebrew UI — 3 persistent sites + 4 toasts, one `gName()` each; FM was at cap); the four unauthored picker terms (gate 2); the staging author URL on the hub (gate 2); the real-words 7.2px grid (layout decision); the 38×22 `.toggle` on torah + trope (C); the dashboard `.tip-icon`-vs-`.tip-wrap` divergence (F); the help tab strip (gate 3, screenshots in the S311 line). **For the next A sweep:** the S335 dark-parity hit was the FM's only raw light-token rule, but a static count finds `background:#fff` / `color:#1a2744` in CSS on 8 other pages (generator 19, torah 5, dashboard 4, flash_cards 3) — worth one runtime arm with the S335 computed-colour detector before registering a pattern. Seed bench: the scroll-style handout (torah, S324), the last-backup date (index, S323).

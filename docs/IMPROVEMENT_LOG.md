...
- [x] 2026-09-16 | `b4afd93` | flash_cards.html | **Locked buttons stop lighting up on hover — the 9th carrier of `hover-feedback-survives-the-disabled-state` and the FIRST found by arm (b) of the definition S387 widened.** Two carriers, two idioms, two different failures: `#savePresetBtn` (`aria-disabled` in the markup, so locked the moment Teacher Tools opens) had opacity 1, cursor pointer and …[full text: IMPROVEMENT_ARCHIVE.md]
...
- 2026-09-16 | **S391** | iters: 1 pass (**M**) + 4 fixes = **5** (full budget) | tools: index (`fa001b0`), scripts+harness (`bfcdc15`), generator (`95fe5ec`), Font Maker (`d5ee54c`) | patterns fixed: hover-feedback-survives-the-disabled-state ×2 (3 controls, closing all 3 open unconfirmed …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`hover-feedback-survives-the-disabled-state`** (registered 2026-09-15, S383 Pass F): ACTIVE. **S391 (Pass M session, ×2): the TENTH and ELEVENTH carriers FIXED (`95fe5ec` generator, `d5ee54c` Font Maker), closing ALL THREE of the carriers S388 left open and unconfirmed** — …[full text: IMPROVEMENT_ARCHIVE.md]
...
- M aesthetics & visual design (one surface): 2026-09-16 (**S391 — 17th-ever M, `index.html`, its 2nd (S246 → S391): 48 commits and +512/-66 between, and the FIRST aesthetics look at the accounts-era hub panels (cloud saves, My Fonts manager, last-backup line, erase gate). Stalest unattended pass for a second session running, and the pointer named both pass and surface. 8 cells (EN/HE × …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S392):** **BRANCH/PR: S391 ran on `claude/loving-einstein-cdmbbz`, cut FRESH off `origin/main` `b304db0` because the S390 branch's PR #244 was MERGED and no PR was open at session start. 4 fix commits + this close-out, draft PR opened at close-out. If that PR is open and unmerged, CONTINUE on it; if merged, cut fresh off latest `origin/main` again — never stack on merged history.** **⚑ DRIFT CHECK: S391 found 7 outside-loop commits — `sw.js` v759→v765 and FM 5.44→**5.45** (six Font Maker fixes shipped as v5.45), plus `b304db0`, the CLAUDE.md + improveloop backend audit. Re-check against `b304db0` + this loop's commits; never trust these numbers unread.** After S391: `sw.js` **v766**, FM **5.45**, migrations 0001–0003 all `Live? = yes`, SDK pin 2.116.0, keep-alive run #4 green. **A fresh container starts SHALLOW — `git fetch --unshallow` FIRST: S391's `origin/main` read 65 commits stale (`12450a8`) before unshallowing, which would have made the entire drift check a fiction.** The `--sdk` fixture: `npm pack @supabase/supabase-js@2.116.0` (npm IS reachable; the CDNs are not), untar, pass `package/dist/umd/supabase.js` — **pass it to `smoke-tools.mjs` too or its signed-in arms silently SKIP**. The smokes hardcode port 8080 — kill your own server first, one at a time. **Never `pkill -f` a server: it matches and kills this shell. Kill by PID.**

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), G (S377), D (S378), I (S379), H (S380), C (S381), F (S383), B (S384), A (S385), E (S386), L (S387), K (S388), N (S389), P (S390), M (S391).** Unattended: take **G** (print & export fidelity) — 14 sessions stale and now the longest-standing unattended gap. Re-derive its surface by staleness: the **generator** (the printed worksheet is the suite's core artifact) and the **dashboard** are the strongest candidates; G owns paper AND every exported file, so print-preview on Letter and A4 and inspect the artifacts themselves. Then D (S378 — next flash_cards or the FM), I (re-run `i379/gates.mjs`), **H next the dashboard (S46, still the stalest tool pairing in the table)**, C, F, B, A, E, L, K, N, P, and M (next: a chrome page never M-audited — `account.html` has never had one).

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1–25 are S384–S390's, in full in loop-findings — read them there first.** **(26) NEW S391 — a composite-contrast probe that reads `backgroundColor` off the element's own parent gets `rgba(0,0,0,0)` from a transparent surface and reports `1.000:1` in BOTH themes: a false clean that looks exactly like "no difference, nothing to see". Walk up to the first ancestor whose background alpha > 0.01.** **(27) NEW S391 — a hover probe's PROPERTY LIST is the detector. `.btn-primary`'s hover is `filter: brightness(1.06)`, so a probe reading background/color/border/opacity/shadow/transform reported its control SILENT; the page was fine and the probe was blind. Include `filter`, and treat a silent control as a diagnosis, never a discard.** **(28) NEW S391 — adding a `:not()` guard RAISES specificity and can flip a pair the page ordered deliberately. Measure the ranking both ways before and after; use `:where()` (zero specificity) whenever the rule is *meant* to lose.** **(29) NEW S391 — this suite's dark tokens are overridden on `body.dark`, NOT `:root`, so `getComputedStyle(document.documentElement).getPropertyValue('--border')` returns the LIGHT value in a dark cell. Read tokens off `body`.** **(30) NEW S391 — an element that is not visible at the test width returns "no diff" from a hover probe and reads as "guarded". That is a VOID, not a pass (`#printBtnMob` at both 1280 and 390).**

**⚑ TOP UNGATED CANDIDATES (S391): the Edge Function's production `localhost` origins** (P4, backend boundary — maintainer redeploy); **"sign out everywhere"** (P4, `scope: 'global'` + one account-page control); **`clampMenu()` on resize/orientationchange** (P4); **the FM chip's unmeasured phone geometry** (P4, behind its wizard); **torah `.btn`, the LAST open carrier of the hover pattern** (needs a loaded parsha to drive); **the tour-card h4 → h2 ×7**; **the sub-floor touch targets**. Gate 2: the flash-card swatch labels; the generator's `restoreLastSetup` corrupt-drop; the dictionary drag-freeze + select-all cap; the corpus's split article tokens; the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the resources Taamim fallback; the FM's `nexthint_nikkud` copy; **the 5 unreferenced credit keys (S360)**. Gate 3/M: the FM stage annotations; `account.html`'s dark `.btn.ghost`/`.btn.danger`; **`.dash-edit-pencil`'s side in Hebrew**; **the hub's seven-size 2.4px type band (S391 measurement)**. Gate 4: the flash-card sticky-bar and landscape proposals. **Seed bench: 26.** **⚑ RESOLVED (skill, maintainer-ratified): the shared-script cap is one such fix per session — any of `js/i18n.js`, `pwa.js`, the four backend modules — and it does not spend the per-page touches.**

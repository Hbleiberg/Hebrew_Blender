...
- [ ] P3 (**NEW S398 Pass B — GATE 2 (copy); the mechanical half shipped as `c71b556`, this is the wording half**) | resources.html | **With js.hcaptcha.com blocked, Suggest-a-Resource's submit says `resources.suggest.captcha_required` — "please complete the check" — when there is no check on screen to complete**, so the teacher is asked for something that is not there. `contact.html` solved …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-17 | (S398 close-out) | branch/deploy note | **S398 CONTINUED on `claude/improve-loop-e8ak03` / PR #248 (open, unmerged, `mergeable_state` clean, 0 review threads, 0 CI runs — neither authored workflow's push paths match it and Pages runs only on `main`). DRIFT: NONE — `origin/main` still `ef75c79`, `sw.js` v774, FM 5.46, SDK 2.116.0, migrations 0001–0003 all `Live?` yes, …[full text: IMPROVEMENT_ARCHIVE.md]
...
- 2026-09-17 | **S398** | iters: 1 pass (**B**) + 4 fixes = **5** (full budget) | tools: hebrew_blend_generator ×2 (`12ecb5f`, `3f93c03`), resources (`c71b556`), hebrew_dictionary ×2 (`3f93c03`, `e53b78e`), classroom_dashboard + torah_trainer (`3f93c03`) | patterns fixed: …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S398 (2026-09-17):** all-page passes keep the *dedicated pass* the useful column. index (M S391, F S397); hebrew_blend_generator (`12ecb5f`+`3f93c03`; G S392, **D S338 — D-next**); flash_cards (D S393, **H S264 — the stalest H tool**, G S279); hebrew_dictionary (`3f93c03`+`e53b78e`; **C S285 — C-next**); classroom_dashboard (`3f93c03`; H S395); torah_trainer (`3f93c03`; N S248); trope_tutor (N S389, C S312); Hebrew_Font_Maker (**B S398 — its +313 outside-loop lines, driven**; H S380); account (C S396, P S390); resources (`c71b556`; **never …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`cdn-dependency-absent-and-unguarded`** (**NEW, registered 2026-09-17 (S398 Pass B)**): ACTIVE — **0 clean sweeps yet (1 hit + 1 unbounded retry fixed this sweep).** A global a third-party `<script src>` defines, or a retry waiting for one, reached where nothing handles its …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S399):** **BRANCH/PR: S398 CONTINUED S397's branch `claude/improve-loop-e8ak03` and PR #248 — 9 commits now. If #248 is still open and unmerged, CONTINUE on it; if merged, cut a FRESH branch off latest `origin/main` and never stack on merged history.** **/!\ DRIFT: NONE this session** — `origin/main` stayed `ef75c79` throughout, and the +313 Font Maker lines the S398 pointer flagged have now been driven by pass B (clean). S398 closed at `sw.js` **v775**, FM **5.46**, SDK **2.116.0**, migrations 0001–0003 all `Live?` yes, `delete-account` untouched since `f141b7d`, keep-alive's last scheduled run green. **Never trust these numbers unread.**

**⚑ /!\ RE-DERIVE THE POINTER'S REASON, DO NOT INHERIT IT — a FOURTH session running, and this time it corrected a measurement, not a date.** S398 re-measured the tour outlines before shipping the maintainer's four and found **S397's stated reason for excluding trope was wrong**: trope's 3 h2s and its only h3 are inside the hidden results screen, so its RENDERED outline at load is `h1,h4` and it does skip. The four shipped are still exactly right; the reason on the bench was not. **Measure what renders, not what the DOM contains** — that is the same mistake as S397's `<h4>` grep, one layer up. Per-tool history: `IMPROVEMENT_ARCHIVE.md`; measurements: `loop-findings.md`. **Grep loop-findings for the file before auditing it.**

**⚑ STALEST PASS: O (S346, attended-only), A (S385), E (S386), L (S387), K (S388), N (S389), P (S390), M (S391), G (S392), D (S393), I (S394), H (S395), C (S396), F (S397), B (S398).** **O is now 53 session-numbers stale and S398 proved the detector still clones (`skill-v4.1.3` tag, worktree, the 4 parser modules) — if the next session is attended, OFFER O FIRST and let the maintainer decide; S398 asked in one AskUserQuestion before the pass and it cost one round trip.** Unattended: take **A** (2nd sweep of `selector-declared-twice-in-one-stylesheet` — run `i394/dupcss2.py`, do not re-derive; the first sweep of `light-literal-text-on-the-gold-token` past its 2 carriers; and the FIRST sweep of the new `cdn-dependency-absent-and-unguarded`, whose detection definition and 4-global census are in its Pattern-health row). Then E, L, K, N, P, M (next: `account.html`), G (next: `flash_cards.html`, S279), D (next: the **generator**, S338), H (next: **`flash_cards.html`, S264, the stalest H tool**), C (next: **`hebrew_dictionary.html`, S285** — or one of `resources`/`contact`/`privacy`/`terms`/`404`, which have never had a dedicated pass of any kind; `resources` is the strongest, it now has two open candidates).

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1–34 are S384–S392's, 35–40 S393's, 41–47 S394's, 48–54 S395's, 55–61 S396's, 62–65 S397's, 66–67 S398's — all of them in full in loop-findings, read them there first.** The S398 two in one line each: **(66)** `offsetParent !== null` is **always false for a `position:fixed` element**, so a visibility assertion built on it reports every fixed modal, backdrop, floating button and toolbar as hidden — S398 nearly filed a working modal as broken; use `getComputedStyle(el).display !== 'none'` plus a non-zero `getBoundingClientRect()`, and for a `.open`-class overlay assert the class too. **(67)** a background command's output file here keeps only the TAIL — a long JSON census printed to stdout comes back unparseable with its head missing; write results to a file in the scratchpad and print a one-line summary. **Also standing: `compact-ledger.mjs --apply` is NOT idempotent — run it ONCE; it tags its archive heading from the handoff pointer, so write the new pointer BEFORE compacting; and the outgoing handoff is NOT archived by the script — move it yourself, the schema requires it.**

**⚑ TOP UNGATED CANDIDATES (S398): the bench turned over — three of S397's four strongest shipped this session.** Strongest now: the **flash cards / Font Maker / trope heading-level trio** (all three skip, none of them because of the tour card — flash `h1,h3×8` and FM `h1,h3×16` jump 1→3, trope `h1,h4` jumps 1→4; fixing means deciding what each page's h2 IS, so it is a structure call, not a tag swap). Then the **Font Maker's `downloadTemplate` PNG branch**, which tests only `typeof html2canvas` and shows the PDF branch's message for a jsPDF-only outage (one condition, existing copy). Held back, unchanged: **account's `#holdsList` list-semantics** (needs a WebKit engine, NOT an assertion); **the sub-floor touch targets** (generator `.bingo-step`, FM `#rulerCorner` — gate 3); **"sign out everywhere"** (gate 1/2); **the Edge Function's `localhost` origins** (backend boundary — maintainer redeploy). The 13 open `selector-declared-twice` carriers are every one DIVERGENT or a judgement call: the next A there is a census and an intent decision, not a fix list. `hover-feedback-survives-the-disabled-state` still has NO open carrier.

**⚑ GATE BENCH — one new entry, the rest unchanged from S396; the full list is in the archive, grep `GATE-2 BENCH (unchanged)`.** **NEW gate 2: resources' Suggest-a-Resource says "complete the check" when a filtered hCaptcha means there is no check on screen** — `contact.html` already has the `widgetMissing` branch to copy, so this is one branch plus a key pair. Gate 2's other strongest is still the generator's Header Labels translating only Name/Date (~10 sites, authoring, not wiring). Gate 3/M: the FM stage annotations; account's dark `.btn.ghost`/`.btn.danger` (S386: specificity, not a leak); `.dash-edit-pencil`'s side in Hebrew; the hub's 2.4px type band. Gate 4: the flash-card sticky-bar and landscape proposals. **Seed bench: 26** — untouched in S398 (budget went to the pass and its four fixes).

...
- [ ] P4 (**NEW S397 Pass F — measured, and the one page where the header is not the first thing a keyboard reaches**) | hebrew_dictionary.html | **The floating `#fsBtn` (Toggle fullscreen) sits before `<header>` in DOM order, so it was tab stop 1 on the page and is now stop 2, ahead of the whole header.** Every other page puts its fullscreen control inside the header or after the content. …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-17 | `1348930` | 14 root pages | **Every page offers the same keyboard bypass of the header** (S397 Pass F, P4 — the pass's headline). Only torah and trope had one; chrome stops before the first content control were 6/6/6/5/4/3/3/3/3/3/2. The other twelve get the existing rule, each pointing at its own main element (`tabindex="-1"`). Two corrections while making it one rule: …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S397 (2026-09-17):** every root page was touched 2026-09-17 by the `1348930`/`bc6df0f` skip-link sweep, so the useful column is the last *dedicated pass*: index (M S391, F S397); hebrew_blend_generator (G S392, A S385, **D S338 — D-next**); flash_cards (D S393, **H S264 — the stalest H tool**, G S279); hebrew_dictionary (`7e8b427`; **C S285 — C-next**); classroom_dashboard (`7e8b427`; H S395, K S388, D S325); Hebrew_Font_Maker (`ecb9706`; H S380, C S368 — **+313 outside-loop lines unaudited**); torah_trainer (I S394, N S389, C S298); …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S398):** **BRANCH/PR: S397 cut a FRESH branch `claude/improve-loop-e8ak03` off latest `origin/main` (`ef75c79`) because PR #246 had MERGED (`dd6c6c5`) — never stack on merged history. 5 commits + close-out, draft PR opened. If it is still open and unmerged, CONTINUE on it; if merged, cut fresh again.** **/!\ DRIFT: YES — the six-session run of "none" ENDED.** `origin/main` `b304db0` → `ef75c79`: the #246 merge plus **5 outside-loop Font Maker commits, +313 lines to `Hebrew_Font_Maker.html`** (punctuation tab, round-pen pressure preview, Preview-PDF digits, Save-all-drawings, the 5.46 release). **That is the unaudited surface, and prime discovery territory.** `sw.js` v773 and FM 5.46 landed there, not in the loop; S397 closed at **v774**, FM **5.46**, SDK 2.116.0. The clone arrived **SHALLOW** again — `git fetch --unshallow` before `update-sitemap.mjs`. Backend read live, unchanged (detail in the close-out entry). **Never trust these numbers unread.**

**⚑ /!\ RE-DERIVE THE POINTER'S REASON, DO NOT INHERIT IT — a THIRD session running, this mattered.** S395's pointer had the dashboard's last H wrong, S396's trope's C history, and S397 found the **S354 tour-heading candidate wrong on 3 of its 7 carriers** — a bench item that would have shipped a ×7 change on a false premise. Per-tool history: `IMPROVEMENT_ARCHIVE.md`; measurements: `loop-findings.md`. **Grep loop-findings for the file before auditing it.**

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), B (S384), A (S385), E (S386), L (S387), K (S388), N (S389), P (S390), M (S391), G (S392), D (S393), I (S394), H (S395), C (S396), F (S397).** Unattended: take **B** (console & error audit) — 14 sessions stale and now the longest unattended gap — and point it at the **FM's +313 outside-loop lines** (new drawing paths, a Preview-PDF branch, a "Save all drawings" control), which no pass has loaded. Then **A** (2nd sweep of `selector-declared-twice-in-one-stylesheet` — run `i394/dupcss2.py`, do not re-derive; ALSO the first sweep of `light-literal-text-on-the-gold-token` past its 2 carriers), E, L, K, N, P, M (next: `account.html`, a chrome page never M-audited), G (next: `flash_cards.html`, S279), D (next: the **generator**, S338), H (next: **`flash_cards.html`, S264, the stalest H tool**), C (next: **`hebrew_dictionary.html`, S285** — or one of `resources`/`contact`/`privacy`/`terms`/`404`, which have never had a dedicated pass of any kind) and F.

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1–34 are S384–S392's, 35–40 S393's, 41–47 S394's, 48–54 S395's, 55–61 S396's, 62–65 S397's — all of them in full in loop-findings, read them there first.** The S397 four in one line each: **(62)** the language key is **`hebrewBlender_lang`**, NOT `ivritSuite_lang` — the wrong one runs the EN arm twice and **every HE cell passes**, so assert `getComputedStyle(document.documentElement).direction` as a hard control inside the cell. **(63)** a generic `.overlay.open` sweep **pre-empts a page's own close function** — on the FM `aCloseModal` then never runs, its focus handoff never fires, and the cell reports a defect that is the probe's; run the page-specific close FIRST. **(64)** never bulk-`sort()` `locales/ui-strings.csv` to place a row — it moved **1559 unrelated lines**; insert each row at its own sorted position and check `git diff --numstat`. **(65)** grepping heading tags over-counts what renders: the FM greps 26 `<h4>` and renders **7** (help prose, template literals) — census headings in the DOM. **Also: `compact-ledger.mjs --apply` is NOT idempotent — twice archives the same block twice. Run it ONCE. S397 also fixed its size report, which counted UTF-16 units while the limit and `check-ledger` are BYTES (~1.5 KB apart here): when it says it is still over, trim your own new text — the fixed TRUNC widths cannot.**

**⚑ TOP UNGATED CANDIDATES (S397): the bench is still thin, and honestly so.** Strongest, now **maintainer-scoped**: the **tour-card `h4` → `h2` on the FOUR pages that actually skip** (generator, dictionary, dashboard, torah — maintainer: "fix only those four, do not converge all seven"), `.tour-card h4` re-pointed on each; CLAUDE.md UX rule 1 still wants the tour engine extracted next time one is *engine*-touched. Then: the **dictionary's `#fsBtn` before `<header>`** in DOM order (the only page where a control precedes the header — one line, but it is `position:fixed`: check load order); **flash cards' missing h2** (L's arm). Held back, unchanged: **account's `#holdsList` list-semantics** (Safari/VoiceOver — needs a WebKit engine, NOT an assertion); **the sub-floor touch targets** (generator `.bingo-step`, FM `#rulerCorner` — gate 3); **"sign out everywhere"** (gate 1/2); **the Edge Function's `localhost` origins** (backend boundary — maintainer redeploy). The 13 open `selector-declared-twice` carriers are every one DIVERGENT or a judgement call: the next A there is a census and an intent decision, not a fix list. `hover-feedback-survives-the-disabled-state` still has NO open carrier.

**⚑ GATE BENCH — unchanged from S396; the full list is in the archive, grep `GATE-2 BENCH (unchanged)`.** Gate 2's strongest is still the generator's Header Labels translating only Name/Date (~10 sites, authoring, not wiring). Gate 3/M: the FM stage annotations; account's dark `.btn.ghost`/`.btn.danger` (S386: specificity, not a leak); `.dash-edit-pencil`'s side in Hebrew; the hub's 2.4px type band. Gate 4: the flash-card sticky-bar and landscape proposals. **Seed bench: 26** — untouched in S397 (budget went to the pass and its fixes).

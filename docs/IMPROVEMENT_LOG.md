...
- [ ] P3 (**S383 Pass F, 5 carriers — 3 SHIPPED S384 (`a585608` FM, `df9ff3b` contact + resources); the 2 left are a DIFFERENT SHAPE and need per-variant rules, analysis below so nobody re-derives it**) | trope_tutor.html, account.html | **Both key off `[aria-disabled="true"]`, not `:disabled`, and both carry VARIANT classes with their own rest colours — trope `.btn btn-secondary` (its own …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S385 (2026-09-15):** hebrew_blend_generator **2026-09-15 (`0a46f67`; A S385, P-audited S382)**; trope_tutor **2026-09-15 (`7be7591`; A S385, M-audited S376)**; classroom_dashboard **2026-09-15 (`902d3ff`, `9614a80`; A S385 + B S384, H stalest since S46)**; Hebrew_Font_Maker 2026-09-15 (`a585608`; B S384, H-audited S380); contact 2026-09-15 (`df9ff3b`; B S384); resources 2026-09-15 (`df9ff3b`; B S384, N-audited S375); flash_cards 2026-09-15 (`a234f9f`; F S383, C-audited S381); hebrew_dictionary 2026-09-15 (`3674d6e`; F S383, D/G-audited …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S386):** **BRANCH/PR: S382+S383+S384+S385 all ran on `claude/wizardly-tesla-xu0ikd` (PR #242, draft, open, unmerged; base `4d6c02d`), now 15 commits + 4 close-outs. If that PR is still open and unmerged, CONTINUE on it; if merged, cut a fresh `claude/*` off latest `origin/main` and open a new draft PR at close-out. Verify via the API** (no workflow triggers on `pull_request`, so `get_status` reads `pending` with `total_count: 0` — GitHub's default for a commit with no legacy statuses, NOT a queued check. `get_check_runs` is the one that answers: exactly 1 run, the Supabase GitHub App's **`Supabase Preview`**, concluded **`skipped`** — correct, because migrations live in `db/` and not `supabase/` precisely so the integration never spins up a paid preview branch. A `skipped` Supabase Preview is a green PR, not a failure to chase.) **⚑ DRIFT CHECK: compare `origin/main` against `4d6c02d` + this loop's commits; S383, S384 and S385 all found NONE, S382 the largest yet — CHECK IT, never trust this unread. `sw.js` v752, FM 5.44, migrations 0001–0003 all `Live? = yes`.** A fresh container starts SHALLOW — `git fetch --unshallow` before `update-sitemap.mjs`. The `--sdk` fixture the required smokes need: `npm pack @supabase/supabase-js@2.116.0` (the npm registry IS reachable; the CDNs are not), untar, pass `package/dist/umd/supabase.js`; **pass `--sdk` to `smoke-tools.mjs` too or its signed-in B arms silently SKIP** (114 checks without it, 135 with).

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), E (S372), L (S373), K (S374), N (S375), M (S376), G (S377), D (S378), I (S379), H (S380), C (S381), P (S382), F (S383), B (S384), A (S385).** Unattended: take **E** (freshness & site health) — 13 sessions stale, and **E has STILL never seen the backend landing**, which is exactly its business: the 4 backend `js/` modules and `account.html` belong in `sw.js` CORE_ASSETS, and the two account origins (`cdn.jsdelivr.net` in `script-src`, `hhkmqwpjsyxdeuhvcyis.supabase.co` in `connect-src`) must be present in the CSP of every page carrying the account scripts and nowhere else. Re-run its 17 arms over `408dbe4..HEAD`. Then L (vs `4a1f186`). O next `torah_trainer.html`; C next torah (S298) then dictionary (S285); M next `index.html` (S246) then torah; N next trope (S248) then index (S249); D next flash_cards or the Font Maker; H next the dashboard (S46) or the generator (S56); P next the dashboard or the dictionary; F next affordance: the disabled CURSOR idiom (S383 measured `not-allowed` 9 / `default` 17 but did not converge) or the empty-state re-sweep; the next K over `4a1f186..HEAD`; the next I re-runs `i379/gates.mjs`; **the next A sweeps `b8ba5ef..HEAD`, re-reads the 3 rows S385 left at clean streak 1 (`vh-capped-sheet-without-dvh-twin` needs 2 more A sweeps to retire) and owes the 2 open hover-vs-disabled carriers a decision.**

**⚑ RUNTIME-PROBE HARNESS (standing; S384 + S385 traps, all in loop-findings).** (1) **`hebrewBlender_darkMode` stores `'1'`/`'0'`, NOT `'true'`** — a probe writing `'true'` runs LIGHT while reporting a dark sweep; assert `body.dark` per cell and void the cell otherwise. Better: read a theme-valued token back (e.g. `--shadow-sm` .05 navy vs .4 black) so the cell proves its own theme. (2) **Playwright matches routes in REVERSE registration order** — catch-all abort FIRST, specific stubs AFTER. (3) **`requestfailed` never fires for a 404**; control that detector with a routed `abort()`. (4) **Never hand-roll an accessible-name walker** — use `page.accessibility.snapshot({root})`; a walker without the `img[alt]` fallback invents nameless-control findings. (5) **Census a dialog OPEN** — the account chip's menu reports `named:false` closed and is correctly named open. (6) **Seed `hebrewDashboard_setupSeen`** or `#frBackdrop` (the first-run modal, not an `.overlay`) covers every dashboard control and every hover reads as not landing. (7) **A planted-defect control that goes UNDETECTED can be the page being immune** — on a print-shadow sweep `planted:false` means the page already carries a blanket reset; read plant + result together. (8) `ivritRestore` BLOCKS on the Merge/Replace modal — fire it WITHOUT awaiting inside `page.evaluate`, then click `.ivrit-ask-btn[data-act="merge"]`; give every wrong-shape import a FRESH CONTEXT or shape N+1 inherits N's damage.

**⚑ TOP UNGATED CANDIDATES (S385): the 2 remaining hover-vs-disabled carriers (trope `.btn`, account `.btn` — both `[aria-disabled]` + variant classes needing per-variant restate rules; account's `#delGo` is `class="btn danger" aria-disabled="true"`, a locked DESTRUCTIVE control that still lights red — the analysis is in the candidate, do not re-derive it; S385 found the accounts layer's own classes already guarded); the wrong-shape `.ivrit` axis on the five liveState blobs B's arm 4 never drove (`b384/arm4b.mjs` takes a page + key); the dictionary's inline `style.opacity` disabled hack; the tour-card h4 → h2 ×7 (the "next touch of a tour engine" — extract the shared block in the same session); the remaining sub-floor M calls (bingo stepper, scaled toggles, `#rulerCorner`). Decide-then-sweep: whether an action TOOLBAR should rove like a selection grid; whether the disabled CURSOR should converge. Gate 2: the flash-card swatch labels; the generator's `restoreLastSetup` corrupt-drop; the dictionary drag-freeze + select-all cap; the corpus's split article tokens; the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the resources preview Taamim fallback. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals. **Seed bench: 26. Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

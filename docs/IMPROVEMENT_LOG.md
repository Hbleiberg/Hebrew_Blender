...
- [x] 2026-09-16 | (S391 close-out) | branch/deploy note | **S391 ran on `claude/loving-einstein-cdmbbz`, cut fresh off `origin/main` `b304db0` — the S390 branch's PR #244 is MERGED, and no PR was open at session start (verified via the API, not assumed).** **Drift check: the FIRST non-trivial drift in two sessions — 7 commits landed outside the loop, `sw.js` v759→**v765** and …[full text: …[full …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | `fa001b0` | index.html | **The Import/Export modal's four dividers follow the theme** (S391 Pass M, P3). Three visible section rules + the JS-built My Fonts row separator hardcoded `rgba(0,0,0,0.12)`/`0.08`, so on the dark modal surface the line composited DARKER than what it divides: **1.320:1 light → 1.053:1 dark, a 4-unit RGB step**. All four now use `var(--border)`, the …[full text: IMPROVEMENT_ARCHIVE.md]

## Metrics

### Per-session log (one line per session)

- 2026-09-17 | **S395** | iters: 1 pass (**H**) + 3 fixes + 1 measurement = **5** (full budget) | tools: classroom_dashboard ×2 (`3e40f28`, `dd2aac6`), hebrew_blend_generator (`04892f3`), Hebrew_Font_Maker (measured, no code) | patterns fixed: selector-declared-twice-in-one-stylesheet ×1 (3 …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S395 (2026-09-17):** classroom_dashboard **2026-09-17 (`3e40f28`, `dd2aac6`; H S395, K S388, C S354, **D S325**)**; hebrew_blend_generator **2026-09-17 (`04892f3`; G S392, A S385, **D S338 — D-next**)**; Hebrew_Font_Maker 2026-09-17 (measured; H S380, N S395-partial); torah_trainer 2026-09-16 (`767064e`; I S394, N S389, M S345); js/ivrit-account 2026-09-16 (`a3d288d`; P S390, N S389); flash_cards 2026-09-16 (`d285c88`; D S393, C S381, **H S264 — H-next**, **G S279 — G-next**); index 2026-09-16 (`fa001b0`; M S391, H S323); …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S396):** **BRANCH/PR: S395 CONTINUED `claude/loving-einstein-cdmbbz` — PR #246 was open, unmerged and draft at session start (read through the API, not assumed), so the skill's continue rule applied. 4 commits on top of S391–S394's 17. If #246 is STILL open and unmerged, continue on it; if merged, cut fresh off latest `origin/main` — never stack on merged history.** **/!\ DRIFT CHECK: NONE, a FOURTH consecutive session — `origin/main` still `b304db0`, clone NOT shallow, migrations 0001–0003 recorded live, advisors one inert lint, `delete-account` ACTIVE with `verify_jwt` on, keep-alive's last SCHEDULED run green. CHECK, never assume either way (S391's clone arrived SHALLOW and read 65 commits stale).** After S395: `sw.js` **v770**, FM **5.45**, SDK pin 2.116.0. Never trust these numbers unread.

**⚑ HARNESS. The `--sdk` fixture: `npm pack @supabase/supabase-js@2.116.0`, untar, pass `package/dist/umd/supabase.js` (**npm IS reachable; the CDNs are not** — the same trick serves html2canvas 1.4.1 + jspdf 2.5.1 for the generator's PDF export). smoke-account / smoke-saves / smoke-tools hardcode port 8080 — **kill your own server FIRST, by PID. Never `pkill -f` a server: it matches and kills this shell.** A backend-file change is verified by the matching smoke, never by the generic recipe. **Always `finally { ctx.close(); browser.close(); }` — a script that leaves the browser open hangs forever behind a `| tail` (trap 54).**

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), C (S381), F (S383), B (S384), A (S385), E (S386), L (S387), K (S388), N (S389), P (S390), M (S391), G (S392), D (S393), I (S394), H (S395).** Unattended: take **C** (accessibility) — 14 sessions stale and now the longest unattended gap — **on a tool C has never had: `account.html` (never C, never M, never H) or `trope_tutor.html`; the dashboard's last C was S354 and flash cards' S381.** Then F, B, **A (its 2nd sweep of `selector-declared-twice-in-one-stylesheet` — 13 rows censused and classified, so run `i394/dupcss2.py`, do not re-derive; only IDENTICAL/SHADOWED-by-accident are deletable)**, E, L, K, N, P, M (next: a chrome page never M-audited — `account.html`), G (next: `flash_cards.html`, S279), D (next: the **generator**, S338) and H (next: **`flash_cards.html`, S264, the stalest H tool** — then `account.html`, which has never had one).

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1–34 are S384–S392's, 35–40 S393's, 41–47 S394's, 48–54 S395's — all of them in full in loop-findings, read them there first.** The S395 seven in one line each: **(48)** `#blackout` is `position:fixed; inset:0` ALWAYS — only `.open`/`opacity` change, so a size-or-visibility helper calls it "shown" at rest and made the `B` shortcut look broken; it is not. **(49)** headless Chromium does not leave native fullscreen on Escape (`document.fullscreenElement` stays set, `fullscreenchange` never fires) — use `document.exitFullscreen()`. **(50)** the dashboard roster commits on **blur**, never `change`, and a programmatic `el.click()` elsewhere does not blur it — drive a real `page.click`. **(51)** `.swm-cell[data-day="N"]` indexes `WEEK_KEYS` (`sun`=0) and only 1–5 render with the weekend hidden, so `data-day="0"` times out on a missing element, not a covered one. **(52)** the picker's reveal runs ~1s — reading `#pickerProgress` straight after a click returns the PREVIOUS count and `#pickControls` measures 0×0. **(53)** a helper returning `textContent` reads `''` for every input/textarea; use `.value`. **(54)** always close the browser in a `finally`, and kill a stuck script **by PID**.

**⚑ TOP UNGATED CANDIDATES (S395): the 13 open carriers of `selector-declared-twice-in-one-stylesheet`** — but every one still open is either DIVERGENT (healthy layering: FM `#fsPanelsBtn`, dashboard `textarea` + `.fr-tour`, generator `.rw-flip-back` + `.main-content`, dictionary `.wc-speak` + the reduced-motion `*`, privacy + terms `main li`) or a documented judgement call (the generator's bingo print-literal layer, `body.dark .answer-key-banner` in `@media print`), **so the next A sweep is a census and an intent decision, not a fix list.** Then **the tour-card h4 → h2 ×7** (breaks the 2-tool cap — split it or take it as an A sweep; CLAUDE.md UX rule 1 also wants the tour engine extracted into a shared block the next time one is touched); **the sub-floor touch targets** (generator `.bingo-step`, FM `#rulerCorner` — both gate 3, a visible size change); **"sign out everywhere"** (new control + copy, so gate 1/2); **the Edge Function's production `localhost` origins** (backend boundary — maintainer redeploy). **`hover-feedback-survives-the-disabled-state` still has NO open carrier** (closed S394).

**⚑ GATE-2 BENCH (unchanged): the generator's Header Labels translates only Name/Date** (~10 render sites; authoring, not wiring). Then: the flash-card swatch labels; the generator's `restoreLastSetup` corrupt-drop; the dictionary drag-freeze + select-all cap; the corpus's split article tokens; the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the resources Taamim fallback; the FM's `nexthint_nikkud` copy; **the 5 unreferenced credit keys (S360)**. Gate 3/M: the FM stage annotations; `account.html`'s dark `.btn.ghost`/`.btn.danger`; **`.dash-edit-pencil`'s side in Hebrew**; **the hub's seven-size 2.4px type band**. Gate 4: the flash-card sticky-bar and landscape proposals. **Seed bench: 26.**

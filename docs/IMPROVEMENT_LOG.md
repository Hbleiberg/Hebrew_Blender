...
- [ ] P4 (**NEW S388 Pass K arm (b) — not runtime-confirmed; same class as `b4afd93`**) | hebrew_blend_generator.html + Hebrew_Font_Maker.html | **Three more classes have a `:hover` rule and NO disabled rule of any form while taking a locked state: generator `.btn-secondary` (`#printBtn`) and `.btn-xs` (`#savePresetBtn`), FM `.help-tab`.** The generator was at its 2-tool cap this session. The …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S389 (2026-09-16):** js/ivrit-account **2026-09-16 (`f5ccea8`; N-measured S389 on all 9 carriers, K S388)**; js/ivrit-saves **2026-09-16 (`ba34a28`; K S388)**; trope_tutor **2026-09-16 (N S389, its 2nd since S248; L S387, M S376)**; flash_cards 2026-09-16 (`b4afd93`; C S381, D-next); hebrew_blend_generator 2026-09-16 (`60cd7ae`; A S385, P S382); hebrew_dictionary 2026-09-15 (`831b414`; G S377 + D S378); classroom_dashboard 2026-09-15 (`68552b3`; A S385 + B S384, **H stalest since S46**); account 2026-09-15 (`fba22e1`; L S387, E S386, P …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`hover-feedback-survives-the-disabled-state`** (registered 2026-09-15, S383 Pass F): ACTIVE. **S388 (Pass K session, ×1): the NINTH carrier FIXED (`b4afd93`, flash cards) and the FIRST ever found by ARM (b)** — two controls, two idioms, two different failures: …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]

- **`physical-property-that-never-mirrors`** (**NEW, registered 2026-09-16 (S388 Pass K) — the shape has now surfaced in THREE sessions (S248, S376, S388), which is what makes it recurring rather than incidental**): ACTIVE. A physical horizontal CSS property …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S390):** **BRANCH/PR: S382-S389 all ran on `claude/wizardly-tesla-xu0ikd` (PR #242, draft, open, unmerged; base `4d6c02d`), now 33 commits + 8 close-outs. If that PR is still open and unmerged, CONTINUE on it; if merged, cut a fresh `claude/*` off latest `origin/main` and open a new draft PR at close-out. Verify via the API** (`get_status` reads `pending`/`total_count: 0` — no workflow runs on `pull_request`, so that is GitHub's default, NOT a queued check. `get_check_runs` answers: 1 run, `Supabase Preview`, concluded **`skipped`** — correct, migrations live in `db/`. A GREEN PR, not a failure to chase.) **⚑ DRIFT CHECK: compare `origin/main` against `4d6c02d` + this loop's commits; S383-S389 all found NONE — CHECK IT, never trust this unread. `sw.js` v756, FM 5.44, migrations 0001-0003 all `Live? = yes`.** A fresh container starts SHALLOW — `git fetch --unshallow` before `update-sitemap.mjs` and any `git log`-dated arm (S389's clone was full at 3,075). The `--sdk` fixture the smokes need: `npm pack @supabase/supabase-js@2.116.0` (npm IS reachable; the CDNs are not), untar, pass `package/dist/umd/supabase.js` — **pass it to `smoke-tools.mjs` too or its signed-in arms silently SKIP**. `smoke-account`/`smoke-saves`/`smoke-tools` all hardcode port 8080 — one at a time, and background them. **Never `pkill -f` a server: it matches and kills this shell (it cost S389 a whole smoke round). Kill by PID.**

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), M (S376), G (S377), D (S378), I (S379), H (S380), C (S381), P (S382), F (S383), B (S384), A (S385), E (S386), L (S387), K (S388), N (S389).** Unattended: take **M** (aesthetics) — 13 sessions stale and now the longest-standing unattended gap; its surface is **`index.html`** (S246, the pointer's standing choice). M ships BEFORE and AFTER screenshots light + dark at the affected breakpoints, and anything a returning teacher would notice is gate 3, unattended-deferred. Then G (S377), D (S378 — next flash_cards or the FM), I (re-run `i379/gates.mjs`), **H next the dashboard (S46, the stalest tool pairing in the table)**, C, P (next the dashboard or dictionary), F (the disabled CURSOR idiom, or the empty-state re-sweep), B, **A (sweeps `b5e1416..HEAD` and must use S388's CORRECTED arm-(b) definition: three disabled forms, ID-keyed guards count, dimming does NOT discharge hover-cancelling)**, E, L, K (re-run `k388/{plant,keys,dyn,blind,acct,fallback,rtl,armb}.mjs`), N (next `index.html` S249 or the generator S262; **re-run `n389/menu.mjs` and reach the Font Maker's chip by dismissing its wizard**).

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1-20 are S384-S388's, in full in loop-findings (3, 4 and 7 cut here for room) — read them there first.** Cues: (1) theme from a themed token; (2) catch-all abort route FIRST; (5) census a dialog OPEN; (6) seed `hebrewDashboard_setupSeen`; (8) `ivritRestore` blocks on Merge; (9) match the attribute quote's kind; (10) LOAD sites only; (11) `url(…)` also matches JS `URL(`; (12) specificity is a measurement; (13) an inline-script `const` is SCRIPT-SCOPED; (14) a junk axis needs a truthy non-string; (15) walk `@graph`; (16) `pkill -f` kills the shell, and `pgrep -f` in an until-loop matches that shell itself, so the loop never exits — wait on the output file; (17) read BOTH quote styles and ALL files; (18) concatenated fallbacks truncate at the first literal; (19) measure the CONTROL before anything that switches views; (20) `I18n.t()`'s missing-key warning is gated on `dictLoaded`. **(21) NEW S389 — a busy loop inside `page.evaluate()` is NOT reported to a `longtask` observer (it runs as a CDP task): drive the control with a REAL tap on a button whose handler blocks, or the arm is blind. (22) NEW S389 — seed a page's first-run gates before tapping its header: `hebrewDashboard_setupSeen` hid 6 real hits behind 8 VOIDs, and the FM's ungated wizard still hides its chip. (23) NEW S389 — `documentElement.scrollWidth` is BLIND to an abspos popup leaving the viewport, in BOTH directions; only a per-element rect census sees it.**

**⚑ TOP UNGATED CANDIDATES (S389): widen the smokes' viewport check** (P3 — `smoke-account`'s one 800x700 cell on `account-test.html` passed clean through 17 real phone overflows; assert the rect under a phone descriptor in `smoke-tools`, which already loads both worst carriers); **`clampMenu()` on resize/orientationchange** (P4, the `f5ccea8` remainder); **the FM chip's unmeasured phone geometry** (P4, behind its wizard); **the three unconfirmed arm-(b) carriers** (generator `.btn-secondary`/`.btn-xs`, FM `.help-tab`, torah `.btn` — torah needs a loaded parsha, its guard is ID-keyed); **the tour-card h4 -> h2 x7** (extract the shared tour block then); **the sub-floor touch targets**. Gate 2: the flash-card swatch labels; the generator's `restoreLastSetup` corrupt-drop; the dictionary drag-freeze + select-all cap; the corpus's split article tokens; the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the resources Taamim fallback; the FM's `nexthint_nikkud` copy; **the 5 unreferenced credit keys (S360)**. Gate 3/M: the FM stage annotations; `account.html`'s dark `.btn.ghost`/`.btn.danger`; **`.dash-edit-pencil`'s side in Hebrew**. Gate 4: the flash-card sticky-bar and landscape proposals. **Seed bench: 26. ⚑ PROTOCOL QUESTION: the shared-`js/`-module cap — S388 read it as one such fix per session, S389 arithmetically (2 modules = 2 of 2 per carrier page) and shipped both, the 2nd a P2 phone sign-in failure. One reading needs to win.**

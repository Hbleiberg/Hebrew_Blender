...
- [ ] P4 (**NEW S325 Pass D — the pattern's 4th carrier, measured; the dashboard/torah/trope recipe applies**) | hebrew_blend_generator.html | **The options-sidebar drag writes `--gen-sidebar-w` on `documentElement` every frame, restyling all 8,722 nodes (6–9 ms style per frame at 1×, ~30 ms at 4×).** Its only consumer is `.sidebar` itself (`width: var(--gen-sidebar-w)`), so pass `scope: <the …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P4 (**NEW S325 Pass D — the pattern's most expensive carrier, blocked by a fixed cousin**) | hebrew_dictionary.html | **The sidebar drag writes `--dict-sidebar-w` on `documentElement` per frame: 14 ms style + 15 ms layout per frame at 1× across 10,369 nodes (the suite's heaviest drag).** `scope` cannot narrow it yet: `.sidebar-toggle-btn` (fixed, `inset-inline-start: …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P4 (**NEW S325 Pass D — same pattern, per `input` event instead of per drag frame; dashboard AT CAP S325**) | classroom_dashboard.html | **The 11 size sliders (`setEngDateSize` … `setDowEmojiSize`) write their `--*-size` vars on `documentElement`, restyling the whole page per slider step (~7 ms at 1×, ~28 ms at 4×).** Every consumer sits under `.dashboard` (124 elements), so writing them …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-03 | (S325 close-out) | branch/deploy note | **Branch/PR: CONTINUED `claude/improve-loop-k280wo` → draft PR #222** (API at start: open, draft, `mergeable_state:clean`, head `56a337e`, 0 check runs — correct, no workflows). **Drift check:** `origin/main` still `735b451`; `sw.js` v648 and `FONT_MAKER_VERSION` 5.39 re-read (both moved on this branch by the outside-loop FM v5.39 …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-03 | `8398c88` | index.html | **`.ie-modal` gains a `max-height: 90dvh` twin under its `90vh`** — the hub's carrier of the S321 centred-box-in-`vh` shape (the dashboard's `.fr-modal`/`.swm-modal` twins stay open: dashboard AT CAP). | Playwright: the rule carries `90dvh`; the modal opens with its footer inside the viewport at 1280×800 (720px), 800×600 (540px) and 390×664 (598px), …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S325 (2026-09-03):** classroom_dashboard **2026-09-03 (×2, `290267e` scoped rail/drawer widths, `af6c40c` fullscreen caption floor; S324 ×1, S323 ×2, S322 ×2, S321 ×2)**; torah_trainer + trope_tutor **2026-09-03 (×1 each, `280965f` drawer scope; torah S324 ×2)**; index **2026-09-03 (×1, `8398c88` dvh twin; S324 ×1, S323 ×2)**; hebrew_dictionary + hebrew_blend_generator 2026-09-03 (shared-block edit only, `290267e`; S322 ×2 each); Hebrew_Font_Maker 2026-09-03 (outside-loop v5.39 `4638e7b`; S321 `09f6220`); flash_cards 2026-09-03 (S321 …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`custom-property-written-on-documentElement-per-frame`** (**NEW, registered 2026-09-03 (S325 Pass D) — 6 carriers found, 3 fixed (`290267e` dashboard rails + drawer, `280965f` torah + trope drawers), 3 filed (generator sidebar, dictionary sidebar, dashboard size sliders). …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S326):** **BRANCH/PR: CONTINUE on `claude/improve-loop-k280wo` → draft PR #222 (S316–S325 commits + FM v5.39; verify via the API first — if it has merged, cut a fresh `claude/*` branch off latest `origin/main`).** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v649, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS I (2026-09-01, S308), THEN C (S312), F (S313), O (S315, attended only), B (S316).** I is suite-wide (every tool in a fresh context, empty localStorage AND IndexedDB); its S308 arms are in the archive — re-run them over the S308→S326 delta, and treat the outside-loop FM v5.39 Exported ✓ / Send flow and the dashboard's scoped resize widths as the first-load surfaces that changed. **C's next target by its own history is the generator (S224)**; **O's is flash_cards.html — attended only.** Run `update-sitemap` at EVERY close-out.

**⚑ THE STRONGEST UNTAKEN CANDIDATES:** the three new `custom-property-written-on-documentElement-per-frame` carriers (generator sidebar — a one-mount `scope` + `@property`; the dashboard's 11 size sliders — write target only, dashboard was AT CAP; the dictionary sidebar — blocked by its fixed toggle); the "Date format" wrap (P4, dashboard); the dashboard `vh` modal twins (P4, the hub's is done); the two S324 torah residuals (side-by-side builder); the S319 gate-2 Hebrew-interface claim and the S323 gate-2 manual-mode copy (attended). Seed bench: the scroll-style handout (torah, S324), the last-backup date (index, S323).

**/!\ THE S325 METHOD LESSONS.** (a) **A profiler charges the page's first forced layout to whichever function reads layout first** — `_railMaxFor` showed 100 ms self time (400 at 4×) and deferring it into rAF changed the boot longtask by nothing; measure a suspected boot cost by moving it before filing it. (b) **A custom-property write invalidates the writer's whole subtree, consumers or not** — the `--zzz-unused` control proved it; the cure is the write target (nearest common ancestor) plus `@property inherits:false` when the consumer is the writer itself, and it is worth 8–30× on a drag frame. (c) **CDP `Performance.getMetrics` deltas (RecalcStyleDuration/LayoutDuration/TaskDuration) around one interaction are the cheap, controlled way to see a per-frame cost** that longtasks miss at 1× — pair them with a rAF gap watcher and an idle control window. (d) **Serve the HEAD file through `route.fulfill` for a same-process BEFORE** (scratchpad `d325/verify1.mjs`): before/after in one script, same machine state.

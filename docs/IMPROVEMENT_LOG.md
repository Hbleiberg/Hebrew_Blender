...
- **S364 (2026-09-10):** classroom_dashboard **2026-09-09 (`eed4c03`, `0dc999c`; G-audited S364)**; hebrew_blend_generator **2026-09-10 (`cabd6ca`, `b83b896`; H-audited S363)**; hebrew_dictionary 2026-09-09 (`10be341`; M-audited S362); Hebrew_Font_Maker 2026-09-09 (`26fd826`); index 2026-09-09 (S360); contact / privacy / terms / 404 2026-09-09 (S360); flash_cards 2026-09-09 (S360; N-audited S361); resources 2026-09-09 (S356); torah_trainer 2026-09-09 (S355); trope_tutor 2026-09-09 (S353).
...
- **`sub-floor touch target`**: ACTIVE. **S364 Pass G measured a new carrier: the generator's bingo ▲/▼ stepper 18.7×10.5 / 9.5px (a visible size change — logged as an M-call candidate). S362: `#presetCopyBtn` FIXED (`a0cc446`, a `::after` hit box). S359: flash cards' card-count …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **authored-but-unreferenced i18n key family** (a translated CSV key referenced nowhere): **S363 Pass H: 1 hit FIXED (`128c449`, `worksheet.preview.settings_changed` — both call sites passed the raw English; found by walking the live-preview-off path in HE). S361: 2 …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S365):** **BRANCH/PR: S364 CONTINUED `claude/inspiring-meitner-rhqskk` → draft PR #227 (base `origin/main` `f2384a7`; S352–S364 = 59 commits). Continue it if open; if merged, cut a fresh `claude/*` off latest `origin/main`. Verify via the API** (0 check runs is correct — no workflows). **Drift note: `sw.js` v689, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), D (S352), I (S353), C (S354), F (S355), B (S356), A (S357), E (S358), L (S359), K (S360), N (S361), M (S362), H (S363), G (S364).** Unattended: take D (performance snapshot) on `trope_tutor.html` — the least-recently-D'd tool per the S352 row's ordering (trope S232 < dictionary S253 < flash S266 < FM S294 < torah S307 < dashboard S325 < generator S338 < index S352): cold load + the heaviest interaction (the whole-chart print build, a drill round, the index/motif loads) at 1× and 4× CPU, main-thread blocks >200 ms with profile evidence, every zero backed by an injected control. **O next `torah_trainer.html`; G next dictionary (S252); H next trope_tutor (S228); C the Font Maker (S257); N next resources (S246); M next trope_tutor (S245).**

**⚑ Harness (standing): torah stubs Sefaria by fulfilling `**/*sefaria.org/**` (`b356/harness.mjs` carries the stubs); hidden toggle inputs → click `label.toggle:has(#id)`; the HTTP server dies across a compaction — curl it first; pypdf needs the stub `cryptography` package (`d352/stub`) and prints `<input type=time>` as 12-hour text; `page.pdf()` fires `afterprint` — re-wear a print-state class before EACH pdf; PyMuPDF is absent — `g364/paper.mjs` CENSUS (fixed elements + dark slabs under print media) is the slab detector; a hover read needs a REAL `page.mouse.move` + 400 ms, a focus read a real Tab, never `page.focus()`; open a probe's host by ITS path (dashboard schedules `openWeekEditor()`; the backup box ⚙ → Presets → `#ivritModeManual`; `#dashEditor` is INSIDE the settings modal; the generator's live-preview toggle is in the collapsed Advanced panel and live sync speaks only after a first Generate, 500 ms debounced; the FM Custom glyphs tab: Advanced → `label.chk:has(#addCustomGlyphsChk)`); an `elementFromPoint` receipt precedes a real click; modal probes read the LAST `[aria-modal]`; `ivritAskMode()` opens by calling it inside `page.evaluate`, never awaited; the harness auto-accepts dialogs — a confirm probe installs a handler that dismisses; K's strongest arm is `k360/hecensus.mjs`; probe screenshot paths are cwd-relative — run probes from the scratchpad; the shared language switcher stretches to its flex LINE (measure `[data-i18n-switcher] > *`).**

**⚑ TOP UNGATED CANDIDATES (S364):** the tour-card h4 → h2 ×7 (CLAUDE.md: the next touch of a tour engine extracts it into a shared block — not small); the sub-floor M calls (generator `.toggle` switches 40×22, the bingo ▲/▼ stepper 18.7×10.5, FM `#rulerCorner` 22×22, dashboard swatches). Gate 2: the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on (`g364/out/board-print-en-dark.png`). Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. The next A sweeps the S357–S364 delta with the NEW `fixed-surface-missing-from-the-print-hide-list` arm on all 7 tools. **Seed bench:** unchanged (4 seeds: resources font-submit form, hub saved-work badges, dashboard per-day overrides, dashboard week cycles).

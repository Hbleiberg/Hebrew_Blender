...
- [ ] P4 (**NEW S371 Pass A — pattern `control-class-without-a-hover-state`, real-mouse 0/4 cells**) | classroom_dashboard.html | **The week editor's colour swatches (`.swm-swatch` with the transparent `.swm-swatch-input` as hit box) give no hover feedback on either element.** A `.swm-swatch:has(.swm-swatch-input:hover) { box-shadow … }` (or `:hover` on the input's parent) — one rule, both …[full …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P4 (**NEW S371 Pass A — pattern `sub-floor touch target`**) | Hebrew_Font_Maker.html | **The Guides panel's `.guide-rename` ("Rename", 0.7rem, padding 1px 3px) measures 52×15.** Fix vehicle: the dashboard `.preset-copy-btn::after` inset idiom (`position:relative` + `::after { inset:-5px 0 }`) → 25px hit box, no visible change to the 23px row. | found S371

- [ ] P4 (**NEW S371 Pass A — pattern `fixed-surface-missing-from-the-print-hide-list`; a twin divergence, not a reachable print path**) | trope_tutor.html | **`#settingsBackdrop` (1280×900) and `#settingsModal` (380×900) print over the page when the settings drawer is open; the Torah Trainer hides its identical pair.** The page's own Print-chart button sits behind the backdrop, so only Ctrl+P …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P4 (**NEW S371 Pass A — pattern `non-finite-number-from-a-loaded-file`; hand-edit only (`1e999` in the settings key), strings already fall back**) | torah_trainer.html + trope_tutor.html | **`parseFloat(x) || default` passes `Infinity`: torah `karaokeRate`/`ttsRate` labels read "Infinity×", trope `hebFontSize` sets `--tu-heb-size: Infinityrem` and `playbackRate` reads "Infinity×".** …[full …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P4 (**NEW S370 — a paper-cut seen in the Beta-badge before/after pair; not a regression**) | flash_cards.html | **The Mode panel's "🎧 Listening" label runs straight into its Beta badge with no space (`.label.has-tip` for `listeningMode`), while every other carrier of the badge has one.** One space (or a `margin-inline-start`) before the badge. | found S370

- [ ] P4 (**NEW S369 Pass F — the remaining sub-11px FUNCTIONAL labels outside the Font Maker, one batch per page; the census is in loop-findings**) | resources.html + hebrew_dictionary.html + hebrew_blend_generator.html + flash_cards.html + privacy.html | **resources `.card-cat-badge` 9.6px ×43 + `.filter-label` 10.88 ×2; dictionary `.theme-count` 9.6 ×14 + `.pos-arrow` 10.4 ×7; generator …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-11 | (S372 close-out) | branch/deploy note | **S372 CONTINUED `claude/improve-loop-swgwqx` (PR #229 open, unmerged; base `66dd97b` = `origin/main`), 4 commits + this close-out; a human merges and confirms the Pages run. Drift: none (`66dd97b`, sw v696, FM 5.39 — as the S371 close-out recorded; clone unshallowed for the sitemap). sw v696→v697 (Font Maker); FM NOT bumped (a load-path …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-11 | `c2b3b10` | docs/reference/shared-components.md + torah-and-trope.md | **The two session ids that leaked into reference docs are gone (a `(converged SNNN)` tag; a `measured SNNN` clause now pointing at loop-findings)** | S372 Pass E; `grep -E '\bS[0-9]{3}\b' docs/reference/*.md` (minus loop-findings) 2 → 0; the date/digest/line-number grep was already 0.

- [x] 2026-09-11 | `21ea7dc` | docs/reference/storage-and-backup.md + generator.md + font-maker.md | **The S370–S371 guards reach the docs: `_sizeNum` + `setHebFontSize` clamps in the restore-guard paragraph and the size-key row, a `#headerSub` two-slot section, `applyProjectData`/`migrateProject` refusal + reduction, `getRecentProjects` filter** | S372 Pass E doc-accuracy arm; every sentence …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-11 | `dcfb085` | Hebrew_Font_Maker.html | **`getRecentProjects()` keeps only `{name: string, data: object}` members of an array — a stored `"abc"`, `null` or `[null,…]` no longer throws in `toggleLoadMenu` and kills the Load menu** (pattern `stored-json-of-the-wrong-shape-trusted`, the S371 P3) | `a371/fmrecents.mjs`: 6 shapes, 3 threw before → 0 after, menu opens 6/6; …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-11 | `919f478` | sitemap.xml | **`lastmod` refreshed for the five pages the S369–S371 commits moved (index, dictionary, Torah Trainer, Font Maker, resources: 09-09 → 09-10)** | S372 Pass E: `update-sitemap.mjs` after `git fetch --unshallow`; a second run is a no-op; `update-llms-txt --check` current before and after.

- [x] 2026-09-10 | (S371 close-out) | branch/deploy note | **S371 CONTINUED `claude/improve-loop-swgwqx` (PR #229 open, unmerged; base `66dd97b` = `origin/main`), 4 commits + this close-out; a human merges and confirms the Pages run. Drift: none (`66dd97b`, sw v695, FM 5.39 — all as the S370 close-out recorded). sw v695→v696 (dashboard, dictionary, generator + the locale JSONs); FM NOT bumped …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-10 | `e5dac20` | hebrew_dictionary.html | **Pass A hit (`apply-settings-trusts-collection-members`, the range-slider SETTER shape, 2nd carrier): `setHebFontSize` ran `Number(val)` on the persisted `hebrewBlender_hebFontSize` (shared with the generator) → `--heb-font-size: NaNrem` + the junk re-persisted; the TTS-rate restore formatted the raw string ("NaN×"). The generator's S370 …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-10 | (S370 close-out) | branch/deploy note | **S370 CONTINUED `claude/improve-loop-swgwqx` (PR #229 open, unmerged; base `66dd97b` = `origin/main`), 5 commits + this close-out; a human merges and confirms the Pages run. Drift: none (`66dd97b`, sw v694, FM 5.39 — all as the S369 close-out recorded). sw v694→v695 (6 precached pages); FM NOT bumped (a load-path guard, no feature). …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-10 | `966179f` | flash_cards.html | **`.letter-tile small` 0.58rem → 0.62rem with 1px tile sides — the generator's `41679da` step (S370, P4, the S369 F candidate, text-size floor)** | `b370/fix4-probe.mjs` 12 cells (1280/800/390 × light/dark × EN/HE): names 9.92px, longest "Chaf Sofit" 52px / "שין שמאלית" 49.8px inside the 57px phone content box, 0 wraps, 0 overflow, 0 pageerrors.

- [x] 2026-09-10 | `edf4b2e` | index + classroom_dashboard + hebrew_blend_generator + flash_cards + hebrew_dictionary | **The Beta badge converges on one `.beta-tag` idiom at 0.7rem: 11 inline spans → the class, the dashboard's carrier-less rule rewritten to the idiom, the hub's heading badge lifted (S370, P4, the S369 F candidate, text-size floor)** | `b370/fix3-probe.mjs` 40 cells (5 pages × …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-10 | `70f62c0` | hebrew_blend_generator.html | **`setHebFontSize(val)` clamps to a finite 0–100 (junk → 50): a non-number from a `?s=` link, a preset or the persisted key set `--heb-font-size` to `NaNrem` for the session and persisted the literal (S370, P3, Pass B arm 5, `apply-settings-trusts-collection-members`, the range-slider SETTER shape)** | `b370/fix2-probe.mjs` 32 cells (8 …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-10 | `7e5905f` | Hebrew_Font_Maker.html | **`.ctl-btn` gets `min-height:24px`: the Spacing tab's six tricky-combo chips (`padding:2px 8px`, 0.68rem) 41×22 / 37×22 → 41×24 / 37×24; every other `.ctl-btn` stays 29px (border-box) (S369, P4, `sub-floor touch target`, the S368 candidate)** | `f369/fix3b.mjs` 4 cells through the wizard + `setCatTab('spacing')`: BEFORE (stash) 6 …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S372 (2026-09-11):** Hebrew_Font_Maker **2026-09-11 (`dcfb085`; C-audited S368)**; hebrew_blend_generator 2026-09-10 (`e4c8d75` S371; H-audited S363); trope_tutor 2026-09-10 (`869ee1c` S369; A-probed S371; H-audited S367); classroom_dashboard 2026-09-10 (`1568296` S371; G-audited S364); hebrew_dictionary 2026-09-10 (`e5dac20` S371; M-audited S362); index 2026-09-10 (`edf4b2e` S370); flash_cards 2026-09-10 (`966179f` S370; N-audited S361); torah_trainer 2026-09-10 (`869ee1c` S369); resources 2026-09-10 (`ce8c9d5` S369); contact / privacy / …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`stored-json-of-the-wrong-shape-trusted`**: ACTIVE. **S372: the S371 P3 FIXED (`dcfb085`, FM `getRecentProjects` filters to `{name, data}` objects). Registered S366 (Pass I); hits 6 (S366 ×2 fixed, S370 ×1 FM project file, S371 ×1 recents), clean streak 0; last swept S371.** …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`apply-settings-trusts-collection-members`**: ACTIVE **S371 (Pass A, `junk.mjs` 176 cells): the RANGE-SLIDER SETTER shape had 13 more sites — the dashboard's 12 `set*Size` (`1568296`, one `_sizeNum` clamp) and the dictionary's `setHebFontSize` + rate label (`e5dac20`), all …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`fixed-surface-missing-from-the-print-hide-list`**: ACTIVE **S371 (Pass A, `print.mjs`, 15 runtime cells, control fires): closed state clean on all 9 print-flow pages + 5 open-surface cells; 1 twin divergence logged P4 (trope `#settingsBackdrop`/`#settingsModal` print when …[full text: IMPROVEMENT_ARCHIVE.md]

- **`control-class-without-a-hover-state`**: ACTIVE. **S371 (Pass A, real mouse light+dark): the S357 P4 (`ivritAskMode` Merge/Replace/Cancel) is CLOSED — 8/8 cells with feedback on dashboard, generator, flash, hub (the S360-era `.ivrit-ask-btn` rules); dictionary …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`sub-floor touch target`**: ACTIVE. **S371 (Pass A, pseudo-aware): FM `.guide-rename` 52×15 NEW carrier logged P4; `.ctl-btn` 24×24, `.ivrit-ask-btn` 103×35, dictionary header buttons 30px, dashboard swatch 24×24 (hit box), `#presetCopyBtn` 36×17→36×27 via `::after`. Hits 1 …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`non-finite-number-from-a-loaded-file`**: ACTIVE **S371 (Pass A, `1e999` per key): 14 sites reached Infinity — dashboard 12 sizes + dictionary size FIXED inside the setter clamps (`1568296`, `e5dac20`); torah `karaokeRate`/`ttsRate` and trope `hebFontSize`/`playbackRate` …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S373):** **BRANCH/PR: S369–S372 ran on `claude/improve-loop-swgwqx` (cut at `66dd97b` = `origin/main`; 20 commits) → draft PR #229. If it is still OPEN and unmerged, continue on it; if MERGED, cut a fresh `claude/*` off latest `origin/main` — never stack on merged history. Verify via the API** (0 check runs is correct — no workflows). **⚑ DRIFT CHECK: compare `origin/main` against `66dd97b`; anything beyond THIS loop's 20 commits is an outside-loop landing. `sw.js` v697, `FONT_MAKER_VERSION` 5.39** — re-read both. The clone was unshallowed in S372 (a fresh container starts shallow again — `git fetch --unshallow` before `update-sitemap.mjs`).

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), L (S359), K (S360), N (S361), M (S362), G (S364), D (S365), I (S366), H (S367), C (S368), F (S369), B (S370), A (S371), E (S372).** Unattended: take L (SEO & discoverability) — byte-check the crawler surfaces (title, description, canonical, OG/Twitter, JSON-LD, h1, FAQ text) of all 14 pages against `b418cf9` (S358→S372; the generator's subtitle keys and the Beta-badge markup moved, nothing else should have), sitemap ↔ canonical ↔ indexable set (sitemap regenerated in S372 — 12 urls), `og:image` resolving, one h1 per page, alt text; copy fixes are gate 2 — log and batch. Then K over `b418cf9..HEAD` (4 subtitle keys + the S370 FM status key added; Check B backlog burndown). **O next `torah_trainer.html`; D next dictionary (S253); G next dictionary (S252); H next Font Maker (S241); N next resources (S246); M next trope_tutor (S245); C next flash_cards (S271); F next the disabled-control look; the next A sweeps `1d4c9cf..HEAD` with the range-setter arm on the Font Maker's 13 sliders + torah/trope; the next I re-runs `i366/{gates,arms,corrupt,strshape}.mjs`; the next E re-runs the 17 arms (`e372/` receipts) over `408dbe4..HEAD`.**

**⚑ Harness (standing): FONT MAKER probes pass `dismiss:false` to `b370/harness.mjs` `open()` — its `.overlay.open` sweep closes the wizard and `#wizName`/`#wizOpenProject` then time out; enter through the wizard (`#wizName`/`#wizAuthor` → `#wizNext` ×2 → `wizardCreate()`), Load menu = `#wizOpenProject` / the toolbar `toggleLoadMenu(event)` button, a recents entry is seeded with `pushRecentProject(project)`; `#exportFormat` offers ttf/woff2/ttfwoff2/ufo, a project file = `JSON.stringify(project)` + `format:'hebrew-font-maker-project'` through `page.setInputFiles('#projFile', {buffer})`; torah stubs Sefaria by fulfilling `**/*sefaria.org/**` (`harness.mjs` `SEF`); hidden toggle inputs → click `label.toggle:has(#id)`; the HTTP server dies across a compaction — curl it first; hover = a REAL `page.mouse.move` + 400 ms, focus = a real Tab; dashboard: `frSkip()` BEFORE any header click, `openWeekEditor()`, `openSettings()`; the generator's sidebar is a drawer at 390; the dictionary's collapsed bodies need a REAL click on Expand-all-menus; flash: the practice screen is `#screenCard.active`; probe screenshot paths are cwd-relative; a junk-store cell = `context.addInitScript` writing the key, then a NaN/Infinity/undefined/[object Object] scan of `*Val` labels, inline styles and root custom props; the shared-block sha arm pairs `end X` with the nearest preceding start marker containing X's core token (nearest-marker pairing mis-pairs app-toast and idle-hide).**

**⚑ TOP UNGATED CANDIDATES (S372):** the dashboard swatch hover (1 rule); FM `.guide-rename` hit box (the `::after` idiom); the trope settings pair in the print block (1 selector pair); torah/trope `Number.isFinite` at 2 restore sites; the flash "Listening" badge space (1 line); the per-page sub-11px batches (resources ×45, dictionary ×21, generator, flash, privacy — `f369/census-en.txt`); the ~30 inline Font Maker hints under 11px; the tour-card h4 → h2 ×7; the remaining sub-floor M calls. Gate 2: the generator's two-subtitle slot (now documented in generator.md); the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on; the trope FAQ a7 copy. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. **Seed bench: 25 (unchanged; E is not an intake). Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

...
- **S402 (2026-09-18):** all-page passes keep the *dedicated pass* the useful column. index (M S391, F S397); hebrew_blend_generator (**S402: 3 seeds shipped** `88b2720` `7345060` `58fc2cd`; G S392, **D S338 — D-next**); flash_cards (D S393, **H S264 — the stalest H tool**, G S279); hebrew_dictionary (`273f449`; **C S285 — C-next**); classroom_dashboard (**S401: 2 seeds shipped** `92217cf` `7d7b89a`; H S395); torah_trainer (**O-next — the ledgered O target, never O-audited**; N S248); trope_tutor (**S400: 3 seeds shipped** `4ce56bb` `3249dba` …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S403):** **BRANCH/PR: S402 shipped all three generator seeds (`88b2720` `7345060` `58fc2cd`) + its close-out on `claude/improveloop-feature-seeds-0tx529` / draft PR #252, cut FRESH off `origin/main` `e5373e4` (S401's #251 was already fast-forwarded onto main). The branch was then FAST-FORWARDED ONTO MAIN at the maintainer's in-session word (so #252 reads merged, and the Pages run for that push is the deploy — confirm it concluded `success`); cut a FRESH branch off latest `origin/main` and never stack on merged history.** **DRIFT vs S401: NONE** — `origin/main` stayed `e5373e4` for the whole session. S402 closed at `sw.js` **v779**, FM **5.46** (untouched), SDK **2.116.0** (untouched); the backend numbers (migrations 0001–0003 `Live?` yes, `delete-account` untouched since `f141b7d`, keep-alive cron `41 4 * * *`) are STILL S399's readings, NOT re-read in S400–S402. **Never trust these numbers unread.** S402 ran no pass, so the stalest-pass order below is unchanged.

**⚑ WALK A SEED'S CALLERS BEFORE BUILDING IT — two S367 seed premises were wrong in S402.** "Bingo cards and caller sheets carry Name/Date": none of `wsMetaHTML`'s 12 callers is a bingo or caller builder. "`onGenerateSettled` is the hook for a page count": Answer Key on/off re-renders through `rerenderCurrentSheet` without `enableButtons`, and the chunked Class Set pump appends sheets after it ran — one `MutationObserver` on `#worksheet` was the hook. Every remaining S367 seed line says "grep-grounded, not walked"; read the seed's named functions and their callers first, and log a premise correction in the Done line when one is wrong.

...
- [ ] P4 (**NEW S396 Pass C — MEASURED suite-wide, not an impression; the convergence target already exists, so this is F's shape**) | 15 root pages | **Only `torah_trainer.html` and `trope_tutor.html` offer a keyboard bypass of the repeated header** (first focusable, target resolves, Enter-then-Tab lands outside the header — both verified). The dashboard's apparent one is the first-run card's …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`physical-property-that-never-mirrors`** (**NEW, registered 2026-09-16 (S388 Pass K) — the shape has now surfaced in THREE sessions (S248, S376, S388), which is what makes it recurring rather than incidental**): ACTIVE. A physical horizontal CSS property …[full text: …[full …[full text: IMPROVEMENT_ARCHIVE.md]

...
- [ ] P3 (**S383 Pass F, 7 carriers — 6 SHIPPED (`a585608` FM, `df9ff3b` contact + resources, `fba22e1` account’s three); ONE left, and the analysis is below so nobody re-derives it**) | trope_tutor.html | **`.btn btn-secondary` keys off `[aria-disabled="true"]`, not `:disabled`, and carries a VARIANT class with its own rest colour and its own `:hover` pair at (0,2,0), so a base-level guard …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S386 (2026-09-15):** account **2026-09-15 (`fba22e1`; E S386, P-audited S382)**; repo metadata — README, THIRD_PARTY_LICENSES, sitemap **2026-09-15 (`e65f7e5`, `ad6bcd8`, `e89e6dd`; E S386)**; hebrew_blend_generator 2026-09-15 (`0a46f67`; A S385, P-audited S382); trope_tutor 2026-09-15 (`7be7591`; A S385, M-audited S376); classroom_dashboard 2026-09-15 (`902d3ff`, `9614a80`; A S385 + B S384, **H stalest since S46**); Hebrew_Font_Maker 2026-09-15 (`a585608`; B S384, H-audited S380); contact 2026-09-15 (`df9ff3b`; B S384); resources …[full …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`hover-feedback-survives-the-disabled-state`** (registered 2026-09-15, S383 Pass F — 7 carriers found, 6 now fixed): ACTIVE. **S386 (Pass E, opportunistic — the top ungated candidate, not an E find): carrier 4 FIXED, and it was three controls not one — `account.html`’s …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S387):** **BRANCH/PR: S382–S386 all ran on `claude/wizardly-tesla-xu0ikd` (PR #242, draft, open, unmerged; base `4d6c02d`), now 19 commits + 5 close-outs. If that PR is still open and unmerged, CONTINUE on it; if merged, cut a fresh `claude/*` off latest `origin/main` and open a new draft PR at close-out. Verify via the API** (no workflow triggers on `pull_request`, so `get_status` reads `pending` with `total_count: 0` — GitHub’s default for a commit with no legacy statuses, NOT a queued check. `get_check_runs` is the one that answers: exactly 1 run, the Supabase GitHub App’s **`Supabase Preview`**, concluded **`skipped`** — correct, because migrations live in `db/` and not `supabase/` precisely so the integration never spins up a paid preview branch. A `skipped` Supabase Preview is a green PR, not a failure to chase.) **⚑ DRIFT CHECK: compare `origin/main` against `4d6c02d` + this loop’s commits; S383–S386 all found NONE, S382 the largest yet — CHECK IT, never trust this unread. `sw.js` v753, FM 5.44, migrations 0001–0003 all `Live? = yes`.** A fresh container starts SHALLOW — `git fetch --unshallow` before `update-sitemap.mjs`. The `--sdk` fixture the required smokes need: `npm pack @supabase/supabase-js@2.116.0` (the npm registry IS reachable; the CDNs are not), untar, pass `package/dist/umd/supabase.js`; **pass `--sdk` to `smoke-tools.mjs` too or its signed-in B arms silently SKIP** (114 checks without it, 135 with).

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), L (S373), K (S374), N (S375), M (S376), G (S377), D (S378), I (S379), H (S380), C (S381), P (S382), F (S383), B (S384), A (S385), E (S386).** Unattended: take **L** (SEO & discoverability) — 13 sessions stale, and like E before it **L has never seen the accounts landing**: `account.html` is IN the indexable set while `account-test.html` and `saves-test.html` are `noindex` harnesses staying out of the sitemap, `llms.txt` and the precache (S386 proved all three exclusions hold mechanically; L owns whether the *semantics* are right — `account.html`’s title/description/canonical/OG/JSON-LD and the crawl graph now that a 13th page exists). Audit the static source offline vs `4a1f186`. Then K (over `4a1f186..HEAD`). O next `torah_trainer.html`; C next torah (S298); M next `index.html` (S246); N next trope (S248); D next flash_cards or the FM; H next the dashboard (**S46, the stalest tool pairing in the table**); P next the dashboard or dictionary; F next the disabled CURSOR idiom or the empty-state re-sweep; the next I re-runs `i379/gates.mjs`; **the next A sweeps `b8ba5ef..HEAD`, re-reads the 3 rows S385 left at clean streak 1 and `hover-feedback-survives-the-disabled-state`, now 6 of 7 closed.** The next E extends `e386/`, not `e372/`.

**⚑ RUNTIME-PROBE HARNESS (standing; S384–S386 traps, all in loop-findings).** (1) `hebrewBlender_darkMode` stores `‘1’`/`‘0’`, NOT `‘true’` — prove each cell’s theme by reading a theme-valued token back (`--shadow-sm`, `--warm-gray`), else VOID it. (2) Playwright matches routes in REVERSE order — catch-all abort FIRST. (3) `requestfailed` never fires for a 404. (4) Never hand-roll an accessible-name walker — `page.accessibility.snapshot({root})`. (5) Census a dialog OPEN. (6) Seed `hebrewDashboard_setupSeen`; `#frBackdrop` is not an `.overlay`. (7) A planted control that goes UNDETECTED can mean the page is immune — read plant + result together. (8) `ivritRestore` BLOCKS on the Merge/Replace modal — fire it unawaited, then click `.ivrit-ask-btn[data-act="merge"]`; fresh context per wrong-shape import. **(9) NEW S386 — parsing a CSP meta with `content=["’]([^"’]*)["’]` truncates at the `‘self’` in `default-src` and every origin test reads false: 11 of 17 pages reported as missing both account origins, a phantom P1. Use `content="([^"]*)"` and assert `/default-src/` matched or VOID. (10) Census LOAD sites only — every `https://` string counts credit `<a href>`s no CSP governs. (11) An unanchored CSS `url(…)` also matches JS `URL(` / `createObjectURL(blob)`. (12) A guard’s SPECIFICITY is a measurement: a light variant restate is (0,4,0) and outranks `body.dark …` (0,3,1) — measure the dark cell too.**

**⚑ TOP UNGATED CANDIDATES (S386): the LAST hover-vs-disabled carrier (trope `.btn btn-secondary` — `fba22e1` is the worked example to copy, per-variant restate × theme; do not re-derive the analysis); the wrong-shape `.ivrit` axis on the five liveState blobs B’s arm 4 never drove (`b384/arm4b.mjs` takes a page + key); the dictionary’s inline `style.opacity` disabled hack; the tour-card h4 → h2 ×7 (the "next touch of a tour engine" — extract the shared block in the same session); the remaining sub-floor M calls (bingo stepper, scaled toggles, `#rulerCorner`). Decide-then-sweep: whether an action TOOLBAR should rove like a selection grid; whether the disabled CURSOR should converge. Gate 2: the flash-card swatch labels; the generator’s `restoreLastSetup` corrupt-drop; the dictionary drag-freeze + select-all cap; the corpus’s split article tokens; the printed teacher-copy wording; the board’s day plates; the trope FAQ a7 copy; the resources preview Taamim fallback. Gate 3/M: the Font Maker stage annotations; **NEW S386: the dark theme has no `.btn.ghost` variant at all on `account.html` and its dark `.btn.danger` has no hover response — both pre-existing, both diagnosed in loop-findings, both M’s call not a disabled-state fix.** Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals. **Seed bench: 26. Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

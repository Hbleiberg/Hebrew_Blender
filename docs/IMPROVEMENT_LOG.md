...
- **S393 (2026-09-16):** hebrew_blend_generator **2026-09-16 (`12c170e`, `ec8bace`; G S392, A S385, P S382, **D S338 — D-next**)**; flash_cards **2026-09-16 (`d285c88`; D S393, C S381)**; index 2026-09-16 (`fa001b0`; M S391, D S352, C S340); Hebrew_Font_Maker 2026-09-16 (`d5ee54c`; H S380); scripts/smoke-account + account-test 2026-09-16 (`bfcdc15`); account 2026-09-16 (P S390, L S387, E S386); all 14 root pages 2026-09-16 (`c0659f3`); js/ivrit-account + js/ivrit-saves 2026-09-16 (P S390, N S389, K S388); trope_tutor 2026-09-16 (N S389, M …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S394):** **BRANCH/PR: S393 CONTINUED `claude/loving-einstein-cdmbbz` — PR #246 was open, unmerged and draft at session start (verified through the API, not assumed), so the skill's continue rule applied. 3 fix commits + this close-out on top of S391/S392's 8. If #246 is STILL open and unmerged, continue on it; if merged, cut fresh off latest `origin/main` — never stack on merged history.** **⚑ DRIFT CHECK: NONE, a second consecutive session — `origin/main` still `b304db0`, nothing on it this branch lacks, clone NOT shallow. CHECK, never assume either way (S391's clone arrived SHALLOW and read 65 commits stale).** After S393: `sw.js` **v768**, FM **5.45**, migrations 0001-0003 all `Live? = yes`, SDK pin 2.116.0. Never trust these numbers unread. The `--sdk` fixture: `npm pack @supabase/supabase-js@2.116.0` (**npm IS reachable; the CDNs are not** — the same trick serves html2canvas 1.4.1 + jspdf 2.5.1 for the generator's PDF export, and pypi timed out where npm did not). The smokes hardcode port 8080 — kill your own server first, by PID. **Never `pkill -f` a server: it matches and kills this shell.**

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), I (S379), H (S380), C (S381), F (S383), B (S384), A (S385), E (S386), L (S387), K (S388), N (S389), P (S390), M (S391), G (S392), D (S393).** Unattended: take **I** (first-load & empty-state) — 14 sessions stale and now the longest unattended gap; re-run `i379/gates.mjs` (26 virgin cells; mechanical gates have been clean 16 consecutive runs, so **plant a defect first or the 17th zero means nothing**). Then **H, next the dashboard (S46 — still the stalest tool pairing in the table by a wide margin)**, C, F, B, A (the NEW `untrusted-card-field-interpolated-unescaped` row is its first sweep target), E, L, K, N, P, M (next: a chrome page never M-audited — `account.html` has never had one), G (next: `flash_cards.html` at S279) and D (next: the **generator**, S338, now the stalest D tool; trope/torah are S307).

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1–34 are S384–S392's; 35–40 are S393's — all of them in full in loop-findings, read them there first.** The S393 six in one line each: **(35)** a page's module-scope `let` is NOT on `window` (read the DOM). **(36)** `Performance.getMetrics` is all-zero until `Performance.enable` — abort when `Nodes` is 0. **(37)** an arm that never armed its control: assert the control's own visible effect before its zero counts. **(38)** the generator's live sync debounces `liveIsHeavyMode() ? 1000 : 500` ms and word search + crossword ARE the heavy modes — a 900ms wait is how S392 filed a candidate S393 had to refute. **(39)** `g392/lib.mjs`'s `openPage` has no `dark` option; `{dark}` is silently ignored and `assertAxis` is what catches it. **(40)** folder-tree row classes differ per page — flash cards' are `.preset-item[data-ft-item]`.

**⚑ TOP UNGATED CANDIDATES (S393): torah `.btn`, the LAST open carrier of `hover-feedback-survives-the-disabled-state`** — recipe most of the way there in loop-findings, the missing step is the bulk-select toggle inside the Copy bar; torah untouched for two sessions. Then **the tour-card h4 → h2 ×7** (breaks the 2-tool cap — split it, or take it as an A sweep); **the sub-floor touch targets**; **`clampMenu()` on resize/orientationchange**; **the FM chip's unmeasured phone geometry** (dismiss the ungated wizard first); **"sign out everywhere"** (new control + copy, so gate 1/2); **the Edge Function's production `localhost` origins** (backend boundary — maintainer redeploy). **NEW S393, both generator, both small: `.bingo-card-num` is declared TWICE** (~1763 and ~1812, the `.rw-flip-front` shape again — `ec8bace` fixed both copies, the duplication itself is still open), and **`12c170e`'s print rule keeps a now-redundant `color:#1a2744` DELIBERATELY** so the print block stays correct if the screen colour is changed back — read its comment before "cleaning it up".

**⚑ GATE-2 BENCH (unchanged unless noted): NEW S393 — the generator's Header Labels translates only Name/Date**; the answer-key banner, the `Answer Key — Teacher Copy` subtitle, the sheet footers and the Fill-Vowels instruction stay English (~10 render sites; authoring, not wiring). Then: the flash-card swatch labels; the generator's `restoreLastSetup` corrupt-drop; the dictionary drag-freeze + select-all cap; the corpus's split article tokens; the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the resources Taamim fallback; the FM's `nexthint_nikkud` copy; **the 5 unreferenced credit keys (S360)**. Gate 3/M: the FM stage annotations; `account.html`'s dark `.btn.ghost`/`.btn.danger`; **`.dash-edit-pencil`'s side in Hebrew**; **the hub's seven-size 2.4px type band**. Gate 4: the flash-card sticky-bar and landscape proposals. **Seed bench: 26.**

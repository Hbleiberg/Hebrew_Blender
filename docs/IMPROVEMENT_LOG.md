...
- [ ] P3 (**NEW S383 Pass F — 5 remaining carriers of the pattern registered this session; the class hit its 2-per-session cap on the two shipped**) | Hebrew_Font_Maker.html, trope_tutor.html, contact.html, resources.html, account.html | **A control dimmed by its disabled rule still lights its hover background: FM `.ctl-btn` (undo/redo start disabled) light + dark, trope `.btn[aria-disabled]`, …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S383 (2026-09-15):** flash_cards **2026-09-15 (`a234f9f`; F S383)**; hebrew_dictionary **2026-09-15 (`3674d6e`; F S383)**; the 9 cloud-panel pages **2026-09-15 (`5cd1b90`, shared modules; F S383)**; hebrew_blend_generator 2026-09-15 (`b5f06f5`; P S382); classroom_dashboard 2026-09-11 (`de49066`; G S364); torah_trainer 2026-09-11 (`f4a102d`); trope_tutor 2026-09-11 (`f4a102d`; M S376); Hebrew_Font_Maker 2026-09-11 (`e100574`; H S380); resources 2026-09-11 (`b1c0655`; N S375); privacy 2026-09-11 (`f8f23e0`; S360); index 2026-09-11 …[full …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`hover-feedback-survives-the-disabled-state`** (**NEW, registered 2026-09-15 (S383 Pass F) — 7 carriers found, 2 fixed**): ACTIVE. A control whose disabled rule dims it (`opacity` + `cursor:default`/`not-allowed`) but whose `:hover`/`:active` rule is unguarded, so the locked …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S384):** **BRANCH/PR: S383 CONTINUED `claude/wizardly-tesla-xu0ikd` (PR #242, draft, open, unmerged; base `4d6c02d`), adding 3 fix commits + this close-out to S382's 4. If that PR is still open and unmerged, CONTINUE on it; if merged, cut a fresh `claude/*` off latest `origin/main` and open a new draft PR at close-out. Verify via the API** (no workflow triggers on `pull_request`, so `get_status` reads `pending` with `total_count: 0` — that is GitHub's default for a commit with no legacy statuses, NOT a queued check. `get_check_runs` is the one that answers: it returns exactly 1, the Supabase GitHub App's **`Supabase Preview`**, concluded **`skipped`** — correct and expected, because migrations live in `db/` and not `supabase/` precisely so the integration never spins up a paid preview branch. A `skipped` Supabase Preview is a green PR, not a failure to chase.) **⚑ DRIFT CHECK: compare `origin/main` against `4d6c02d` + this loop's commits; S383 found NONE, S382 the largest yet — CHECK IT, never trust this unread. `sw.js` v750, FM 5.44, migrations 0001–0003 all `Live? = yes`.** A fresh container starts SHALLOW — `git fetch --unshallow` before `update-sitemap.mjs`. The `--sdk` fixture the required smokes need: `npm pack @supabase/supabase-js@2.116.0` (the npm registry IS reachable; the CDNs are not), untar, pass `package/dist/umd/supabase.js`; **pass `--sdk` to `smoke-tools.mjs` too or its signed-in B arms silently SKIP** (114 checks without it, 135 with).

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), B (S370), A (S371), E (S372), L (S373), K (S374), N (S375), M (S376), G (S377), D (S378), I (S379), H (S380), C (S381), P (S382), F (S383).** Unattended: take **B** (console & error audit — 5 arms, every probe controlled), then A. **O next `torah_trainer.html`; C next torah (S298) then dictionary (S285); M next `index.html` (S246) then torah; N next trope (S248) then index (S249); D next flash_cards or the Font Maker; H next the dashboard (S46) or the generator (S56); P next the dashboard or the dictionary; F next affordance: the CURSOR idiom on disabled controls (S383 measured the split but did not converge it), or the empty-state re-sweep; the next A sweeps `4d6c02d..HEAD` and must add the NEW `hover-feedback-survives-the-disabled-state` row plus `synced-key-wiped-without-forgetRow`, `tile-grid-built-hidden-never-roved`, `vh-capped-sheet-without-dvh-twin` and `dark-print-shadow-slab`; the next E re-runs its 17 arms over `408dbe4..HEAD` — E has still never seen the backend landing; the next L re-runs vs `4a1f186`; the next K over `4a1f186..HEAD`; the next I re-runs `i379/gates.mjs`.**

**⚑ F-PASS HARNESS (standing, NEW S383): census the CSSOM, never a grep — `document.styleSheets` sees the two account modules' INJECTED CSS, which is where 4 of the 7 carriers lived and which no grep of the HTML would ever find. Two traps S383 hit and you will hit again: (1) a guard written as a `background:var(--x)` shorthand does NOT expand to `background-color` in the CSSOM, so it reads as unguarded — re-check every `background-color` row against the shorthand before calling it a hit; (2) omitting `filter` from the property list hides an entire idiom (`.tbtn:hover{filter:brightness(1.06)}` guarded by `.tbtn:disabled{filter:none}`). And for any hover/press probe: assert `el.disabled` (or the aria attribute) AT the hover sample, not just before it — flash cards' 700ms auto-advance swaps in a fresh ENABLED card between the two, which manufactures a hit that looks exactly like the real one. The honest locked state there is a graded card revisited with Prev.**

**⚑ TOP UNGATED CANDIDATES (S383): the 5 remaining hover-vs-disabled carriers (FM `.ctl-btn` first — undo/redo start disabled, so it is the one a user meets soonest; then trope, contact, resources, account); the dictionary's inline `style.opacity` disabled hack; the tour-card h4 → h2 ×7 (it is the "next touch of a tour engine" — extract the shared block in the same session); the remaining sub-floor M calls (bingo stepper, scaled toggles, `#rulerCorner`); the `.rw-letter-tile .name` 7.2px grid (a layout decision); the unreachable nikkud hint (P4, half gate 2). Decide-then-sweep: whether an action TOOLBAR should rove like a selection grid (dashboard ×3, torah ×2 — S382 census), and whether the disabled CURSOR should converge (`not-allowed` 9 / `default` 17 — S383). Gate 2: the flash-card colour swatch labels (raw English, S207); the generator's `restoreLastSetup` corrupt-drop (forget or not); the dictionary drag-freeze and select-all preview-cap (S378); the corpus's split article tokens (S377); the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the four English-font picker headers; the resources preview stack's Taamim fallback. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals. The 5 torah/trope credit rows: wire-or-prune. **Seed bench: 26. Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

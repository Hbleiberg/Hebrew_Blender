...
- [ ] P3 (**NEW S367 seed scan — a friction, not a seed; the toast half is ungated, auto-Generate on Load is gate 2**) | hebrew_blend_generator.html | **`loadPreset(name)` ends at `applySettings(p)`: no toast, no re-render, no marker of which preset is active, while its sibling `runQuickStart` applies → `generate()` → `showPreviewNotice('worksheet.quickstart.loaded_notice')`.** A teacher clicks …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-10 | (S367 close-out) | branch/deploy note | **PR #227 MERGED before the session → S367 ran on the FRESH designated branch `claude/awesome-bardeen-jpi6sn` cut at `0ff0989` = `origin/main`; a NEW draft PR opened at close-out (number recorded in the handoff); a human merges + confirms Pages. Drift: none (`0ff0989`, sw v691, FM 5.39). sw v691→v692 (4 pages + locales); FM NOT bumped (a …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-10 | (S364 close-out) | branch/deploy note | **PR #227 open (draft, clean, head `f323c1a`) → S364 CONTINUED `claude/inspiring-meitner-rhqskk`, 5 commits; a human merges + confirms Pages. Drift: none (`f2384a7`, sw v688, FM 5.39). sw v688→v689 (2 pages); FM untouched; sitemap moved. DoD + compact/check. Deferred: micro-feature; O; 2 gate-4; credit rows; teacher copy; day plates.**

- [x] 2026-09-10 | `b83b896` | hebrew_blend_generator.html | **the bingo ▲/▼ card stepper gets `.bingo-step` (+ `-up`) with the `.btn-xs` hover step and its dark twin; inline styles dropped (S364, P4, `control-class-without-a-hover-state`)** | `g364/bingo.mjs` 8 cells, Bingo by its real mode button: real-mouse hover 8/8 (before 0/8), geometry/colours identical, click still steps 30→31

- [x] 2026-09-09 | `cabd6ca` | hebrew_blend_generator.html | **`showPreviewNotice` keeps its renderer (sites pass `() => I18n.t(…)`), `applyI18n` re-runs it, hide clears it (S364, P4, `validation-note-stores-rendered-text`)** | `g364/notice*.mjs` 4 cells: Settings-changed + Quick-Start notices EN→HE→EN by real clicks, dismiss/autohide still retire, a switch never revives; gates clean

- [x] 2026-09-09 | `0dc999c` | classroom_dashboard.html | **the print block's `.col-title` rule names `body.dark .col-title` too — dark-source prints with backgrounds on no longer ship #0a0f1c title slabs at 1.29:1 (S364 Pass G, P3, dark-override-outranks-print-remap)** | `g364/verify.mjs` 8 cells: titles white under print in dark (before rgb(10,15,28) ×4), light + dark screen unchanged

- [x] 2026-09-09 | `eed4c03` | classroom_dashboard.html | **`#blackout`, `#inplaceToolbar` join the print hide list; the editing outline drops on paper (S364 Pass G, P3, `fixed-surface-missing-from-the-print-hide-list` — NEW)** | `g364/verify.mjs` 8 cells by the real Blank-screen button + edit pencil: all three `none` under print, intact on screen, Escape closes both
...
- **S367 (2026-09-10):** trope_tutor **2026-09-10 (`92eadd0`; H-audited S367)**; hebrew_blend_generator **2026-09-10 (`e529293`; H-audited S363)**; classroom_dashboard **2026-09-10 (`5c7c796`; G-audited S364)**; Hebrew_Font_Maker **2026-09-10 (`33fcf13`)**; hebrew_dictionary 2026-09-09 (`10be341`; M-audited S362); index 2026-09-09 (S360); contact / privacy / terms / 404 2026-09-09 (S360); flash_cards 2026-09-10 (`e54dd1a`; N-audited S361); resources 2026-09-09 (S356); torah_trainer 2026-09-09 (S355).
...
- **`sub-floor touch target`**: ACTIVE. **S365: 2 FIXED — the generator's 7 unscaled `.toggle` switches 40×22 → 40×24 (`d86e728`, the `f92ae1d` step) and the dashboard week-editor swatches via the transparent colour input overhanging to 24×24 (`095bc47`, invisible — pixels …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S368):** **BRANCH/PR: S367 ran on the FRESH branch `claude/awesome-bardeen-jpi6sn` (cut at `0ff0989` = `origin/main`, PR #227 merged) → a NEW draft PR (see the S367 close-out entry / `list_pull_requests` on that head). Continue it if open; if merged, cut a fresh `claude/*` off latest `origin/main`. Verify via the API** (0 check runs is correct — no workflows). **Drift note: `sw.js` v692, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), C (S354), F (S355), B (S356), A (S357), E (S358), L (S359), K (S360), N (S361), M (S362), G (S364), D (S365), I (S366), H (S367).** Unattended: take C (accessibility, one tool) on `Hebrew_Font_Maker.html` — its last dedicated C was S257 (the S340 ordering carried since S365: FM S257 < the rest); the wizard, the category tabs, the glyph stage (`aria-` on the SVG/canvas), every modal's Escape/Enter, focus visibility in both themes, reduced motion, the touch-floor census (`#rulerCorner` 22×22 is a known gate-3 item — census it, don't re-file it), contrast light + dark. **O next `torah_trainer.html`; D next dictionary (S253); G next dictionary (S252); H next Font Maker (S241); N next resources (S246); M next trope_tutor (S245); the next I re-runs `i366/{gates,arms,corrupt,strshape}.mjs` and weights the S366–S367 fixes (the trope drill under blocked audio across SEVERAL sessions, not one).**

**⚑ Harness (standing): torah stubs Sefaria by fulfilling `**/*sefaria.org/**` (`b356/harness.mjs`); hidden toggle inputs → click `label.toggle:has(#id)`; the HTTP server dies across a compaction — curl it first; pypdf needs the stub `cryptography` package (`d352/stub`); `page.pdf()` fires `afterprint` — re-wear a print-state class before EACH pdf; PyMuPDF is absent (`g364/paper.mjs` CENSUS is the slab detector); a hover read needs a REAL `page.mouse.move` + 400 ms, a focus read a real Tab (never `page.focus()`); open a probe's host by ITS path (dashboard: `openWeekEditor()`, dismiss `#frBackdrop` via `frSkip()`, expand collapsed panels by `.panel-title`; the generator's live-preview toggle is in the collapsed Advanced panel, live sync speaks only after a first Generate, 500 ms debounced; FM Save Project ▾ = `[data-i18n="fontmaker.toolbar.save_project_menu"]`); an `elementFromPoint` receipt precedes a real click; modal probes read the LAST `[aria-modal]` whose computed `display` is not none; a `let` page global is not on `window` but IS reachable by bare name in `page.evaluate`; `ivritAskMode()` opens by calling it inside `page.evaluate`, never awaited; the harness auto-accepts dialogs — a confirm probe installs a dismissing handler; `page.addInitScript` re-runs on every navigation — seed a garbage key ONCE (`sessionStorage` flag); K's strongest arm is `k360/hecensus.mjs`; probe screenshot paths are cwd-relative (run from the scratchpad).**

**⚑ TOP UNGATED CANDIDATES (S367):** the generator's silent `loadPreset` (toast half; NEW S367); the tour-card h4 → h2 ×7 (CLAUDE.md: the next tour-engine touch extracts a shared block — not small); the remaining sub-floor M calls (the generator's 33 `scale(0.75)` sub-option toggles, gate 3; the bingo ▲/▼ stepper; FM `#rulerCorner`). Gate 2: the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on (`g364/out/board-print-en-dark.png`); the trope FAQ a7 copy. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. The next A sweeps the S357–S367 delta with the print-hide-list arm on all 7 tools, the wrong-shape arm (`i366/corrupt.mjs` + the static reader grep; the dictionary's `ivritSuite_wordLists` reader is the one unprobed store) and a `wired-then-clobbered label` re-sweep of every `data-i18n` element a JS setter also writes; the next K greps every authored key literally. **Seed bench: 4 → 25 (the user-directed S367 scan). Gate 1 next attended session: offer the trope "drill the marks I missed" (measured, S, no storage) and the three dual-audience S seeds (dashboard Everyone's-here, dashboard +1 min, generator sheet count) first; scan seeds are grep-grounded, NOT walked — ground each in a real run before building.**

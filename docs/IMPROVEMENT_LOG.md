...
- [ ] P4 (**NEW S344 — carriers 5–7 of `validation-note-stores-rendered-text`; the class was at cap after 2 fixes**) | classroom_dashboard.html | **Three notes keep the old language across a live EN⇄HE switch:** `pickerClassNote(msg)` (the S343 class-limit note), `setPickNote(msg)` (`dashboard.picker.new_round`) and `timerCustomNote(show)` (renders `dashboard.timer.custom_invalid` at show time) …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-08 | `74ec03d` | docs/reference/shared-components.md | **Pass E doc drift 2: §5's "Implemented on" still described the first preset-save slice ("≈15–21 alert() calls each…") while the pages carry 15 note helpers on 4 tools.** Re-trued to the census (with the grep that regenerates it), the retire rule now names the S343 helpers (`_retireWsHandoffNote`, `_retireWeekSaveEmptyNote`), …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-08 | `c7e990e` | flash_cards.html | **Three inline notes keep the old language across a live EN⇄HE switch (the S343 candidate, P4; `validation-note-stores-rendered-text` carriers 1–3).** `_setPresetNameNote`, `_setSaveDeckNote` and `_setWsHandoffNote` stored rendered `I18n.t()` text, so `applyI18n` could not re-render them. Each now stores its key (+ params for `deck_saved`) like …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-08 | (S343 close-out) | branch/deploy note | **Branch/PR: NEW `claude/improve-loop-divp8z` → draft PR #224 (opened at close-out; base `origin/main` `e0a94d9` = the merged PR #223's head, `merged_at` 2026-09-04).** **Zero outside-loop drift:** `origin/main` = the S342 close-out; `sw.js` v666 and `FONT_MAKER_VERSION` 5.39 re-read from the files. **SW v666→v667** (flash cards, …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-08 | `3866c82` | flash_cards.html | **The worksheet-handoff note and the Start note retire when the selection or mode changes (S343 Pass A, P3, `stale-validation-note-after-its-input-changes` — carriers 1–2, the pass's headline).** `wsHandoffNote` ('Pick at least one letter and one vowel' / 'Turn off Other mode first') cleared only inside `openInWorksheet`; and the S339 fix …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S344 (2026-09-08):** flash_cards **2026-09-08 (×1, `c7e990e` language-flip notes; S343 ×1)**; hebrew_blend_generator **2026-09-08 (×1, `608aa28` preset-name note; S342 ×1)**; classroom_dashboard 2026-09-08 (S343 ×2); resources 2026-09-04 (S341 ×1, `39cf34a`); index 2026-09-04 (S341 ×2, `ccd4898` + `9068087`); trope_tutor 2026-09-04 (S339 ×1, `fea54f5`); Hebrew_Font_Maker 2026-09-04 (S338 ×1, `fc73fd9`; outside-loop v5.39); torah_trainer 2026-09-04 (`f92ae1d`); hebrew_dictionary 2026-09-03 (`ecd0720`); contact / privacy / terms 2026-09-03 …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`validation-note-stores-rendered-text`**: ACTIVE. **Registered 2026-09-08 (S344, out of the S343 flash-cards candidate): first suite-wide read of every `*Note(` helper on 8 pages — hits 4 helpers on 2 pages, ALL FIXED (`c7e990e`, `608aa28`); 3 more on the dashboard logged as …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S345):** **BRANCH/PR: S344 CONTINUED `claude/improve-loop-divp8z` → draft PR #224 (base `origin/main` `e0a94d9`; head after this close-out). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v668, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS O (S315, attended only), THEN L (S332), K (S333), N (S334), M (S335), H (S336), G (S337), D (S338), I (S339), C (S340), F (S341), B (S342), A (S343), E (S344).** **Unattended: skip O and take L** (SEO: byte-check every page's head metas + JSON-LD against the S332 close-out — grep the archive for its commit — then the 14/14 mechanical arms in the skill's L definition; copy fixes are gate 2 — log and batch). O's surface is still flash_cards.html. **Harness notes (standing; full list in the S341–S344 archive entries):** CSP forbids string predicates; never run the FM wizard arm beside another harness; a refused `aria-disabled` click is a FINDING on a just-fixed action but an ARTIFACT on a deliberately-disabled Save — reach the handler with `press(…,'Enter')` or `locator.dispatchEvent('click')`; settle 900 ms after `classList.add('dark')` before reading colours; sweep `.open` off `modal|overlay|backdrop` and `openSettings()` + strip `.collapsed` to reach the dashboard drawer; a `cd` in a chained Bash command persists — use absolute paths. **Ledger lesson (standing):** write open candidates without the `NON_ACTIONABLE` phrases or `compact-ledger` files them.

**⚑ ONE UNGATED DEFECT IS OPEN — the S344 dashboard language-flip notes (P4, `validation-note-stores-rendered-text` carriers 5–7: `pickerClassNote`/`setPickNote`/`timerCustomNote`; fix shape in the candidate; the class is fresh next session) — take it if L's findings leave budget; the S285 tooltip-binding divergence (P4, F's remit, dashboard) is the only other.** The rest are gates or decisions (the picker terms, hub author URL, PDF paper choice, 7.2px grid, help tab strip, timer placeholder copy, Save-as-Word-List entry — each marked in its candidate). **Seed bench (unchanged):** the holiday share link (torah, S336 — ~15 lines, the smallest seed), the handout selection (torah, S336), the scroll-style handout (torah, S324), the last-backup date (index, S323).

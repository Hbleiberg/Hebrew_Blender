...
- [ ] P4 (**NEW S343 Pass A — a K/F shape, not fixed: the pattern class was at cap**) | flash_cards.html | **Three inline notes keep the old language across a live EN⇄HE switch:** `_setWsHandoffNote`, `_setPresetNameNote` and `_setSaveDeckNote` store `I18n.t()` TEXT, so `applyI18n` cannot re-render them (it re-renders only the key-based `_setStartNote`/`_setWeakNote`, line ~8433), while the …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-08 | (S343 close-out) | branch/deploy note | **Branch/PR: NEW `claude/improve-loop-divp8z` → draft PR (opened at close-out; base `origin/main` `e0a94d9` = the merged PR #223's head, `merged_at` 2026-09-04).** **Zero outside-loop drift:** `origin/main` = the S342 close-out; `sw.js` v666 and `FONT_MAKER_VERSION` 5.39 re-read from the files. **SW v666→v667** (flash cards, dashboard …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-08 | `6fce9be` | classroom_dashboard.html + locales/ui-strings.csv | **The student picker's class-limit note is localized (S343 Pass A, P3 — a check-i18n Check-A blind spot, the template-literal shape).** `newClass()` wrote `You can have up to ${ROSTER_MAX_CLASSES} classes. Delete one to add another.` straight into `#pickerClassNote`, so the Hebrew UI showed it in English. New key …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-08 | `df367c9` | classroom_dashboard.html | **The week editor's 'Add at least one schedule entry first' Save note retires when a period arrives (S343 Pass A, P3, `stale-validation-note-after-its-input-changes` — carrier 3).** `_setWeekSaveNote('dashboard.schedule.save_empty')` set Save `aria-disabled` and cleared only on a NAME keystroke or the next Save press, so + Add period left …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-08 | `3866c82` | flash_cards.html | **The worksheet-handoff note and the Start note retire when the selection or mode changes (S343 Pass A, P3, `stale-validation-note-after-its-input-changes` — carriers 1–2, the pass's headline).** `wsHandoffNote` ('Pick at least one letter and one vowel' / 'Turn off Other mode first') cleared only inside `openInWorksheet`; and the S339 fix …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | (S342 close-out) | branch/deploy note | **Branch/PR: CONTINUED `claude/improve-loop-0mbq58` → draft PR #223** (API at start: open, draft, `mergeable_state:clean`, head `494086e`, base `4b66a64`, 0 check runs — correct, no workflows). **Zero outside-loop drift:** `origin/main` still `4b66a64`; `sw.js` v665 and `FONT_MAKER_VERSION` 5.39 re-read from the files. **SW v665→v666** …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | `114cae6` | flash_cards.html + hebrew_blend_generator.html (+ storage-and-backup.md) | **A non-object preset member is never applied (S342 Pass B, P3, `apply-settings-trusts-collection-members` — carriers 15–16).** The import merges the presets map shallowly, so a hand-edited `.ivrit` can leave `presets:{X:"str"}` / `{X:{settings:"str"}}` in the store; the folder tree renders …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | `655f4d7` | classroom_dashboard.html | **A saved week without a periods array is rejected, not minted as a blank grid (S342 Pass B, P3, `apply-settings-trusts-collection-members` — carrier 17).** `normalizeScheduleWeek` built an empty week from any object, so a saved-schedule entry `{v:2, week:{}}` or a garbage `scheduleWeek` in the settings blob REPLACED the teacher's real …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S343 (2026-09-08):** flash_cards **2026-09-08 (×1, `3866c82` handoff/Start notes; S342 ×1)**; classroom_dashboard **2026-09-08 (×2, `df367c9` week-save note + `6fce9be` class-limit i18n — AT CAP; S342 ×1)**; hebrew_blend_generator 2026-09-04 (S342 ×1, `114cae6`); resources 2026-09-04 (S341 ×1, `39cf34a`); index 2026-09-04 (S341 ×2, `ccd4898` + `9068087`); trope_tutor 2026-09-04 (S339 ×1, `fea54f5`); Hebrew_Font_Maker 2026-09-04 (S338 ×1, `fc73fd9`; outside-loop v5.39); torah_trainer 2026-09-04 …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`dark-hover-resolves-to-the-rest-colour`**: ACTIVE. **Re-swept 2026-09-08 (S343 Pass A, delta-only): the delta's 7 new `:hover` rules are all the S331 `details > summary:hover{text-decoration:underline}` idiom (no `warm-gray` fill) — hits 0, clean streak 2.** S330: 45 …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S344):** **BRANCH/PR: S343 ran on a NEW branch `claude/improve-loop-divp8z` → a draft PR opened at close-out (base `origin/main` `e0a94d9`; PR #223 merged 2026-09-04). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v667, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS O (S315, attended only), THEN E (S331), L (S332), K (S333), N (S334), M (S335), H (S336), G (S337), D (S338), I (S339), C (S340), F (S341), B (S342), A (S343).** **Unattended: skip O and take E** (freshness & site health over the delta since `211af87`/S331 — sitemap `lastmod` vs git, SW precache list vs files both ways, THIRD_PARTY_LICENSES vs deps, robots/CNAME, README / CLAUDE.md / `docs/reference/*` accuracy — including whether `shared-components.md` §5's retire rule should now name `_retireWsHandoffNote`/`_retireWeekSaveEmptyNote`, and `storage-and-backup.md` after S342). O's surface is still flash_cards.html. **Harness notes (standing; the full list lives in the S341–S343 archive entries):** CSP forbids string predicates; never run the FM wizard arm beside another harness; a refused `aria-disabled` click on a just-fixed primary action is a FINDING (and the BEFORE detector); gate import-time arms on `process.argv`; settle ~150 ms after `scrollIntoView` and ≥ the longest `transition` (900 ms) after `classList.add('dark')` before reading computed colours; sweep `.open` off `modal|overlay|backdrop` (the dashboard's `.fr-modal` gate intercepts every click in a fresh context) and `openSettings()` + strip `.collapsed` to reach the drawer. **Ledger lesson (standing):** write open candidates without the `NON_ACTIONABLE` phrases or `compact-ledger` files them.

**⚑ ONE UNGATED DEFECT IS OPEN — the S343 flash cards language-flip notes (P4, K/F shape, `wsHandoffNote`/`presetNameNote`/`saveDeckNote` keep the old language; fix shape in the candidate) — take it if E's findings leave budget; the S285 tooltip-binding divergence (P4, F's remit, dashboard) is the only other.** The rest are gates or decisions — the four unauthored picker terms (gate 2); the staging author URL on the hub (gate 2); the specimen-A4-vs-template-Letter paper choice (gate 2); the real-words 7.2px grid (layout decision); the help tab strip (gate 3); the S321 timer placeholder copy (gate 2); the dictionary Save-as-Word-List entry (maintainer chose log-only). **Seed bench (unchanged):** the holiday share link (torah, S336 — ~15 lines, the smallest seed), the handout selection (torah, S336), the scroll-style handout (torah, S324), the last-backup date (index, S323).

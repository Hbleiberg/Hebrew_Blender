...
- [ ] P4 (**S369 Pass F — the remaining sub-11px FUNCTIONAL labels, one 0.7rem batch per page; resources + dictionary SHIPPED S374 (`f4b3326`, `1de1ee8`); census in loop-findings**) | hebrew_blend_generator.html + flash_cards.html + privacy.html | **generator `.font-opt-attr` 9.92 ×12 + `.font-opt-default` + "Slower"/"Fast" 9.92 + the vowel-group sound labels 10.88 ×11 (also on the dictionary) + `.fs-val` 10.88; flash `.vname` 10.56 ×13 + `[DEFAULT]` 9.6 + the credit line 10.4; privacy `.badge` 9.92.** Letter-tile names (`41679da`) and `.rw-letter-tile` (7.2) stay as logged. | found S369
...
- [x] 2026-09-11 | (S374 close-out) | branch/deploy note | **S374 CONTINUED `claude/improve-loop-swgwqx` (PR #229 open, unmerged; base `66dd97b` = `origin/main`), 4 commits + this close-out; a human merges and confirms the Pages run. Drift: none (`66dd97b`, sw v698, FM 5.39 — as the S373 close-out recorded). sw v698→v699 (torah, trope, resources, dictionary); FM NOT bumped (untouched). Scripts: …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S374 (2026-09-11):** torah_trainer **2026-09-11 (`31a84f7`; K S374)**; trope_tutor **2026-09-11 (`5d1d13a`; H S367)**; resources **2026-09-11 (`f4b3326`; N S246)**; hebrew_dictionary **2026-09-11 (`1de1ee8`; M S362)**; Hebrew_Font_Maker 2026-09-11 (`9257fb8` S373; C S368); classroom_dashboard 2026-09-11 (`c365786` S373; G S364); flash_cards 2026-09-11 (`5fd4bc1` S373; N S361); hebrew_blend_generator 2026-09-10 (`e4c8d75` S371; H S363); index 2026-09-10 (`edf4b2e` S370); contact / privacy / terms / 404 2026-09-09 (S360); all 14 pages L-audited S373, K-audited S374.
...
- **`non-finite-number-from-a-loaded-file`**: ACTIVE (consequence-critical). **S374: the 4 S371 carriers FIXED (`31a84f7` torah `karaokeRate`/`ttsRate`; `5d1d13a` trope `hebFontSize`/`playbackRate`) via `_sliderNum` at `loadSettings`; open hits 0, clean streak 0.** Shape: `1e999` is valid JSON → `Infinity`; `parseFloat(x) || d` passes it. Detection (runtime): seed each store with a RAW `1e999` per numeric key (`JSON.stringify` writes null), load, scan `*Val` labels, inline styles, root custom props and the saved-back key for Infinity/NaN. Fix: `Number.isFinite(v) ? clamp(v) : default`.
...
**Next session (S375):** **BRANCH/PR: S369–S374 ran on `claude/improve-loop-swgwqx` (cut at `66dd97b` = `origin/main`; 30 commits) → draft PR #229. If it is still OPEN and unmerged, continue on it; if MERGED, cut a fresh `claude/*` off latest `origin/main` — never stack on merged history. Verify via the API** (0 check runs is correct — no workflows). **⚑ DRIFT CHECK: compare `origin/main` against `66dd97b`; anything beyond THIS loop's 30 commits is an outside-loop landing. `sw.js` v699, `FONT_MAKER_VERSION` 5.39** — re-read both. A fresh container starts shallow again — `git fetch --unshallow` before `update-sitemap.mjs`.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), N (S361), M (S362), G (S364), D (S365), I (S366), H (S367), C (S368), F (S369), B (S370), A (S371), E (S372), L (S373), K (S374).** Unattended: take N (mobile & touch) over `resources.html` (S246, its stalest surface; the directory cards, the category/age filter rows, the font gallery + preview modal, the Suggest form) with real descriptors (`n361/` is the harness shape: iPhone SE 320, iPhone 13 390 + landscape, Pixel 7 412; `pointer:coarse`, `hover:none`), all 7 arms, the phone contact sheet even when clean; a fix inside the phone query stays autonomous, anything else is gate 4 → log with screenshot paths. Then M over trope_tutor (S245). **O next `torah_trainer.html`; D next dictionary (S253); G next dictionary (S252); H next Font Maker (S241); C next flash_cards (S271); F next the disabled-control look; the next A sweeps `1d4c9cf..HEAD` with the range-setter arm on the Font Maker's 13 sliders + the new torah/trope `_sliderNum` sites; the next I re-runs `i366/{gates,arms,corrupt,strshape}.mjs`; the next E re-runs the 17 arms over `408dbe4..HEAD`; the next L re-runs `l373/{audit,bytecheck}.mjs` vs `4a1f186`; the next K re-runs `k374/{static.py,he.mjs}` over `4a1f186..HEAD` (probe defects g/h are closed in `static.py`; seed the Infinity cell with the RAW `1e999` string).**
...
**⚑ TOP UNGATED CANDIDATES (S374):** the remaining sub-11px batches (generator, flash, privacy — `f369/census-en.txt`; resources + dictionary shipped S374); the ~30 inline Font Maker hints under 11px; the tour-card h4 → h2 ×7; the remaining sub-floor M calls (bingo stepper, scaled toggles, `#rulerCorner`). Gate 2: the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on; the trope FAQ a7 copy; the four English-font picker headers. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. **Seed bench: 25 (unchanged; K is not an intake). Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

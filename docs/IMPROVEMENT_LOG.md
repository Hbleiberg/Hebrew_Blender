...
- [x] 2026-09-04 | (S341 close-out) | branch/deploy note | **Branch/PR: CONTINUED `claude/improve-loop-0mbq58` → draft PR #223** (API at start: open, draft, `mergeable_state:clean`, head `53066b7`, base `4b66a64`, 0 check runs — correct, no workflows). **Zero outside-loop drift:** `origin/main` still `4b66a64`; `sw.js` v664 and `FONT_MAKER_VERSION` 5.39 re-read from the files. **SW v664→v665** …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | `9068087` | index.html | **The Resources card's Hebrew Fonts chip reaches the 24px touch floor (S341, P4, `sub-floor touch target` — the S340 pointer's named first pick).** `.card-attr` gains `min-block-size:24px`: the text-only chip goes 141×23 → 141×24 (HE 137×24), the image-bearing Sefaria sibling stays 83×36, the card grows 1px. | Verified headless in 4 cells + iPhone 13: …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | `2d985c1` | classroom_dashboard.html | **The week editor's ⋯ day menu is keyboard-operable (S341 Pass F, P2 — the dashboard's own copy of the floating-menu pattern).** It already carried `role=menu`/`menuitem`, but the menu is body-appended, outside the editor's Tab trap, so focus never entered it and the editor's capture-phase Escape closed it without refocusing. Now: …[full …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | `ccd4898` | index.html + classroom_dashboard.html + flash_cards.html + hebrew_blend_generator.html (+ storage-and-backup.md) | **The shared folder-tree Move ▾ / ↗ menu is keyboard-operable on all four carriers (S341 Pass F, P2 — the pass's headline).** The block's menu had no role, no `aria-haspopup`, focus stayed on the trigger, the body-appended menu came last in the Tab …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | `39cf34a` | resources.html | **The Suggest a Resource and Font Preview modals take the hub's dialog contract (S341 Pass F, P2).** Both `.sg-backdrop` modals advertised `role=dialog aria-modal="true"` while a keyboard user's focus stayed on the page behind (Suggest: 12/12 Tabs on the cards under the backdrop; Preview: Tab fell out of the page at its last field) and no close path …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | (S340 close-out) | branch/deploy note | **Branch/PR: CONTINUED `claude/improve-loop-0mbq58` → draft PR #223** (API at start: open, draft, `mergeable_state:clean`, head `94927a1`, base `4b66a64`, 0 check runs — correct, no workflows). **Zero outside-loop drift:** `origin/main` still `4b66a64`; `sw.js` v663 and `FONT_MAKER_VERSION` 5.39 re-read from the files. **SW v663→v664** …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | `dc9bb62` | index.html | **The bookmark tip is a live region (S340 Pass C, P3, WCAG 4.1.3).** `showBookmarkTip` wrote the only response to 🔖 Bookmark into a plain div, so a screen-reader user heard nothing; `role="status"` on `#bookmarkTip` (the role `#ivritStatus` already uses) announces the Ctrl+D / ⌘D hint when it is set. Nothing visual changes. | Verified headless in 4 …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | `b46648b` | index.html | **The hub's `.ivrit` Merge/Replace dialog is a keyboard-operable accessible dialog (S340 Pass C, P2 — the pass's headline).** The hub's `ivritAskMode` never took the shared engine's accessible-dialog clause: no dialog role, focus stayed on Browse…, 0 of 12 Tabs entered it, and Escape closed the AllTools modal BEHIND it while the prompt stayed up — a …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | (S339 close-out) | branch/deploy note | **Branch/PR: CONTINUED `claude/improve-loop-0mbq58` → draft PR #223** (API at start: open, draft, `mergeable_state:clean`, head `73c2ed6`, base `4b66a64`, 0 check runs — correct, no workflows). **Zero outside-loop drift:** `origin/main` still `4b66a64`; `sw.js` v662 and `FONT_MAKER_VERSION` 5.39 re-read from the files. **SW v662→v663** …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | f001815 | flash_cards.html | **A blocked Start note retires when the teacher fixes its input** (`_retireStartNote(key, ok)` from the letter / lock / vowel / color / emoji / word-list / number-range handlers — the S339 Pass I P3; registers `stale-validation-note-after-its-input-changes`; one clause in `shared-components.md` §5) | real clicks: letters + vowels empty → 2 lines; a …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | fea54f5 | trope_tutor.html | **`applyI18n`'s Learn re-render no longer writes `hebrewTropeTutor_progress`** (`_i18nRerender` guard beside `_chartPrintBuild` — the S339 Pass I P4, CLAUDE.md i18n §6) | virgin profile with every localStorage write stack-traced: the one progress write comes from the data-ready init render; `setLang('he')` and `setLang('en')` write 0; a family-chip …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | — | Hebrew_Font_Maker.html | **STRUCK (refuted): the partner-wizard stall candidate** (S337 once-seen; S338 8 clean loads) — S339 saw its face exactly once, in the cell that ran while the Torah arm shared the CPU, then instrumented `#wizNext`'s `aria-disabled` writer and both fields' value setters at 1× / 4× / 10× CPU throttle: only the two legitimate `oninput` writes, step 3 …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | (S338 close-out) | branch/deploy note | **Branch/PR: CONTINUED `claude/improve-loop-0mbq58` → draft PR #223** (API at start: open, draft, `mergeable_state:clean`, head `bdc04e4`, base `4b66a64`, 0 check runs — correct, no workflows). **Zero outside-loop drift:** `origin/main` still `4b66a64`; `sw.js` v661 and `FONT_MAKER_VERSION` 5.39 re-read from the files. **SW v661→v662** …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | 5b55d19 | hebrew_blend_generator.html | **The Export PDF's page images are Flate-compressed** (`uncompressed-jspdf-raster`, the pattern's last carrier — S338 Pass D measured the defect first: a one-sheet worksheet 10.46 MB, a landscape sheet 6.0 MB, an 8-card bingo set 107 MB with a 3.3 s main-thread block while the file was assembled). One argument pair, `undefined, 'FAST'`. | …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | fc73fd9 | Hebrew_Font_Maker.html | **The UFO export declares installable embedding** — `openTypeOS2Type: []` in `buildUfoFontInfo` (the S337 P3: absent, ufo2ft/fontmake default to fsType 4 while the TTF export writes 0). | Verified with the real engine (the S337 mirror recipe, `g337/g1.mjs`): the downloaded `.ufo.zip`'s fontinfo.plist carries the empty array, ufoLib 4.64 …[full …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-03 | (S337 close-out) | branch/deploy note | **Branch/PR: CONTINUED `claude/improve-loop-0mbq58` → draft PR #223** (API at start: open, draft, `mergeable_state:clean`, head `171b914`, base `4b66a64`, 0 check runs — correct, no workflows). **Zero outside-loop drift:** `origin/main` still `4b66a64`; `sw.js` v660 and `FONT_MAKER_VERSION` 5.39 re-read from the files. **SW v660→v661** …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-03 | 9ae3495 | Hebrew_Font_Maker.html | **The UFO export works in the browser again — every "Export UFO" since the format shipped (`98e47a9`, 2026-06-10) ended in "UFO build failed: No module named 'fs'"** (S337 Pass G, P1 — a dead advertised export). `hfm_build_ufo` imported `fontTools.ufoLib.glifLib.GlyphSet`; ufoLib imports pyfilesystem2 at module load, Pyodide's fonttools …[full …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-03 | e0caf12 | Hebrew_Font_Maker.html | **The specimen and template PDFs Flate-compress their page rasters** (S337 Pass G; registers `uncompressed-jspdf-raster`). Both `addImage` calls passed no compression argument and jsPDF's default stores the decoded PNG raw: the Punctuation template weighed 59.8 MB, the specimen 45.9 MB, the one-page vowel sheet 15.0 MB — the same page as a PNG …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-03 | (struck — verified) | Hebrew_Font_Maker.html | **The standing P4 "the template PDF path could not be verified offline" is closed:** S337 produced all five template PDFs, the specimen PDF, the PNG route, the TTF, WOFF2, zip and UFO through the real jsPDF/html2canvas/Pyodide path (CDNs mirrored locally). Receipts in `loop-findings.md`. | `g337/out/ezra-sil-sr/*.pdf`, pymupdf.
...
- **S341 (2026-09-04):** resources **2026-09-04 (×1, `39cf34a` Suggest + Font Preview dialog contract)**; index **2026-09-04 (×2, `ccd4898` folder-tree block + `9068087` Fonts chip; S340 ×2)**; classroom_dashboard **2026-09-04 (×2, `ccd4898` folder-tree block + `2d985c1` ⋯ day menu)**; flash_cards 2026-09-04 (S341 ×1 `ccd4898`; S339 ×1); hebrew_blend_generator 2026-09-04 (S341 ×1 `ccd4898`; S338 ×1); trope_tutor 2026-09-04 (S339 ×1, `fea54f5`); Hebrew_Font_Maker 2026-09-04 (S338 ×1, `fc73fd9`; outside-loop v5.39); torah_trainer 2026-09-04 …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`popup-without-a-keyboard-contract`**: ACTIVE (**NEW, registered 2026-09-04 (S341 Pass F) — 4 carriers found in one pass, ALL FIXED: the resources Suggest + Font Preview modals `39cf34a`, the shared folder-tree Move ▾ menu on its 4 carriers `ccd4898`, the dashboard week …[full text: IMPROVEMENT_ARCHIVE.md]

- **`stale-validation-note-after-its-input-changes`**: ACTIVE (**NEW, registered 2026-09-04 (S339 Pass I) — 2 known carriers, both fixed: the generator's `toggleLetter` / vowel clear (earlier, `4515`-shape) and flash cards' Start note `f001815`**). Detection: every …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]

- **`uncompressed-jspdf-raster`**: ACTIVE (registered S337 Pass G). **S338: the 3rd and last known site FIXED `5b55d19` (the generator's worksheet / bingo `addImage`, measured by Pass D at 10.46 MB per sheet and 107 MB per 8-card bingo set) — 0 open carriers; clean streak 0 (the …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **blocking-alert-for-a-routine-path** **S336: +1 carrier FIXED `009611b` — the dashboard timer's `custom_invalid` alert → inline `.sched-warn` note (the `#locNote` idiom); dashboard static census 11→10. Still ACTIVE.**  (an `alert()` guarding an outcome that is NORMAL rather …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S342):** **BRANCH/PR: S341 CONTINUED `claude/improve-loop-0mbq58` → draft PR #223 (head after this close-out; base `origin/main` `4b66a64`). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v665, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS O (S315, attended only), THEN B (S329), A (S330), E (S331), L (S332), K (S333), N (S334), M (S335), H (S336), G (S337), D (S338), I (S339), C (S340), F (S341).** **Unattended: skip O and take B** (the S329 row's four arms are the floor; add the S341 surfaces — the resources modals, a Move ▾ destination chosen by keyboard, the ⋯ day menu — and a listener-leak arm: CDP `getEventListeners` on `document`/`window` before vs after an open+close cycle of every `.ft-move-menu` / `.sg-backdrop`). O's surface is still flash_cards.html. **Harness notes (standing; older ones in the S341 archive entry):** CSP forbids string predicates; never run the FM wizard arm beside another harness; a refused `aria-disabled` click on a just-fixed primary action is a FINDING; a module that runs an arm at import time re-runs it in every importer — gate on `process.argv` AND pass a dummy arm name when importing; `page.evaluate` of a Promise-returning opener awaits the modal — call it as `(fn(), 'called')`; `scrollIntoView` right before a keyboard open fires scroll-close listeners — settle ~150 ms; an "outside click" at the screen corner hits the dashboard drawer's backdrop — click inside the drawer; a capture-phase document handler runs before an element-level one — put the refocus on the capture path too. **Ledger lesson (standing):** write open candidates without the `NON_ACTIONABLE` phrases or `compact-ledger` files them.

**⚑ THE STRONGEST UNTAKEN CANDIDATE IS THE S285 TOOLTIP-BINDING DIVERGENCE (P4, F's remit — the dashboard binds the tip to `.tip-icon`, the other four to `.tip-wrap`; both valid, converge only with a measured reason; the dashboard was at cap this session).** Every other open candidate is a gate or a decision — the four unauthored picker terms (gate 2); the staging author URL on the hub (gate 2); the specimen-A4-vs-template-Letter paper choice (gate 2); the real-words 7.2px grid (layout decision); the help tab strip (gate 3); the S321 timer placeholder copy (gate 2); the dictionary Save-as-Word-List entry (maintainer chose log-only) — so S342's fixes come from its pass. **For the next A sweep:** `popup-without-a-keyboard-contract` wants its first delta sweep (any new `classList.add('open')` / body-appended menu since `9068087`); `stale-validation-note-after-its-input-changes` still wants its first suite-wide runtime sweep; `uncompressed-jspdf-raster` continues its retirement streak; `blocking-alert-for-a-routine-path` still wants its dashboard click census; the S335 runtime arm for `background:#fff` / `color:#1a2744` on the 8 other pages still stands. **Seed bench (unchanged):** the holiday share link (torah, S336 — ~15 lines, the smallest seed), the handout selection (torah, S336), the scroll-style handout (torah, S324), the last-backup date (index, S323).

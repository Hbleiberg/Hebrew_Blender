...
- [ ] P3 (**NEW S351 — found while verifying the micro-feature; a pre-existing S349 defect the S349 probe could not see**) | torah_trainer.html | **The handout bar's "Print only the selected verses" row renders with nothing selected: `#ttHandoutSelRow` carries the `hidden` attribute but its `.tt-ctrl` `display:flex` beats the UA `[hidden]` rule** (measured on a fresh profile: hidden=true, …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P3 (**NEW S351 Pass G — the pointer's question, answered: a real paper divergence**) | torah_trainer.html | **A selected-verses-only handout prints the print band and the sheets-2..N margin label with the FULL reading range** — measured: 1:2–1:4 selected, Print only the selected verses on, `_handoutEnter` + beforeprint → `#ttPrintBand` "Bereshit — Genesis 1:1-6:8", `--tt-print-l` the same, …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P4 (**NEW S351 Pass G — GATE 2 ASKED AND APPROVED: "paper always gets the chart"; ships next session, trope_tutor was at its 2/tool cap**) | trope_tutor.html | **Ctrl+P on the Drill tab prints the drill screen, not the chart:** `#tuPrintSheet` lives inside `#learnView`, which is `hidden` while Drill is open, so the start screen prints 2 sheets (the mark picker as 26 "—" rows) and a live …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S351 (2026-09-08):** trope_tutor **2026-09-08 (×2, `bdfd57c` footer FAQ, `d9176bc` plates)**; torah_trainer **2026-09-08 (×1, `dca9609` scroll-style handout; S349 ×1; S348 ×1)**; index 2026-09-08 (S350 ×1, `aab1392`; S347 ×1); contact 2026-09-08 (S350 ×1, `ac5f930`); 404 2026-09-08 (S349 ×1, `5e64c95`); flash_cards 2026-09-08 (S347 ×1, `7c6337a`; S346 ×1); classroom_dashboard 2026-09-08 (S345 ×1, `4e25247`); hebrew_blend_generator 2026-09-08 (S344 ×1, `608aa28`); resources 2026-09-04 (S341 ×1, `39cf34a`); Hebrew_Font_Maker 2026-09-04 (S338 …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S352):** **BRANCH/PR: S351 CONTINUED `claude/improveloop-deslop-je9vs9` → draft PR #225 (base `origin/main` `aebb86b`; S346–S351 on it — 18 commits; ask the maintainer to merge before the branch grows further). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v675, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS D (S338), THEN I (S339), C (S340), F (S341), B (S342), A (S343), E (S344), L (S345), O (S346), K (S347), N (S348), M (S349), H (S350), G (S351).** Take D (performance snapshot, one tool): re-derive the least-recently-D'd tool from the archive's D rows (the generator was S338). **O is attended-only; its next surface is `torah_trainer.html`.** **K owes its next run two arms** (runtime HE census; whole-corpus plain-argument probe). **N's next surface is `flash_cards.html` (S244).** **H's next is likely the generator (S251) — verify.** **G's next is the dashboard (S231).** Harness (standing): O detector = the `skill-v4.1.3` tag; torah verification stubs Sefaria by fulfilling `**/*sefaria.org/**` (use te'amim-bearing text when the probe counts marks); hidden toggle inputs → click `label.toggle:has(#id)`; the HTTP server does not survive a compaction or a long idle — curl it before every Playwright run; `page.emulateMedia({media:'screen'})` poisons later `page.pdf()` calls — reset with `media:null`; pypdf imports only with the `cryptography` stub (scratchpad s351/pdftext.py). **Ledger lesson (standing):** write open candidates without the `NON_ACTIONABLE` phrases; run `compact-ledger --apply` ONCE.

**⚑ FOUR UNGATED CANDIDATES ARE OPEN, all from S351 — take them in order:** (1) torah `#ttHandoutSelRow` renders despite `hidden` (P3, one CSS line; verify by the RENDERED box, not the attribute); (2) torah print band names the full range on a selected-verses handout (P3); (3) trope Drill-tab print → the chart (P4, GATE 2 APPROVED — ship it); (4) trope 13-sheet chart compaction (P3, render and inspect). The S285 tooltip-binding divergence (P4) stays filed for F; every other candidate is a gate or a decision. **Seed bench:** the tool-card data-indicator seed (index, S; dual-audience), the submit-a-font form (resources, M), the dashboard per-day overrides (M) — SHIPPED: scroll-style handout S351 (`dca9609`), erase-offers-backup S350 (`aab1392`), handout selection S349 (`c3d2e11`), holiday share link S348 (`4329006`), last-backup date S347 (`72104a2`).

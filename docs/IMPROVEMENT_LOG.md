...
- [ ] P4 (**NEW S394 — the 9 the `selector-declared-twice-in-one-stylesheet` census left OPEN in its worst carrier; the 5 IDENTICAL flip-card ones shipped `7de7842`**) | hebrew_blend_generator.html | **`.bingo-card-num` and `.bingo-grid` are byte-identical twins, but they sit inside a deliberate later layer that re-declares the bingo shapes in print-safe literals (`.bingo-card` gains …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [ ] P4 (**NEW S389 Pass N — a VOID diagnosed, not a clean; the one carrier the census could not reach**) | Hebrew_Font_Maker.html | **The account chip's phone geometry is UNMEASURED there: all 8 cells came back "menu not open" because the onboarding wizard (ungated by design) covers the header, even with `hebrewFontMaker_mobileWarnDismissed` seeded.** The next N or P dismisses the wizard first, …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-16 | `bfcdc15` | scripts/smoke-account.mjs + account-test.html | **The accounts smoke measures the chip at phone widths** (S390's strongest ungated candidate, P3). Arm F's narrowest cell was 800×700 — a tablet — so all 17 phone overflows Pass N found in S389 were invisible to it while it reported 33/33 green. New arm G: 320 and 390, EN + HE, asserting both that the open menu stays …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-16 | (S390 close-out) | branch/deploy note | **S390 ran on `claude/modest-edison-bgpu71`, a FRESH branch — PR #242 is MERGED (`af96827`), so the skill's merged-PR rule applied: cut from latest `origin/main`, new draft PR at close-out.** **Drift, the first in eight sessions:** `origin/main` `44963bd`, `sw.js` v756→**v758**, and one outside-loop commit (`44963bd`, Google's four-colour …[full text: IMPROVEMENT_ARCHIVE.md]

## Metrics

### Per-session log (one line per session)

- 2026-09-16 | **S394** | iters: 1 pass (**I**) + 4 fixes = **5** (full budget) | tools: torah_trainer (`767064e`), js/ivrit-account (`a3d288d`), hebrew_blend_generator (`7de7842`), Hebrew_Font_Maker (`d14c4f9`) | patterns fixed: hover-feedback-survives-the-disabled-state ×1 (**its last open …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S394 (2026-09-16):** torah_trainer **2026-09-16 (`767064e`; I S394, N S389, M S345)**; js/ivrit-account **2026-09-16 (`a3d288d`; P S390, N S389, K S388)**; hebrew_blend_generator **2026-09-16 (`7de7842`, `12c170e`, `ec8bace`; G S392, A S385, **D S338 — D-next**)**; Hebrew_Font_Maker **2026-09-16 (`d14c4f9`, `d5ee54c`; H S380)**; flash_cards 2026-09-16 (`d285c88`; D S393, C S381, **G S279 — G-next**); index 2026-09-16 (`fa001b0`; M S391, D S352, C S340); scripts/smoke-account + account-test 2026-09-16 (`bfcdc15`); account 2026-09-16 (I …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`selector-declared-twice-in-one-stylesheet`** (**NEW, registered 2026-09-16 (S394) — 21 carriers censused, 5 fixed**): ACTIVE. One file declares the same selector twice in the same at-rule context with OVERLAPPING properties; the later wins, so an edit to the earlier one …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S395):** **BRANCH/PR: S394 CONTINUED `claude/loving-einstein-cdmbbz` — PR #246 was open, unmerged and draft at session start (read through the API, not assumed), so the skill's continue rule applied. 4 fix commits + this close-out on top of S391/S392/S393's 12. If #246 is STILL open and unmerged, continue on it; if merged, cut fresh off latest `origin/main` — never stack on merged history.** **/!\ DRIFT CHECK: NONE, a THIRD consecutive session — `origin/main` still `b304db0`, nothing on it this branch lacks, clone NOT shallow, keep-alive's last SCHEDULED run green. CHECK, never assume either way (S391's clone arrived SHALLOW and read 65 commits stale).** After S394: `sw.js` **v769**, FM **5.45**, migrations 0001-0003 all `Live? = yes`, SDK pin 2.116.0. Never trust these numbers unread.

**⚑ HARNESS. The `--sdk` fixture: `npm pack @supabase/supabase-js@2.116.0`, untar, pass `package/dist/umd/supabase.js` (**npm IS reachable; the CDNs are not** — the same trick serves html2canvas 1.4.1 + jspdf 2.5.1 for the generator's PDF export; pypi timed out where npm did not). smoke-account / smoke-saves / smoke-tools hardcode port 8080 — **kill your own server FIRST, by PID. Never `pkill -f` a server: it matches and kills this shell.** A backend-file change is verified by the matching smoke, never by the generic recipe.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), H (S380), C (S381), F (S383), B (S384), A (S385), E (S386), L (S387), K (S388), N (S389), P (S390), M (S391), G (S392), D (S393), I (S394).** Unattended: take **H** (teacher walkthrough) — 15 sessions stale and now the longest unattended gap — **on `classroom_dashboard.html`, whose last H was S46, the stalest tool pairing in the table by a wide margin.** Then C, F, B (B's next is the 3 pages I S394 added: `account.html` + the 2 harnesses), **A (its first sweep of the NEW `selector-declared-twice-in-one-stylesheet` row — 15 carriers already censused and classified, so run the classifier, do not re-derive it)**, E, L, K, N, P, M (next: a chrome page never M-audited — `account.html` has never had one), G (next: `flash_cards.html`, S279) and D (next: the **generator**, S338; trope/torah are S307).

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1-34 are S384-S392's, 35-40 S393's, 41-47 S394's — all of them in full in loop-findings, read them there first.** The S394 seven in one line each: **(41)** the account chip's handle is `[data-ivacct] .ivacct-btn`, never `#acctChip` — a guess read `found:false` on all 9 pages. **(42)** assert every functional receipt non-zero on a CLEAN profile before trusting it: `.tool-card`/`#profileSelect` matched nothing and turned 35 clean cells into false HITs (the hub is `.page-card`, flash cards `#activeProfileSelect`). **(43)** `.toggle input { opacity:0; width:0; height:0 }` is the suite's toggle idiom — `page.check()` times out on "not visible"; click its `label.toggle`. **(44)** a raw-i18n-key regex must require each segment to START WITH A LETTER, or `0.78rem`/`1.50rem` read as keys. **(45)** `g392/torah2.mjs` seeds `hebrewTorah_darkMode`; torah's key is `hebrewBlender_darkMode`, so both its cells ran LIGHT — assert `body.dark` per cell. **(46)** `IvritAccount.closeMenu` is NOT exported (only `openMenu`, returning true/false) — close by clicking the chip button. **(47)** the FM's `#engCaseUpper` is 0x0 on a first load even with every `.panel.collapsed` cleared; take the first VISIBLE `.mode-toggle > .mode-btn` after removing `.open` from `#wizardOverlay` — and for a focus ring, press a real Tab BEFORE `.focus()` and assert `matches(':focus-visible')`.

**⚑ TOP UNGATED CANDIDATES (S394): the 15 open carriers of the new `selector-declared-twice-in-one-stylesheet` row** — 8 in the generator (the bingo layer is a judgement call, the other four SHADOWED ones are not), 1 FM, 2 dashboard, 2 dictionary, 1 privacy, 1 terms; the classified census is in loop-findings, so the work is reading, not detecting. Then **the tour-card h4 → h2 ×7** (breaks the 2-tool cap — split it, or take it as an A sweep); **the sub-floor touch targets** (generator `.bingo-step`, FM `#rulerCorner` — both gate 3, a visible size change); **the FM chip's unmeasured phone geometry** (S394 shows the dismissal: remove `.open` from `#wizardOverlay`); **"sign out everywhere"** (new control + copy, so gate 1/2); **the Edge Function's production `localhost` origins** (backend boundary — maintainer redeploy). **`hover-feedback-survives-the-disabled-state` has NO open carrier for the first time since S383** — its next A sweep is a census, not a fix list, and the S388 note filing FM `.help-tab` as open is stale.

...
- [ ] P4 (**NEW S327 Pass C — accessible names; 8 CSV rows EN+HE, screen-reader only so not gate 2**) | hebrew_blend_generator.html | **The word-search "Directions" checkboxes are named only by an arrow glyph** (`.ws-dir`: ← → ↓ ↑ ↘ ↙ ↗ ↖ inside their `<label>`s), so a screen reader announces "leftwards arrow" for right-to-left. Add `aria-label` per input from new …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S327 (2026-09-03):** hebrew_blend_generator **2026-09-03 (×2, `d0bf245` Full Screen caption, `41679da` tile names; S326 ×1, S322 ×2)**; classroom_dashboard **2026-09-03 (×2, `45a5554` dvh twins, `b02d715` Date format row; S326 ×1, S325 ×2, S324 ×1, S323 ×2, S322 ×2, S321 ×2)**; Hebrew_Font_Maker 2026-09-03 (S326 ×2; outside-loop v5.39 `4638e7b`); torah_trainer + trope_tutor 2026-09-03 (S325 ×1 each, `280965f`; torah S324 ×2); index 2026-09-03 (S325 ×1, S324 ×1, S323 ×2); hebrew_dictionary 2026-09-03 (shared-block edit only, S325; S322 ×2); …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`sub-floor touch target`**: **re-swept 2026-09-03 (S327 Pass C, `hebrew_blend_generator.html`, pseudo-aware runtime census, 162 visible controls × EN/HE × 1280/800) — hits: 0 new; clean streak: 1 — ACTIVE.** Not hits: the FAQ/How-To `<summary>` rows at 19/21px on a 25/29px …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S328):** **BRANCH/PR: CONTINUE on `claude/improve-loop-k280wo` → draft PR #222 (S316–S327 commits + FM v5.39; verify via the API first — if it has merged, cut a fresh `claude/*` branch off latest `origin/main`).** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v651, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS F (2026-09-02, S313), THEN O (S315, attended only), B (S316), A (S317), E (S318), L (S319), K (S320).** **F's next affordance:** the collapsible-header hover idiom filed by S317 (`.panel-title` / `.sub-section-hdr` / `.pos-sec-hdr` / `.vowel-group-header` on 4 tools), or the tooltip-binding divergence filed by S285. **O's surface is flash_cards.html — attended only; unattended, skip O and take B.** B's delta since S316 is this branch's 40+ page commits (generator header/tiles, dashboard drawer rows, the FM v5.39 Send flow). Run `update-sitemap` at EVERY close-out.

**⚑ THE STRONGEST UNTAKEN CANDIDATES:** the picker section headers half-translated in HE (P4, K — 7 CSV rows + 9 sites, touches 6 tools ×1); the dictionary `--dict-sidebar-w` carrier (the pattern's last, blocked by the fixed toggle — reposition it first); the word-search direction checkboxes' arrow-only names (P4, generator, 8 CSV rows); the S317 sub-floor trio (FAQ summaries ×5 pages are spacing-exempt — decide once whether the house 30px bar applies; `.ed-btn`; FM `#unfinishedBtn`); the two S324 torah residuals; the S319/S323 gate-2 copy (attended). Seed bench: the scroll-style handout (torah, S324), the last-backup date (index, S323).

**/!\ THE S327 METHOD LESSONS.** (a) **A caption lift is a width change too:** the 8→11.2px "Full Screen" caption widened its button 68→74px, which is enough to wrap a flex header one line earlier at in-between widths (640/768) — measure header height across the whole width ladder, not just 1280/800. (b) **Measure the longest name in BOTH languages before picking a step:** EN "Tzadi sofit" fits a 54px tile at 0.7rem-minus, HE "שין שמאלית" is 5px wider at every size and decides the ceiling; the clone-and-`white-space:nowrap` width probe (loop-findings, generator) gives the exact px per size. (c) **Grids hidden behind a mode toggle are invisible to a census** — the lock grids live under `#focusOptions` (two-letter mode + Focus on) and the vowel locks under `#vowelLockOptions`; reveal them by their real toggles or the census reports "clean" on 39 tiles it never saw. (d) A non-modal tour (`aria-modal=false`, Tab leaves the card) is the documented design (CLAUDE.md "a tour is a read-only overlay") — not a focus-trap finding.

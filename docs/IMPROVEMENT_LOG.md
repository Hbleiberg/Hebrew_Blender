...
- [ ] P4 (**NEW S321 Pass N arm 3 — a STATIC reading, headless cannot measure the URL-bar viewport**) | classroom_dashboard.html | **`.fr-modal { max-height: 88vh }` and `.swm-modal { max-height: 90vh }` are centred `position:fixed` boxes sized in `vh`, which on a phone with the URL bar expanded is taller than the visible viewport the box is centred in** (iPhone 13: 90vh ≈ 675 vs 664 visible → …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-03 | (S321 close-out) | branch/deploy note | **Branch/PR: CONTINUED `claude/improve-loop-k280wo` → draft PR #222** (API at start: open, draft, `mergeable_state:clean`, head `7cffb51`, 0 check runs — correct, no workflows). **Zero outside-loop drift:** `origin/main` still `735b451`; `sw.js` v643 and `FONT_MAKER_VERSION` 5.38 re-read from the files. **sw v643→v644** (3 precached pages …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-03 | `09f6220` | Hebrew_Font_Maker.html | **Primary buttons get a hover colour** — the S318 candidate said a hovered `.ctl-btn.primary` paints warm-gray under white; MEASURED with a real `page.hover`: hover = rest (navy/white) in light, because `.ctl-btn:hover` and `.ctl-btn.primary` tie at (0,2,0) and the primary rule comes later — the candidate counted `:hover` as a …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-03 | `35bc4d6` | classroom_dashboard.html | **The board-text edit pencil shows on touch screens** — `.dash-edit-pencil` was `opacity:0` until `#dashTextWrap:hover`, which never fires under `(hover: none)`, so on a phone/tablet in-place editing had no visible way in (the drawer's text box still worked). A `@media (hover: none)` rule shows it at rest; mouse behaviour unchanged (S321 …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-03 | `6aa37ec` | classroom_dashboard.html | **The fullscreen quick strip fits a phone** — the six-button `#fsStrip` is ~435px wide, so at 320–430px its end buttons hung off both edges (at 390px "Settings" and "Blank" measured 11px off-screen each side, unreachable; at 412px too). Inside the page's existing `@media (max-width: 430px)` the strip fills the width as two rows of three (a …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S321 (2026-09-03):** classroom_dashboard **2026-09-03 (×2, `6aa37ec` phone fullscreen strip, `35bc4d6` touch pencil; S320 `ad7e5a8`)**; Hebrew_Font_Maker **2026-09-03 (×1, `09f6220` primary hover; S320 `fffe886`)**; flash_cards **2026-09-03 (×1, `c2dea7e` member filter; S319 `9f57093`)**; trope_tutor 2026-09-03 (S320 `d57326a`); hebrew_dictionary 2026-09-03 (S319 `16cb1c0`); hebrew_blend_generator 2026-09-03 (S319 `b0f1583`); all 13 pages via `js/i18n.js` 2026-09-02 (S317 `b13c1e5`); index, resources, contact, privacy, terms, 404 …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`non-finite-number-from-a-loaded-file`**: ACTIVE (consequence-critical: saved work). **Last swept 2026-09-02 (S317 Pass A, runtime, no-poison control per tool): settings arm clean on all 7 tools; the one survivor (flash cards `vowelPositions.0`) FIXED S319 `9f57093` — hits …[full text: IMPROVEMENT_ARCHIVE.md]
...
- N mobile & touch-device (one surface): 2026-09-03 (**S321 — 12th-ever N, `classroom_dashboard.html`, its 2nd dashboard N (S247 → S321, 74 sessions) and the pointer's named target: the outside-loop panel-width lock `735b451` + the S315 drawer had never been phone-audited. Real descriptors (iPhone SE 320, iPhone 13 390, Pixel 7 412, iPhone 13 landscape; `pointer:coarse`/`hover:none` confirmed on …[full text: IMPROVEMENT_ARCHIVE.md]
...
- L SEO & discoverability audit: 2026-09-03 (**S319 — 14th run, its first since S301 (~18 sessions); L was the single stalest by BOTH date and session number and the S319 pointer named it AND its arms. Ran the S301 shape, and for the FIRST TIME SINCE S274 THE BYTE-CHECK REPORTED REAL MOVEMENT: `4bedf00..HEAD` (92 commits / 41 files / +13,505 −4,881), 13/14 files moved and **7/14 SEO surfaces …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S322):** **BRANCH/PR: CONTINUE on `claude/improve-loop-k280wo` → draft PR #222 (S316–S321 commits; verify via the API first — if it has merged, cut a fresh `claude/*` branch off latest `origin/main`).** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v644, `FONT_MAKER_VERSION` 5.38** — re-read both.

**⚑ THE STALEST PASS IS M (2026-08-31, S304), THEN H (S305), G (S306), D (S307), I (S308).** M's coverage is complete (all 7 tools + 6 chrome pages), so per the S304 row re-audit a CHANGED surface: `classroom_dashboard.html` (the outside-loop panel-width lock, the S315 drawer, and this session's phone strip — never M-audited since S248) or the Font Maker's footer fold (S315/S316 outside-loop, S317 `fb34b61` hover only). BEFORE/AFTER light+dark at the affected breakpoints, saved in the scratchpad, delivered at close-out; anything dramatic is gate 3. **O's next target is flash_cards.html — attended only.** Run `update-sitemap` at EVERY close-out.

**⚑ THE STRONGEST UNTAKEN CANDIDATES:** the S319 gate-2 Hebrew-interface claim (needs an attended session); the dashboard "Full Screen" 8px label (P3); the two dashboard `vh`→`dvh` modal twins (P4, one line each — the dashboard is at cap this session, so it is the FIRST dashboard item next time); the flash cards Colors-mode tile labels in raw English (a gate); the font picker attribution line at 8.64px across four carriers (P4, converge together). `apply-settings-trusts-collection-members` has NO open carriers — its next A sweep is a re-check, not a burndown.

**/!\ THE S321 METHOD LESSONS.** (a) **Collect tap coordinates ONE target per `evaluate`:** three `scrollIntoView`s inside one evaluate left every rect but the last stale, and the pass reported three sliders 'did not move on finger tap' — a real `touchscreen.tap` control on one slider moved it 0.8 → 1.6, by touch and by mouse alike. A zero from a batched-coordinate probe is the probe's. (b) **Measure a specificity candidate before filing it:** `:hover` is a pseudo-CLASS, (0,1,0) — the S318 reading gave it (0,0,1) and predicted warm-gray under white; the real state was no hover at all. (c) **A phone-only fix stays in a device or width media query** (`(hover: none)`, the page's existing 430px block) — both shipped ungated because desktop bytes were proven identical (435px strip, pencil rest 0 → hover 0.9).

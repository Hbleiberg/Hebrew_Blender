...
- [ ] P4 (**NEW S403 Pass E arm 17**) | docs/reference/ops.md | **"Everywhere: … `data:` / `blob:` in `img-src`" is false for `account.html`, whose `img-src` carries neither** (it shows no images; its zip download is an `<a download>`, not an image load). The page is the TIGHTER one and the sentence is the error — worded as it is, it invites "fixing" the CSP looser. One clause names the …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P4 (**NEW S403 Pass E arm 13**) | README.md | **The `fonts/` row lists Frank Ruhl Libre, Lakhish Bold, Reuben and the two TzviScripts but not `NotoSerifHebrew-Taamim.ttf`**, the precached te'amim fallback every Hebrew stack ends with (added 2026-09-09, `f2384a7`). | found S403

- [ ] P4 (**NEW S403 Pass E arm 20 — K's to fix, notes are translator-facing**) | locales/ui-strings.csv | **122 rows keep a `leading-emoji` note although their `en` value no longer leads with a glyph** (the icon sweep re-noted 74 others `svg-icon-in-markup`); a translator adding a third language would put the emoji back beside the SVG. Mechanical, but each row's markup must be read to pick …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-18 | (S402 close-out) | branch/deploy note | **S402 was a feature-seeds session, not a pass — the maintainer asked for all three generator seeds ("Build all the blend generator seeds"): S + S + M (dual), one commit each (`88b2720` `7345060` `58fc2cd`), on `claude/improveloop-feature-seeds-0tx529` cut FRESH off `origin/main` `e5373e4` → draft PR #252; a human merges and confirms the …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-18 | `58fc2cd` | hebrew_blend_generator.html + CSV + sw.js + 3 references | **"⭑ Save as Word List" beside the pasted-list import — shipped (S402, seed S367, M dual).** `saveImportedAsWordList()`: the textarea parsed (no prior Import click), the Dictionary's word shape / id shape / caps (50 lists, 200 words, deduped by `word`), written as a NEW list into `ivritSuite_wordLists` (this …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-18 | `7345060` | hebrew_blend_generator.html + CSV + reference | **"Name / Date line" toggle in the Layout panel — shipped (S402, seed S367).** `wsMetaHTML()` gets one gate (`#nameDateToggle`, default on, read live so the chunked Class Set thunks follow it); `toggleNameDate()` re-renders in place through `rerenderCurrentSheet()` (routes number practice and the Real Words drills too …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-18 | `88b2720` | hebrew_blend_generator.html + CSV + reference | **"N pages to print" under Print / Export PDF — shipped (S402, seed S367).** `updatePrintCount()` counts the units `exportPDF` captures one per page (`.sheet`, `.bingo-page`, `.caller-sheet`) through ONE `MutationObserver` on `#worksheet`'s child list (every page unit is a direct child), coalesced to a frame — **the …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-18 | (S401 close-out) | branch/deploy note | **S401 was a feature-seeds session, not a pass — the maintainer asked for "only the S seeds for the dashboard": two `S (dual)` seeds shipped, one commit each (`92217cf` `7d7b89a`), on a FRESH branch `claude/improveloop-feature-seeds-mwdbk4` cut off `origin/main` `5f1941d` (PR #250 read through the API: merged 2026-09-18) → draft PR #251; …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-18 | `7d7b89a` | classroom_dashboard.html + CSV + sw.js + reference | **"+1 min" on the timer — shipped (S401, seed S367).** `timerAddMinute()` adds 60 s to `timerRemaining` ONLY (Reset returns to the preset), keeps a running clock running, resumes a just-finished one, leaves a paused/idle one stopped; the "Time!" hold is now `timerFinishTimeout`, cleared by …[full text: …[full …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-18 | `92217cf` | classroom_dashboard.html + CSV | **"Everyone's here" clears the class's absence marks in one tap — shipped (S401, seed S367).** `clearAbsences()` beside `resetPickerCycle()`; one `.btn-xs` in `#pickerProgress` while `absent.length`, fair cycle on or off, gone when all-present; active class only, `picked` untouched, no `confirm()`, no new state. Seed premise …[full …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-17 | (S400 close-out) | branch/deploy note | **S400 was a feature-seeds session, not a pass: three Trope Tutor seeds pushed STRAIGHT TO MAIN (`4ce56bb` `3249dba` `c57eb53` + `ee2d5f4` llms-full regen), each Pages run green — run 1238 for `ee2d5f4` is the deploy; `sw.js` v776→v777; this close-out was written afterwards (2026-09-18) on `claude/improveloop-feature-seeds-3tczsw` / draft …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-17 | `ab491a7` | Hebrew_Font_Maker.html + CSV | **A blocked image builder stops being reported as a PDF problem** (S399, maintainer gate 2). Template sheets download as PNG or PDF; both refusals showed the PDF message, so a teacher behind a cdnjs filter asking for PNG was sent after the wrong library. The guard is unchanged — only the message it picks. New key …[full text: …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S403 (2026-09-22):** all-page passes keep the *dedicated pass* the useful column. index (**S403: `7552ba0` `6e28af3`**; M S391, F S397); hebrew_blend_generator (S402 seeds; G S392, **D S338 — D-next**); flash_cards (D S393, **H S264 — the stalest H tool**, G S279); hebrew_dictionary (**C S285 — C-next**); classroom_dashboard (S401 seeds; H S395); torah_trainer (**O-next, never O-audited**; N S248); trope_tutor (S400 seeds; N S389, C S312); Hebrew_Font_Maker (**S403: `6e28af3`**; B S398, H S380; 5.47–5.56 landed outside the loop, no …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S404):** **BRANCH/PR: S403 = pass E + 4 fixes (`7552ba0` `6e28af3` `7820327` `9dfc1e1`) + its close-out on `claude/eager-bardeen-jdve4x` → draft PR #265, cut FRESH off `origin/main` `041ddfa`. If #265 is open and unmerged, continue on it; if merged, cut a FRESH branch off latest `origin/main` and confirm that merge's Pages run concluded `success`.** **DRIFT vs S402 was the largest outside-loop landing since the backend** (54 commits: the emoji→SVG icon sweep, the dashboard timer's sounds, FM 5.47→5.56 with five glyph-set tabs) — S403's E caught the sweep's leftovers; **the FM 5.47–5.56 surface (+2,225 lines) has had NO dedicated pass**. S403 closed at `sw.js` **v823**, FM **5.56**, SDK **2.116.0**; backend re-read LIVE in S403 (3 recorded migrations = 0001–0003, `delete-account` v3 `verify_jwt` on, advisors = the known leaked-password WARN only). **Never trust these numbers unread.**

**⚑ STALEST PASS: O (S346 — BLOCKED here, S399 + S403), L (S387), K (S388), N (S389), P (S390), M (S391), G (S392), D (S393), I (S394), H (S395), C (S396), F (S397), B (S398), A (S399), E (S403).** Take **L** unless the maintainer says the O permission rule exists (then test ONE `node <detector> torah_trainer.html` first and say the answer in the question). **K** is next and has a concrete target: 122 `leading-emoji` notes whose `en` no longer leads with a glyph. For a surface pass, the **Font Maker's five new glyph-set tabs** are the largest unaudited surface (B, I or C on `Hebrew_Font_Maker.html`).

**⚑ THE ICON SWEEP LEFT TWO SHAPES — both now registered patterns with detection definitions in Pattern health.** `icon-markup-into-a-textContent-writer` (a builder helper printing SVG source; S403 found it on the hub's My Fonts rows, a real-click-only surface no load census sees) and `per-page-code-inside-a-shared-block` (both sweep-era blocks). The sweep's own pre-push check proved each SVG snippet identical and non-empty (S403 re-trued it: 368 snippets, 49 glyphs, 0 diverged, 0 empty) — it never looked at the blocks around the snippets or at the helpers they are passed to. **A's next run owns both.**

**⚑ RUNTIME-PROBE HARNESS (standing). Traps 1–70 S384–S399 in loop-findings; S403 adds 71–73 there** — **(71)** a runtime CSP-violation census is blind to SECOND-ORDER loads: route-aborting the Google Fonts stylesheet or `gtag.js` means `fonts.gstatic.com` and the analytics beacons are never attempted, so 0 violations says nothing about `font-src` or their `connect-src` — the static directive-claims arm is what covers them. **(72)** a provenance adjective about a vendored copy is a claim to measure: S403's own draft called the QR library "unmodified"; it is minified (20,434 B vs 56,694 B). **(73)** a list-builder's icon handling is invisible until a row exists: seed the store (one IndexedDB record) and open the dialog by its real button. **Also standing: `compact-ledger.mjs --apply` is NOT idempotent — run it ONCE, after writing the new pointer; the outgoing handoff is moved by hand.**

**⚑ TOP UNGATED CANDIDATES: the four S403 P4s** (ops.md's `account.html` img-src clause and the README fonts row are one-line doc fixes; the CSV notes are K's; the My Fonts 24px icon buttons are C's), then the older bench: the flash/FM/trope heading-level trio (a structure call), account's `#holdsList` (needs WebKit). **Gate bench unchanged from S402** (no gate asked in S403): gate 2 — the generator's Header Labels translating only Name/Date; gate 3 — the FM stage annotations, account's dark `.btn.ghost`/`.btn.danger`, `.dash-edit-pencil`'s side in Hebrew, the hub's 2.4px type band; gate 4 — the flash-card sticky bar + landscape. **Seed bench: 12**, unchanged (dashboard's week-editor undo is the strongest).

...
- [ ] P2 (**NEW S337 Pass G — the `uncompressed-jspdf-raster` pattern's open carrier; the Font Maker's two sites closed `e0caf12`**) | hebrew_blend_generator.html | **The flash-card sheet PDF's `pdf.addImage(imgData, 'PNG', …)` passes no compression argument, so jsPDF stores each page raster raw** — the same call shape that made the Font Maker's template sheets 11 MB per page and its specimen 46 …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P3 (**NEW S337 Pass G — the S293 loop-findings note, now confirmed on the real artifact**) | Hebrew_Font_Maker.html | **The exported UFO's `fontinfo.plist` carries no `openTypeOS2Type`, so the standard toolchain compiles it to fsType 4 (Restricted License embedding) while the TTF export writes fsType 0.** S337 compiled the tool's own `.ufo.zip` with ufo2ft 3.9.0: `fsType compiled 4, export …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [ ] P4 (**NEW S337 Pass G — a C-pass shape, seen once in three `?start=` loads**) | Hebrew_Font_Maker.html | **On the partner wizard's step 1 (no inputs on that step) the Next button once carried `aria-disabled="true"` while painting enabled**, so a screen reader hears "dimmed" and Playwright refused the click for 30 s (`g337/out/ezra-sil-sr/wiz1.png`, first run). The two later loads …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S337 (2026-09-03):** Hebrew_Font_Maker **2026-09-03 (×2, `9ae3495` UFO exporter, `e0caf12` PDF Flate; S336 ×1; outside-loop v5.39)**; torah_trainer **2026-09-03 (×2, `36f2a5a` chapter max, `fa4dce9` holiday name; S336 ×2)**; classroom_dashboard 2026-09-03 (S336 ×1, `009611b`); locales/ui-strings.csv 2026-09-03 (S334 `fa4dce9`); flash_cards + hebrew_blend_generator + trope_tutor 2026-09-03 (S333 ×2 each); hebrew_dictionary 2026-09-03 (S333 ×1); resources 2026-09-03 (S333 ×1); contact/privacy/terms 2026-09-03 (S332 ×1 each); …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`uncompressed-jspdf-raster`**: ACTIVE (registered S337 Pass G). **S337: 2 of 3 sites FIXED `e0caf12` (the Font Maker's specimen + template `addImage` calls); the generator's card-sheet call is the open carrier (P2 candidate).** Definition: a jsPDF `addImage(...)` call with …[full text: IMPROVEMENT_ARCHIVE.md]
...
- G print & export fidelity (one tool): 2026-09-03 (**S337 — `Hebrew_Font_Maker.html`, its 2nd dedicated G (S169 → S337, ~168 sessions, v4.x → v5.39 between: the `.ttf`/`.otf`/UFO/WOFF2 exporters, the specimen PDF, the mark editor, the Starting Fonts intake). G was the stalest unattended pass (O attended-only) and the S337 pointer named it AND the target. THE FIRST G TO RUN THE REAL EXPORT …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S338):** **BRANCH/PR: S337 CONTINUED `claude/improve-loop-0mbq58` → draft PR #223 (head after this close-out; base `origin/main` `4b66a64`). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v661, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS O (S315, attended only), THEN D (S325), I (S326), C (S327), F (S328), B (S329), A (S330), E (S331), L (S332), K (S333), N (S334), M (S335), H (S336), G (S337).** **Unattended: skip O and take D** (performance snapshot, one tool: cold load + one heavy interaction, main-thread blocks >200 ms with profile evidence). D's per-tool history must be RE-DERIVED from the ARCHIVE (the rotation-row shape changed over time; the `Pass D` grep under-reads): the S325 row records the dashboard's D lineage S14 → S117 → S209 → S325, torah S307, and the older grep shape gives trope_tutor S95, hebrew_dictionary S105, generator S128, flash_cards S170, Hebrew_Font_Maker S294 — **`trope_tutor.html` is the probable pick; confirm before running.** O's surface is still flash_cards.html. **A harness note that changes what G/D/B can do:** PyPI, the npm registry and GitHub release assets are reachable through the proxy (`pip install --target`, `npm pack`, `curl | tar --wildcards`), so any CDN library a page lazy-loads can be mirrored and served at its exact URL with `page.route` — recipe in `loop-findings.md` (Font Maker section) and `ops.md`. **Ledger lesson:** an open candidate whose text contains "logged not fixed" / "measured, not …" (see `NON_ACTIONABLE` in `ledger-rules.mjs`) is filed by `compact-ledger` as a record — the two S336 torah candidates vanished from `## Candidates` that way and were fixed from the archive's copy; write open candidates without those phrases.

**⚑ THE STRONGEST UNTAKEN CANDIDATES:** the generator's raw-raster `addImage` (P2, the `uncompressed-jspdf-raster` open carrier — one argument pair, verify with pymupdf); the UFO `openTypeOS2Type: []` one-liner (P3, now measured live); the four unauthored picker terms (gate 2); the staging author URL on the hub (gate 2); the real-words 7.2px grid (layout decision); the 38×22 `.toggle` on torah + trope (C); the dashboard `.tip-icon`-vs-`.tip-wrap` divergence (F); the help tab strip (gate 3); the S321 timer placeholder copy (gate 2); the specimen-A4-vs-template-Letter paper choice (gate 2). **For the next A sweep:** add the `uncompressed-jspdf-raster` grep; `blocking-alert-for-a-routine-path` still wants its dashboard click census (10 static `alert(` left); the S335 runtime arm for `background:#fff` / `color:#1a2744` on the 8 other pages still stands. **Seed bench (unchanged):** the holiday share link (torah, S336 — ~15 lines, the smallest seed), the handout selection (torah, S336), the scroll-style handout (torah, S324), the last-backup date (index, S323).

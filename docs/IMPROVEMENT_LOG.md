...
- [x] 2026-09-11 | (S373 close-out) | branch/deploy note | **S373 CONTINUED `claude/improve-loop-swgwqx` (PR #229 open, unmerged; base `66dd97b` = `origin/main`), 4 commits + this close-out; a human merges and confirms the Pages run. Drift: none (`66dd97b`, sw v697, FM 5.39 — as the S372 close-out recorded). sw v697→v698 (dashboard, Font Maker, trope, flash); FM NOT bumped (a hit-box …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-11 | `d5285f6` | trope_tutor.html | **The settings drawer + backdrop join the print hide list** (`fixed-surface-missing-from-the-print-hide-list`, the S371 twin divergence; one selector pair) | `l373/tropeprint.mjs`: drawer opened by `openSettings()`, `emulateMedia('print')` → display none/none in 4 cells, screen control flex/block, the torah twin measured identical (8 cells); 0 …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S373 (2026-09-11):** Hebrew_Font_Maker **2026-09-11 (`9257fb8`; C-audited S368)**; classroom_dashboard **2026-09-11 (`c365786`; G-audited S364)**; trope_tutor **2026-09-11 (`d5285f6`; H-audited S367)**; flash_cards **2026-09-11 (`5fd4bc1`; N-audited S361)**; hebrew_blend_generator 2026-09-10 (`e4c8d75` S371; H-audited S363); hebrew_dictionary 2026-09-10 (`e5dac20` S371; M-audited S362); index 2026-09-10 (`edf4b2e` S370); torah_trainer 2026-09-10 (`869ee1c` S369); resources 2026-09-10 (`ce8c9d5` S369); contact / privacy / terms / 404 …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`fixed-surface-missing-from-the-print-hide-list`**: ACTIVE. **S373: the S371 twin divergence FIXED (`d5285f6`, trope `.settings-modal, .settings-backdrop`). Registered S364; hits 3 (S364 ×2 `eed4c03`, S371 ×1), clean streak 0; last swept S371.** Detection: every …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S374):** **BRANCH/PR: S369–S373 ran on `claude/improve-loop-swgwqx` (cut at `66dd97b` = `origin/main`; 25 commits) → draft PR #229. If it is still OPEN and unmerged, continue on it; if MERGED, cut a fresh `claude/*` off latest `origin/main` — never stack on merged history. Verify via the API** (0 check runs is correct — no workflows). **⚑ DRIFT CHECK: compare `origin/main` against `66dd97b`; anything beyond THIS loop's 25 commits is an outside-loop landing. `sw.js` v698, `FONT_MAKER_VERSION` 5.39** — re-read both. A fresh container starts shallow again — `git fetch --unshallow` before `update-sitemap.mjs`.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), K (S360), N (S361), M (S362), G (S364), D (S365), I (S366), H (S367), C (S368), F (S369), B (S370), A (S371), E (S372), L (S373).** Unattended: take K (i18n audit) over `b418cf9..HEAD` — the 4 gates first (Check A stays clean; Check B's backlog is the burndown, prefer sites whose CSV keys exist), then the blind-spot probe: English in template literals or plain arguments in the delta (the S370 FM status key, the S371 generator subtitle keys; the S373 commits are CSS-only), and grep every authored key literally (the S365 trope leak hid behind a `'trope.learn.' + x` prefix); the 5 credit rows stay wire-or-prune. Then N over resources (S246). **O next `torah_trainer.html`; M next trope_tutor (S245); D next dictionary (S253); G next dictionary (S252); H next Font Maker (S241); C next flash_cards (S271); F next the disabled-control look; the next A sweeps `1d4c9cf..HEAD` with the range-setter arm on the Font Maker's 13 sliders + torah/trope; the next I re-runs `i366/{gates,arms,corrupt,strshape}.mjs`; the next E re-runs the 17 arms over `408dbe4..HEAD`; the next L re-runs `l373/{audit,bytecheck}.mjs` vs `5fd4bc1` (count FAQ twins by `faq[._]q`-keyed summaries; scan `<img>` in the script-stripped BODY — whole-file regexes hit script comments).**
...
**⚑ TOP UNGATED CANDIDATES (S373):** torah/trope `Number.isFinite` at the restore sites (`parseFloat(x) || d` passes Infinity — 8 torah sites, 2 trope; hand-edit only); the per-page sub-11px batches (resources ×45, dictionary ×21, generator, flash, privacy — `f369/census-en.txt`); the ~30 inline Font Maker hints under 11px; the tour-card h4 → h2 ×7; the remaining sub-floor M calls. Gate 2: the generator's two-subtitle slot (documented in generator.md); the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on; the trope FAQ a7 copy. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. **Seed bench: 25 (unchanged; L is not an intake). Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

...
- [x] 2026-09-10 | `df7f98e` | Hebrew_Font_Maker.html | **A project file with wrong-shape members loads or is refused with "Not a valid project file." — `migrateProject` reduces every collection (letters, contours+curves as a pair, sheets, precomposed, guides, custom fonts, the 3 mark stores + their custom maps, the metrics box) to plain-object members; `applyProjectData` refuses a non-object …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S370 (2026-09-10):** Hebrew_Font_Maker **2026-09-10 (`df7f98e`; B-probed S370, C-audited S368)**; hebrew_blend_generator **2026-09-10 (`70f62c0`, `edf4b2e`; H-audited S363)**; trope_tutor 2026-09-10 (`869ee1c`; H-audited S367); classroom_dashboard **2026-09-10 (`edf4b2e`; G-audited S364)**; hebrew_dictionary **2026-09-10 (`edf4b2e`; M-audited S362)**; index **2026-09-10 (`edf4b2e`; S360)**; contact / privacy / terms / 404 2026-09-09 (S360; F-censused S369); flash_cards **2026-09-10 (`966179f`, `edf4b2e`; N-audited S361)**; resources …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`stored-json-of-the-wrong-shape-trusted`**: ACTIVE. **S370 (Pass B arm 4): the Font Maker's project-file load FIXED (`df7f98e`; 16 of 54 wrong-shape files threw uncaught, 5 past the `project =` line → 0) — the last carrier S356 named; 6 carriers closed lifetime, clean streak …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`apply-settings-trusts-collection-members`**: ACTIVE (consequence-critical: garbage is applied AND SAVED — never retires). **S370 (Pass B arm 5): a RANGE-SLIDER SETTER shape FIXED — the generator's `setHebFontSize` ran `Number(val)` on whatever the slider had refused …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`sub-floor touch target`**: ACTIVE. **S369: the `.ctl-btn` pair FIXED (`7e5905f`, `min-height:24px` on the class; 6 chips 22→24, the rest unchanged). S368 (Pass C, Font Maker census): 2 NEW carriers logged (`.ctl-btn` 41×22 / 37×22 on the Spacing tab — cap-blocked), …[full …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S371):** **BRANCH/PR: S369 + S370 ran on `claude/improve-loop-swgwqx` (cut at `66dd97b` = `origin/main`; 10 commits) → draft PR #229. If it is still OPEN and unmerged, continue on it; if MERGED, cut a fresh `claude/*` off latest `origin/main` — never stack on merged history. Verify via the API** (0 check runs is correct — no workflows). **⚑ DRIFT CHECK: compare `origin/main` against `66dd97b`; anything beyond THIS loop's 10 commits is an outside-loop landing. `sw.js` v695, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), A (S357), E (S358), L (S359), K (S360), N (S361), M (S362), G (S364), D (S365), I (S366), H (S367), C (S368), F (S369), B (S370).** Unattended: take A (recurring-pattern sweep) over the S357–S370 delta — the arms the S369 handoff named (print-hide-list, wrong-shape incl. the FM `userFonts` blob, `wired-then-clobbered label`, the sub-floor arm pseudo-aware) plus the two S370 shapes (a setter doing arithmetic on a refused value; a collection member trusted past a `project =` line); then E. **O next `torah_trainer.html`; D next dictionary (S253); G next dictionary (S252); H next Font Maker (S241); N next resources (S246); M next trope_tutor (S245); C next flash_cards (S271); F next the disabled-control look; the next I re-runs `i366/{gates,arms,corrupt,strshape}.mjs` and weights the S366–S370 fixes.**

**⚑ Harness (standing): torah stubs Sefaria by fulfilling `**/*sefaria.org/**` (`b370/harness.mjs` `SEF`: calendars → `calendar_items`, v3 texts → `{sections,versions:[{language:'he',text:[[…]]}]}`); hidden toggle inputs → click `label.toggle:has(#id)`; the HTTP server dies across a compaction — curl it first; paper/PDF notes: the S368 handoff in the archive; hover = a REAL `page.mouse.move` + 400 ms, focus = a real Tab; open a probe's host by ITS path (dashboard: `frSkip()` BEFORE any header click, `openWeekEditor()`, `openSettings()`; the generator's `livePreviewToggle` is in the collapsed Advanced panel, presets CARRY `livePreview`, `applySettings` rebuilds `#letterGrid`; the generator's sidebar is a drawer at 390 — its rows measure 0×0 until opened; FM: enter through the wizard (`#wizName`/`#wizAuthor` → `#wizNext` ×2 → `wizardCreate()`), `#exportFormat` offers ttf/woff2/ttfwoff2/ufo, a project file = `JSON.stringify(project)` + `format:'hebrew-font-maker-project'` through `page.setInputFiles('#projFile', {buffer})`, the Spacing panel = `setCatTab('spacing')` + drop `.collapsed` on `.cat-panel[data-cat="spacing"]`; the dictionary's collapsed bodies need a REAL click on Expand-all-menus; flash: the practice screen is `#screenCard.active`); an `elementFromPoint` receipt precedes a real click; a nowrap width clone must copy `getComputedStyle(n).font`, never `font:inherit` (S370 measured 84px for a 48px name that way); probe screenshot paths are cwd-relative.**

**⚑ TOP UNGATED CANDIDATES (S370):** the flash "Listening" badge space (1 line); the per-page sub-11px batches (resources ×45, dictionary ×21, generator, flash, privacy — `f369/census-en.txt`); the ~30 inline Font Maker hints under 11px (one batch); the tour-card h4 → h2 ×7 (not small); the remaining sub-floor M calls (the generator's 33 `scale(0.75)` sub-option toggles, gate 3; the bingo ▲/▼ stepper; FM `#rulerCorner`). Gate 2: the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on (`g364/out/board-print-en-dark.png`); the trope FAQ a7 copy. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. The next K greps every authored key literally. **Seed bench: 25 (unchanged; B is not an intake). Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

...
- [ ] P2 (**NEW S340 Pass C — the pass's headline; an a11y blocker on the suite's master backup**) | index.html | **The hub's `.ivrit` Merge/Replace dialog cannot be reached, operated or dismissed from the keyboard:** `ivritAskMode` on the hub never took the shared engine's accessible-dialog clause (the generator, dashboard and flash cards copies carry `role=dialog`, focus on Merge, a Tab trap, Escape → cancel, focus restore). Measured: focus stays on Browse…, 0 of 12 Tabs enter the dialog, and Escape closes the AllTools modal BEHIND it while the overlay stays up — a keyboard user is stranded until a mouse clicks the backdrop. Port the clause (+ stop the handled keys before the hub's document-level modal handler sees them). | found S340
- [ ] P3 (**NEW S340 Pass C — WCAG 4.1.3 status message**) | index.html | **The bookmark tip ("Press Ctrl+D to bookmark this page") is set by `showBookmarkTip` into a plain div with no live region**, so a screen-reader user who activates 🔖 Bookmark gets no response at all; `#ivritStatus` on the same page already uses `role="status"`. One attribute. | found S340
- [ ] P4 (**NEW S340 Pass C — `sub-floor touch target`, the only sub-floor control on the hub in 8 cells + 2 phone cells**) | index.html | **The Resources card's "🔤 Hebrew Fonts →" `.card-attr` link paints 141×23 (12.8px text + 4px padding-block, `display:block` as a flex item)**, one pixel under the 24px WCAG 2.5.8 floor; its Sefaria sibling is 36px. `min-block-size:24px` on `.card-attr` lifts it without touching the image pill. | found S340
...
- **S339 (2026-09-04):** trope_tutor **2026-09-04 (×1, `fea54f5` applyI18n write; S338 ×1)**; flash_cards **2026-09-04 (×1, `f001815` Start note; S333 ×2)**; hebrew_blend_generator 2026-09-04 (S338 ×1, `5b55d19`); Hebrew_Font_Maker 2026-09-04 (S338 ×1, `fc73fd9`; outside-loop v5.39); torah_trainer 2026-09-04 (S338 ×1, `f92ae1d`); classroom_dashboard 2026-09-03 (S336 ×1, `009611b`); locales/ui-strings.csv 2026-09-03 (S334 `fa4dce9`); hebrew_dictionary 2026-09-03 (S333 ×1); resources 2026-09-03 (S333 `43983b5`); index 2026-09-03 (S323 ×2); …[full text: IMPROVEMENT_ARCHIVE.md]

### Pattern health (per recurring pattern: last swept, hits that sweep, consecutive clean sweeps; detail in the sweep log below)

- **`stale-validation-note-after-its-input-changes`**: ACTIVE (**NEW, registered 2026-09-04 (S339 Pass I) — 2 known carriers, both fixed: the generator's `toggleLetter` / vowel clear (earlier, `4515`-shape) and flash cards' Start note `f001815`**). Detection: every …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`sub-floor touch target`**: **1 carrier FIXED 2026-09-04 (S338, the S334 Pass N filing): the shared `.toggle` switch 38×22 → 38×24 on its only two carriers, torah_trainer + trope_tutor (`f92ae1d`).** **1 new carrier found 2026-09-03 (S334 Pass N census on …[full text: …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S340):** **BRANCH/PR: S339 CONTINUED `claude/improve-loop-0mbq58` → draft PR #223 (head after this close-out; base `origin/main` `4b66a64`). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v663, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS O (S315, attended only), THEN C (S327), F (S328), B (S329), A (S330), E (S331), L (S332), K (S333), N (S334), M (S335), H (S336), G (S337), D (S338), I (S339).** **Unattended: skip O and take C** (accessibility, one tool — re-derive the per-tool C lineage from the archive with `grep -n "^- C accessibility" docs/IMPROVEMENT_ARCHIVE.md`; the generator (S327) is the freshest, so it is NOT the target). O's surface is still flash_cards.html. **Harness notes (standing):** CSP forbids string predicates — pass functions to `waitForFunction`; the raw-i18n-key detector must skip SCRIPT/STYLE text nodes or it reads source comments; never run the FM wizard arm beside another harness process (the S337/S339 stall face is a lost input event under contention); Playwright refuses to click an `aria-disabled="true"` target — that refusal on a primary action the teacher has just fixed is a FINDING (the S339 flash-cards defect), not a harness bug. **Ledger lesson (standing):** write open candidates without the `NON_ACTIONABLE` phrases or `compact-ledger` files them.

**⚑ THE STRONGEST UNTAKEN CANDIDATES:** all gated or decisions — the four unauthored picker terms (gate 2); the staging author URL on the hub (gate 2); the specimen-A4-vs-template-Letter paper choice (gate 2); the real-words 7.2px grid (layout decision); the help tab strip (gate 3); the S321 timer placeholder copy (gate 2). **For the next A sweep:** `stale-validation-note-after-its-input-changes` wants its first suite-wide runtime sweep (preset-name notes on the generator / flash cards, the dashboard timer note, the torah custom picker, the FM wizard step 2); `uncompressed-jspdf-raster` starts its retirement streak (`grep -n "addImage(" *.html`: 3 sites, all `'FAST'`); `blocking-alert-for-a-routine-path` still wants its dashboard click census; the S335 runtime arm for `background:#fff` / `color:#1a2744` on the 8 other pages still stands. **Seed bench (unchanged):** the holiday share link (torah, S336 — ~15 lines, the smallest seed), the handout selection (torah, S336), the scroll-style handout (torah, S324), the last-backup date (index, S323).

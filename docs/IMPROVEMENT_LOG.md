...
- [x] 2026-09-11 | `e452c8f` | classroom_dashboard.html | **The projector picker's close button follows the inline end (S376 M candidate, P4, logical CSS):** `.pick-close` `right:20px` → `inset-inline-end:20px`. | verified: `d378/pickclose.mjs`, 8 cells EN/HE × light/dark × 1280/800 via `openPickOverlay()`: LTR unchanged (20px from the right), RTL 20px from the left (was the right); centre …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-11 | `20baafc` | torah_trainer.html + trope_tutor.html + classroom_dashboard.html | **The footer's "Related Hebrew Tools" header converges on 0.78rem across all seven carriers (S376 M candidate, P4):** torah/trope 0.72rem (11.52px) and the dashboard 0.7rem (11.2px) → the 0.78rem majority; one inline `font-size` each. | verified: `d378/footer.mjs`, computed style on the 7 carriers, …[full text: IMPROVEMENT_ARCHIVE.md]
...
- 2026-09-11 | **S378** | iters: 1 pass (**D**) + 4 fixes = **5** (full budget) | tools: dictionary ×2 (`595ffca`, `589a723`), classroom_dashboard (`e452c8f`), torah + trope + dashboard (`20baafc`) | patterns fixed: logical-CSS ×1 | pass run: D (dictionary, 2nd dedicated; 2 hits fixed, 3 …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S378 (2026-09-11):** hebrew_dictionary **2026-09-11 (`589a723`; D S378)**; classroom_dashboard **2026-09-11 (`20baafc`; G S364)**; torah_trainer **2026-09-11 (`20baafc` S378)**; trope_tutor **2026-09-11 (`20baafc`; M S376)**; flash_cards 2026-09-11 (`82bbf4c`; N S361); Hebrew_Font_Maker 2026-09-11 (`24a4c91`; C S368); resources 2026-09-11 (`b1c0655`; N S375); privacy 2026-09-11 (`f8f23e0`; S360); hebrew_blend_generator 2026-09-11 (`5dfae78`; H S363); index 2026-09-10 (`edf4b2e` S370); contact / terms / 404 2026-09-09 (S359–S361 chrome …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S379):** **BRANCH/PR: S369–S378 ran on `claude/improve-loop-swgwqx` (cut at `66dd97b` = `origin/main`; 50 commits at this close-out) → draft PR #229. If it is still OPEN and unmerged, continue on it; if MERGED, cut a fresh `claude/*` off latest `origin/main` — never stack on merged history. Verify via the API** (0 check runs is correct — no workflows). **⚑ DRIFT CHECK: compare `origin/main` against `66dd97b`; anything beyond THIS loop's commits is an outside-loop landing. `sw.js` v703, `FONT_MAKER_VERSION` 5.39** — re-read both. A fresh container may start shallow — `git rev-parse --is-shallow-repository` before `update-sitemap.mjs`; the HTTP server on 8080 dies across a compaction — curl it first; the scratchpad keeps only `b370/`…`d378/` (older harness dirs are gone — rebuild from the receipts in loop-findings).

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), I (S366), H (S367), C (S368), F (S369), B (S370), A (S371), E (S372), L (S373), K (S374), N (S375), M (S376), G (S377), D (S378).** Unattended: take I (first-load & empty-state; the 28th run was S366 — 13 pages × EN/HE virgin contexts, the corrupt-key and string-shape arms; `i366/` is gone, rebuild from the S366 receipts). Then H (Font Maker, S241). **O next `torah_trainer.html`; C next flash_cards (S271); F next the disabled-control look, then the four-picker `.font-opt-attr` step; G next: re-derive per tool from the archive; M next `index.html` (S246) then torah; N next trope (S248) then index (S249); D next flash_cards or the Font Maker (re-derive from the archive); the next A sweeps `1d4c9cf..HEAD` with the range-setter arm on the Font Maker's 13 sliders + the torah/trope `_sliderNum` sites, the `vh-capped-sheet-without-dvh-twin` row (0 open; 3 clean sweeps retire it) and the `dark-print-shadow-slab` row (sweep :hover/:focus shadows at any alpha); the next E re-runs the 17 arms over `408dbe4..HEAD`; the next L re-runs vs `4a1f186`; the next K re-runs over `4a1f186..HEAD` (seed the Infinity cell with the RAW `1e999` string).**

**⚑ Harness (standing): FONT MAKER probes pass `dismiss:false` to `b370/harness.mjs` `open()`; on a PHONE descriptor `#mobileWarnOverlay` opens INSTEAD of the wizard — call `dismissMobileWarn(false)` then wait `#wizardOverlay.open`; enter via `#wizName`/`#wizAuthor` → `#wizNext` ×2 → `wizardCreate()`; Load menu `#wizOpenProject`; torah stubs Sefaria (`harness.mjs` `SEF`); hidden toggles → click `label.toggle:has(#id)`; dashboard: `frSkip()` BEFORE any header click, `openPickOverlay()` + `renderGroupCards([[…]])`; flash dialogs: `chooseProfileModal()`, `openWeaknessView(name)`, `openLadderMap()` after `createProfile(name)`; dictionary: ready = `filtered.length>0` + `#loadingMsg` hidden, search = set `#searchInput.value` + `onFilter()`, bulk = `#bulkModeCb` (onchange), `wlAddList(name, words)` returns `{ok,id}` → `wlLoadListAsFilter(id)`, the Advanced sliders sit in nested collapsed sections (expand outermost first); PRINT: `page.pdf()` fires beforeprint, measure at the PAPER width, `g377/` + `pdfcheck.py`/`topline.py`; PERF: `d378/lib.mjs` (longtask/mutation/rAF counters, CDP throttle + profiler, the 300ms click control), `waitForFunction` polling is an interval number (never `'mutation'`, rAF counts itself), a delay `page.route` goes AFTER the harness catch-all; CSSOM walks skip `CSSStyleRule.cssRules`; contrast probes composite alpha; N `n375/mobile.mjs`; M `m376/audit.mjs`; screenshot paths are cwd-relative.**

**⚑ TOP UNGATED CANDIDATES (S378):** the ~30 inline Font Maker hints under 11px; the tour-card h4 → h2 ×7; the four-picker `.font-opt-attr` step (one F change or a waiver); the remaining sub-floor M calls (bingo stepper, scaled toggles, `#rulerCorner`). Gate 2: the dictionary drag-freeze and select-all preview-cap proposals (S378 D, numbers in loop-findings); the corpus's split article tokens (S377); the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on; the trope FAQ a7 copy; the four English-font picker headers. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. **Seed bench: 25 (unchanged; D is not an intake). Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

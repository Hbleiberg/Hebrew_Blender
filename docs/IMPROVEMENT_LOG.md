...
- **S377 (2026-09-11):** hebrew_dictionary **2026-09-11 (`a92d135`; G S377)**; flash_cards **2026-09-11 (`82bbf4c`; N S361)**; Hebrew_Font_Maker **2026-09-11 (`24a4c91`; C S368)**; trope_tutor 2026-09-11 (`16f2d18`; M S376); classroom_dashboard 2026-09-11 (`d1f04d2`; G S364); resources 2026-09-11 (`b1c0655`; N S375); privacy 2026-09-11 (`f8f23e0`; S360); hebrew_blend_generator 2026-09-11 (`5dfae78`; H S363); torah_trainer 2026-09-11 (`31a84f7` S374); index 2026-09-10 (`edf4b2e` S370); contact / terms / 404 2026-09-09 (S359–S361 chrome sweep).
...
**Next session (S378):** **BRANCH/PR: S369–S377 ran on `claude/improve-loop-swgwqx` (cut at `66dd97b` = `origin/main`; 45 commits at this close-out) → draft PR #229. If it is still OPEN and unmerged, continue on it; if MERGED, cut a fresh `claude/*` off latest `origin/main` — never stack on merged history. Verify via the API** (0 check runs is correct — no workflows). **⚑ DRIFT CHECK: compare `origin/main` against `66dd97b`; anything beyond THIS loop's commits is an outside-loop landing. `sw.js` v702, `FONT_MAKER_VERSION` 5.39** — re-read both. A fresh container may start shallow — `git rev-parse --is-shallow-repository` before `update-sitemap.mjs`; the HTTP server on 8080 dies across a compaction — curl it first.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), D (S365), I (S366), H (S367), C (S368), F (S369), B (S370), A (S371), E (S372), L (S373), K (S374), N (S375), M (S376), G (S377).** Unattended: take D (performance) over `hebrew_dictionary.html` (S253, its stalest tool; the on-screen keyboard, sidebar drag, saved-list banner and print collapse all moved since — start from the S253 receipts in loop-findings: the default-Print freeze is layout-bound and refuted as fixable, fragment batching is noise, the shoresh explorer is fast). Then I (S366; re-run `i366/{gates,arms,corrupt,strshape}.mjs`). **O next `torah_trainer.html`; H next Font Maker (S241); C next flash_cards (S271); F next the disabled-control look, then the footer-nav header step (S376); G next: re-derive per tool from the archive; M next `index.html` (S246) then torah; N next trope (S248) then index (S249); the next A sweeps `1d4c9cf..HEAD` with the range-setter arm on the Font Maker's 13 sliders + the torah/trope `_sliderNum` sites, the `vh-capped-sheet-without-dvh-twin` row (0 open; 3 clean sweeps retire it) and the `dark-print-shadow-slab` row (sweep :hover/:focus shadows at any alpha — the 0.18 bar missed a printing 8% halo); the next E re-runs the 17 arms over `408dbe4..HEAD`; the next L re-runs `l373/{audit,bytecheck}.mjs` vs `4a1f186`; the next K re-runs `k374/{static.py,he.mjs}` over `4a1f186..HEAD` (seed the Infinity cell with the RAW `1e999` string).**

**⚑ Harness (standing): FONT MAKER probes pass `dismiss:false` to `b370/harness.mjs` `open()`; on a PHONE descriptor `#mobileWarnOverlay` opens INSTEAD of the wizard — call `dismissMobileWarn(false)` then wait `#wizardOverlay.open`; enter via `#wizName`/`#wizAuthor` → `#wizNext` ×2 → `wizardCreate()` (`#wizCancelBtn` is `hidden` on a first run); Help/QA/Edit shape = `openHelp()`/`openQA()`/`openMarkEdit('nikkud', cp, {global:true})`; Load menu = `#wizOpenProject`, recents via `pushRecentProject(project)`; project file = `JSON.stringify(project)` + `format:'hebrew-font-maker-project'` through `#projFile`; torah stubs Sefaria (`harness.mjs` `SEF`); hidden toggles → click `label.toggle:has(#id)`; dashboard: `frSkip()` BEFORE any header click, `openPickOverlay()` + `renderGroupCards([[…]])`; flash dialogs: `chooseProfileModal()`, `openWeaknessView(name)`, `openLadderMap()` after `createProfile(name)`; dictionary: ready = `filtered.length>0` + `#loadingMsg` hidden, search = set `#searchInput.value` + `onFilter()`, `toggleBulkMode(true)`, `toggleDictAudio(true)`, `wlAddList(name, words)` + `wlLoadListAsFilter(id)`; PRINT probes: `page.pdf()` fires beforeprint, measure at the PAPER width, `g377/{pdf,export,hover,blank}.mjs` + `pdfcheck.py`/`topline.py` (`pip install pymupdf` works; never name a script `inspect.py`); CSSOM walks skip `CSSStyleRule.cssRules`; contrast probes composite alpha; N `n375/mobile.mjs`; M `m376/audit.mjs`; screenshot paths are cwd-relative.**

**⚑ TOP UNGATED CANDIDATES (S377):** the ~30 inline Font Maker hints under 11px; the tour-card h4 → h2 ×7; the four-picker `.font-opt-attr` step (one F change or a waiver); the footer-nav header step ×3 (F); the dashboard `.pick-close` physical pin; the remaining sub-floor M calls (bingo stepper, scaled toggles, `#rulerCorner`). Gate 2: the corpus's split article tokens (S377); the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on; the trope FAQ a7 copy; the four English-font picker headers. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. **Seed bench: 25 (unchanged; G is not an intake). Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

...
- [x] 2026-09-03 | `bc11d44` | hebrew_blend_generator.html | **The options-sidebar drag writes `--gen-sidebar-w` on `.sidebar` (`scope`) and the property is registered `inherits:false` with its 320px initial value** — pattern carrier 4 of 6, the S325 candidate. 4×: 716 → 74 ms of style per 80px drag, frame gaps [50,50,50] → []; 1×: 13 ms. | Verified before/after in one script: saved width 380 …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S326 (2026-09-03):** Hebrew_Font_Maker **2026-09-03 (×2, `fc2bad5` My Fonts trace gate, `4b1238c` loadScript Error; outside-loop v5.39 `4638e7b`; S321 `09f6220`)**; hebrew_blend_generator **2026-09-03 (×1, `bc11d44` sidebar scope; S322 ×2)**; classroom_dashboard **2026-09-03 (×1, `c0520f7` slider write target; S325 ×2, S324 ×1, S323 ×2, S322 ×2, S321 ×2)**; torah_trainer + trope_tutor 2026-09-03 (S325 ×1 each, `280965f`; torah S324 ×2); index 2026-09-03 (S325 ×1, S324 ×1, S323 ×2); hebrew_dictionary 2026-09-03 (shared-block edit only, …[full text: IMPROVEMENT_ARCHIVE.md]
...
- I first-load & empty-state: 2026-09-03 (**S326 — 25th run, its first since S308 (~18 sessions); I was the single stalest by BOTH date and session and the S326 pointer named it AND both weighted surfaces. Mechanical gates clean a TWELFTH consecutive run:** 26 genuinely-virgin loads (13 pages × EN/HE, new context per load = empty localStorage AND IndexedDB, SW blocked) → 0 pageerrors / 0 …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S327):** **BRANCH/PR: CONTINUE on `claude/improve-loop-k280wo` → draft PR #222 (S316–S326 commits + FM v5.39; verify via the API first — if it has merged, cut a fresh `claude/*` branch off latest `origin/main`).** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v650, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS C (2026-09-02, S312), THEN F (S313), O (S315, attended only), B (S316), A (S317), E (S318).** **C's target by its own per-tool history is `hebrew_blend_generator.html` (S224)** — re-derive against the ARCHIVE; the generator's changed surface since S224 includes the S316/S317 `applySettings` filters, the S322 font-picker tile floor, the gold tokens and the S326 sidebar scope. **F's next affordance:** the collapsible-header hover idiom filed by S317 (4 tools), or the tooltip-binding divergence filed by S285. **O's is flash_cards.html — attended only.** Run `update-sitemap` at EVERY close-out.

**⚑ THE STRONGEST UNTAKEN CANDIDATES:** the picker section headers half-translated in HE (P4, K — 7 CSV rows + 9 sites); the dictionary `--dict-sidebar-w` carrier (the pattern's last, blocked by the fixed toggle — reposition it first); the "Date format" wrap (P4, dashboard); the dashboard `vh` modal twins (P4); the two S324 torah residuals (side-by-side builder); the S319 gate-2 Hebrew-interface claim and the S323 gate-2 manual-mode copy (attended). Seed bench: the scroll-style handout (torah, S324), the last-backup date (index, S323).

**/!\ THE S326 METHOD LESSONS.** (a) **"Has contours" is not "is traced":** a brand-new FM project ships 17 pre-drawn punctuation glyphs, so any `l.contours` count is ≥17 on an empty font — `coreLetterStats()` (which excludes `cat:'punct'`) is the one gate; grep new guards for `.contours` counts. (b) **A `<script>` `onerror` rejects with the DOM Event**, and every `e.message || e` sink prints "[object Event]" — fix at the loader, not the sinks. (c) **Parallel probe processes: a crashed node leaves its Chromium holding the shell's stdout and the whole command hangs to the timeout** — run each with `timeout N`, close the browser in `finally`, and never `pkill -f chromium` (it matches its own shell). (d) **Assert box arithmetic, not the var:** the generator sidebar rect = `--gen-sidebar-w` + 41px padding/border; a probe that expected 400 read 441 and called the fix broken. (e) The virgin-profile cross-tool handshake is now provable from the FM side too (Exported ✓ → ★ My IvritSuite Fonts → the dictionary's `.font-section-hdr`), with the app creating `ivritsuite-fonts` — never open that DB from the probe (S308).

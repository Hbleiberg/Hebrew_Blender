...
- [ ] P4 (**NEW S375 Pass N — pattern `vh-capped-sheet-without-dvh-twin`; resources FIXED `b1c0655`; census in loop-findings**) | flash_cards.html ×3 + hebrew_dictionary.html + Hebrew_Font_Maker.html ×4 + classroom_dashboard.html | **Nine centred fixed sheets cap at `max-height:NNvh` with no `NNdvh` twin (flash's three JS dialogs, the dictionary `.wm-dialog`, the FM wizard/QA/metrics/help modals, …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-11 | (S375 close-out) | branch/deploy note | **S375 CONTINUED `claude/improve-loop-swgwqx` (PR #229 open, unmerged; base `66dd97b` = `origin/main`), 4 commits + this close-out; a human merges and confirms the Pages run. Drift: none (`66dd97b`, sw v699, FM 5.39 — as the S374 close-out recorded). sw v699→v700 (resources, privacy, flash, generator); FM NOT bumped (untouched). Scripts: …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-11 | `b1c0655` | resources.html | **`.sg-modal` caps at `max-height:90vh;max-height:90dvh` (the hub's `.ie-modal` pair) — the Suggest and Preview sheets no longer overflow a phone backdrop with the URL bar expanded** (`vh-capped-sheet-without-dvh-twin`, registered S375) | `n375/dvh.mjs`: the cascade resolves to `90dvh` (CSSOM), computed max-height unchanged headless …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S375 (2026-09-11):** resources **2026-09-11 (`b1c0655`; N S375)**; privacy **2026-09-11 (`f8f23e0`; S360)**; hebrew_blend_generator **2026-09-11 (`5dfae78`; H S363)**; flash_cards **2026-09-11 (`c5a491e`; N S361)**; torah_trainer 2026-09-11 (`31a84f7` S374); trope_tutor 2026-09-11 (`5d1d13a` S374; H S367); hebrew_dictionary 2026-09-11 (`1de1ee8` S374; M S362); Hebrew_Font_Maker 2026-09-11 (`9257fb8` S373; C S368); classroom_dashboard 2026-09-11 (`c365786` S373; G S364); index 2026-09-10 (`edf4b2e` S370); contact / terms / 404 2026-09-09 …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`vh-capped-sheet-without-dvh-twin`**: ACTIVE (registered S375). **S375: resources FIXED (`b1c0655`); 9 open (flash ×3, dictionary, FM ×4, dashboard — the S375 candidate). Hits 10, clean streak 0.** Detection: `grep -nE '(max-)?height:\s*(calc\(|min\()?\s*[0-9]+vh' | grep -v …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S376):** **BRANCH/PR: S369–S375 ran on `claude/improve-loop-swgwqx` (cut at `66dd97b` = `origin/main`; 35 commits) → draft PR #229. If it is still OPEN and unmerged, continue on it; if MERGED, cut a fresh `claude/*` off latest `origin/main` — never stack on merged history. Verify via the API** (0 check runs is correct — no workflows). **⚑ DRIFT CHECK: compare `origin/main` against `66dd97b`; anything beyond THIS loop's 35 commits is an outside-loop landing. `sw.js` v700, `FONT_MAKER_VERSION` 5.39** — re-read both. A fresh container starts shallow again — `git fetch --unshallow` before `update-sitemap.mjs`; the HTTP server on 8080 dies across a compaction — curl it first.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), M (S362), G (S364), D (S365), I (S366), H (S367), C (S368), F (S369), B (S370), A (S371), E (S372), L (S373), K (S374), N (S375).** Unattended: take M (aesthetics) over `trope_tutor.html` (S245, its stalest surface; the settings drawer, the 8-sheet print chart, the roving tabs and the retry path landed between) in the full matrix (EN/HE × light/dark × 1280/800 + the drawer, chart and fullscreen views), BEFORE/AFTER screenshots per fix, anything dramatic = gate 3 → log with paths. Then G over the dictionary (S252). **O next `torah_trainer.html`; D next dictionary (S253); H next Font Maker (S241); C next flash_cards (S271); F next the disabled-control look; N next trope (S248) then index (S249); the next A sweeps `1d4c9cf..HEAD` with the range-setter arm on the Font Maker's 13 sliders + the torah/trope `_sliderNum` sites + the NEW `vh-capped-sheet-without-dvh-twin` row (9 open sites, detection in its row); the next I re-runs `i366/{gates,arms,corrupt,strshape}.mjs`; the next E re-runs the 17 arms over `408dbe4..HEAD`; the next L re-runs `l373/{audit,bytecheck}.mjs` vs `4a1f186`; the next K re-runs `k374/{static.py,he.mjs}` over `4a1f186..HEAD` (seed the Infinity cell with the RAW `1e999` string).**

**⚑ Harness (standing): FONT MAKER probes pass `dismiss:false` to `b370/harness.mjs` `open()` (its `.overlay.open` sweep closes the wizard); enter through the wizard (`#wizName`/`#wizAuthor` → `#wizNext` ×2 → `wizardCreate()`), Load menu = `#wizOpenProject` / the toolbar `toggleLoadMenu(event)`, seed recents with `pushRecentProject(project)`; a project file = `JSON.stringify(project)` + `format:'hebrew-font-maker-project'` via `page.setInputFiles('#projFile', {buffer})`; torah stubs Sefaria by fulfilling `**/*sefaria.org/**` (`harness.mjs` `SEF`); hidden toggle inputs → click `label.toggle:has(#id)`; dashboard: `frSkip()` BEFORE any header click, `openWeekEditor()`, `openSettings()`; the generator's sidebar is a drawer at 390; the dictionary's collapsed bodies need a REAL click on Expand-all-menus; flash: the practice screen is `#screenCard.active`; probe screenshot paths are cwd-relative; a junk-store cell = `context.addInitScript` writing the RAW key string (`JSON.stringify(1e999)` is null), then a NaN/Infinity/undefined/[object Object] scan of `*Val` labels, inline styles and root custom props; the shared-block sha arm pairs `end X` with the nearest preceding start marker containing X's core token. N harness: `n375/mobile.mjs` (real descriptors, `page.tap`, an overflow census skipping clipped scrollers, `modalFit` via `elementFromPoint` on the ×; tap the footer FAQ's OUTER summary first; `sheet.mjs` builds contact sheets without PIL).**

**⚑ TOP UNGATED CANDIDATES (S375):** the 9 vh-capped sheets (flash ×3, dictionary, FM ×4, dashboard — one declaration pair per site, ≤2 tools per session); the ~30 inline Font Maker hints under 11px; the tour-card h4 → h2 ×7; the four-picker `.font-opt-attr` step (one F change or a waiver); the remaining sub-floor M calls (bingo stepper, scaled toggles, `#rulerCorner`). Gate 2: the printed teacher-copy wording under a Hebrew header; the board's day plates under backgrounds-on; the trope FAQ a7 copy; the four English-font picker headers. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals (`n361/`). The 5 torah/trope credit rows: wire-or-prune. **Seed bench: 25 (unchanged; N is not an intake). Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

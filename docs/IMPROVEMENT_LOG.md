...
- [ ] P4 (**S337 Pass G, seen once; S338 could not reproduce it in 8 controlled `?start=ezra-sil-sr` loads — 5 polled every 120 ms for 3 s after step 1 then clicked Next for real (step 2 every time; the only `aria-disabled="true"` writes are step 2's own empty-name validation), 3 with both fields filled 200 / 800 / 4500 ms after step 1 (step 3 every time). The one S338 stall was a harness run …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-04 | fc73fd9 | Hebrew_Font_Maker.html | **The UFO export declares installable embedding** — `openTypeOS2Type: []` in `buildUfoFontInfo` (the S337 P3: absent, ufo2ft/fontmake default to fsType 4 while the TTF export writes 0). | Verified with the real engine (the S337 mirror recipe, `g337/g1.mjs`): the downloaded `.ufo.zip`'s fontinfo.plist carries the empty array, ufoLib 4.64 …[full text: IMPROVEMENT_ARCHIVE.md]

- [x] 2026-09-04 | f92ae1d | torah_trainer.html + trope_tutor.html | **The shared `.toggle` switch reaches the 24px touch floor** (`sub-floor touch target`; the S334 Pass N filing — it had been filed to the archive by its own "measured, not fixed" phrase — on its only two carriers): height 22 → 24, knob `top` 3 → 4; width, 16px travel and colours unchanged. | Verified at 1280/800 × light/dark on …[full text: IMPROVEMENT_ARCHIVE.md]
...
- 2026-09-04 | **S338** | iters: 1 pass (**D**) + 3 fixes = **4** (5th unspent — the pass's one defect was the ledger's own P2 and every other open candidate is gated or a decision) | tools touched: hebrew_blend_generator ×1 (`5b55d19` PDF Flate), Hebrew_Font_Maker ×1 (`fc73fd9` UFO fsType), …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S338 (2026-09-04):** hebrew_blend_generator **2026-09-04 (×1, `5b55d19` PDF Flate; S333 ×2)**; Hebrew_Font_Maker **2026-09-04 (×1, `fc73fd9` UFO fsType; S337 ×2; outside-loop v5.39)**; torah_trainer **2026-09-04 (×1, `f92ae1d` toggle; S337 ×2)**; trope_tutor **2026-09-04 (×1, `f92ae1d` toggle; S333 ×2)**; S336 ×1; outside-loop v5.39)**; S336 ×2)**; classroom_dashboard 2026-09-03 (S336 ×1, `009611b`); locales/ui-strings.csv 2026-09-03 (S334 `fa4dce9`); flash_cards 2026-09-03 (S333 ×2); hebrew_dictionary 2026-09-03 (S333 ×1); resources …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`sub-floor touch target`**: **1 carrier FIXED 2026-09-04 (S338, the S334 Pass N filing): the shared `.toggle` switch 38×22 → 38×24 on its only two carriers, torah_trainer + trope_tutor (`f92ae1d`).** **1 new carrier found 2026-09-03 (S334 Pass N census on …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S339):** **BRANCH/PR: S338 CONTINUED `claude/improve-loop-0mbq58` → draft PR #223 (head after this close-out; base `origin/main` `4b66a64`). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v662, `FONT_MAKER_VERSION` 5.39** — re-read both.

**⚑ THE STALEST PASS IS O (S315, attended only), THEN I (S326), C (S327), F (S328), B (S329), A (S330), E (S331), L (S332), K (S333), N (S334), M (S335), H (S336), G (S337), D (S338).** **Unattended: skip O and take I** (first-load & empty-state, every tool in a fresh context — empty localStorage AND IndexedDB; S326 ran 26 virgin loads with the mechanical gates clean a twelfth run). Weight what landed since S326: the generator's remembered-setup bar and its `startFreshSetup()` on a virgin profile, the Font Maker's partner `?start=` wizard on a virgin profile (one S338 harness stall under CPU contention; 8 controlled loads did not reproduce it), the torah holiday strip. O's surface is still flash_cards.html. **Harness notes:** the S337 CDN mirror (loop-findings, Font Maker section; `ops.md`) also serves the generator's PDF export (html2canvas + jsPDF, same vendor dir); the generator's `#bingoToggle` / `#realWordsToggle` sit in `display:none` rows — drive the modes through the `setWorksheetType('bingo')` / `setPracticeType('realwords')` buttons; Playwright's `addInitScript` takes ONE argument (pass an object — a second positional argument is dropped silently, which made S338's first "restored" cold cells virgin). **Ledger lesson (standing):** write open candidates without the `NON_ACTIONABLE` phrases ("measured, not fixed", "logged not fixed") or `compact-ledger` files them — the S334 toggle candidate had already been filed that way and was fixed from the archive's copy.

**⚑ THE STRONGEST UNTAKEN CANDIDATES:** the once-seen partner-wizard stall (P4, unreproduced in 8 loads — only with a fresh sighting that captures the stack); the four unauthored picker terms (gate 2); the staging author URL on the hub (gate 2); the real-words 7.2px grid (layout decision); the dashboard `.tip-icon`-vs-`.tip-wrap` divergence (F); the help tab strip (gate 3); the S321 timer placeholder copy (gate 2); the specimen-A4-vs-template-Letter paper choice (gate 2). **For the next A sweep:** `uncompressed-jspdf-raster` has 0 open carriers — its first sweep (`grep -n "addImage(" *.html`: 3 sites, all `'FAST'`) starts the retirement streak; `blocking-alert-for-a-routine-path` still wants its dashboard click census; the S335 runtime arm for `background:#fff` / `color:#1a2744` on the 8 other pages still stands. **Seed bench (unchanged):** the holiday share link (torah, S336 — ~15 lines, the smallest seed), the handout selection (torah, S336), the scroll-style handout (torah, S324), the last-backup date (index, S323).

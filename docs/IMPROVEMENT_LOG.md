...
**Next session (S384):** **BRANCH/PR: S383 CONTINUED `claude/wizardly-tesla-xu0ikd` (PR #242, draft, open, unmerged; base `4d6c02d`), adding 3 fix commits + this close-out to S382's 4. If that PR is still open and unmerged, CONTINUE on it; if merged, cut a fresh `claude/*` off latest `origin/main` and open a new draft PR at close-out. Verify via the API** (0 check runs is correct — no workflow triggers on `pull_request`; the `pending` state on a commit with `total_count: 0` is GitHub's default, not a queued check). **⚑ DRIFT CHECK: compare `origin/main` against `4d6c02d` + this loop's own commits; S383 found NONE (S382 ran the same day, unmerged), but S382 found the largest drift yet, so CHECK IT, never trust this note unread. `sw.js` v750, `FONT_MAKER_VERSION` 5.44, migrations 0001–0003 all `Live? = yes`.** A fresh container starts SHALLOW — `git fetch --unshallow` before `update-sitemap.mjs`. The `--sdk` fixture the required smokes need: `npm pack @supabase/supabase-js@2.116.0` (the npm registry IS reachable; the CDNs are not), untar, pass `package/dist/umd/supabase.js`; **pass `--sdk` to `smoke-tools.mjs` too or its signed-in B arms silently SKIP** (114 checks without it, 135 with).

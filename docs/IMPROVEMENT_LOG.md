...
- [ ] P3 (**NEW S382 Pass P follow-on census — the handoff's F item, MEASURED; the same class S382 fixed twice, so it was at its 2-per-session cap**) | hebrew_dictionary.html | **The letter-picker pages are one Tab stop PER TILE: `.lpt` ×32 on each of `#pg0`–`#pg5` + `#pgAny` = 224 stops, and `#resultsGrid .word-card` adds 100 at the default page size.** One `roveTileGrid(grid, '.lpt')` per page …[full text: IMPROVEMENT_ARCHIVE.md]
...
- [x] 2026-09-15 | `b5f06f5` | hebrew_blend_generator.html + flash_cards.html | **The lock ×3 and real-word letter grids were one Tab stop per tile (26+21+26+31 = 104 of 105 across the five grids). Adding `roveTileGrid()` to the builders was a NO-OP: all four are built while their mode-gated ancestor is `display:none`, and the helper's `offsetParent !== null` tile filter came back empty, so it …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S382 (2026-09-15):** hebrew_blend_generator **2026-09-15 (`b5f06f5`; P S382)**; flash_cards **2026-09-15 (`b86faca`; P S382)**; hebrew_dictionary 2026-09-11 (`f4a102d`; D S378); classroom_dashboard 2026-09-11 (`de49066`; G S364); torah_trainer 2026-09-11 (`f4a102d`); trope_tutor 2026-09-11 (`f4a102d`; M S376); Hebrew_Font_Maker 2026-09-11 (`e100574`; H S380); resources 2026-09-11 (`b1c0655`; N S375); privacy 2026-09-11 (`f8f23e0`; S360); index 2026-09-11 (`4f9a2e7`; S379); contact / terms / 404 2026-09-09 (S359–S361 chrome); **account …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **`tile-grid-built-hidden-never-roved`** (**NEW, registered 2026-09-15 (S382) — 4 carriers, all fixed**): ACTIVE. A `roveTileGrid()` call placed in a builder that runs while the container is `display:none` silently does nothing: the helper's tile list filters `offsetParent !== …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S383):** **BRANCH/PR: S382 ran on a FRESH `claude/wizardly-tesla-xu0ikd` cut off `4d6c02d` (PR #229 merged at S381), 3 fix commits + this close-out. If that PR is still open and unmerged, CONTINUE on it; if merged, cut a fresh `claude/*` off latest `origin/main` and open a new draft PR at close-out. Verify via the API** (0 check runs is correct — no workflows). **⚑ DRIFT CHECK: compare `origin/main` against `4d6c02d` + this loop's own commits; anything else is an outside-loop landing — and S382 found the largest drift yet (v706→v748, FM 5.39→5.44, Accounts phases 9–10, migrations 0002/0003 now Live), so CHECK IT, never trust this note unread. `sw.js` v749, `FONT_MAKER_VERSION` 5.44, migrations 0001–0003 all `Live? = yes`.** A fresh container starts SHALLOW — `git fetch --unshallow` before `update-sitemap.mjs`. The `--sdk` fixture the four required smokes need: `npm pack @supabase/supabase-js@2.116.0` (the npm registry IS reachable; the CDNs are not), untar, pass `package/dist/umd/supabase.js`.

**⚑ STALEST PASS: O (S346, attended-only — unattended skips it), F (S369), B (S370), A (S371), E (S372), L (S373), K (S374), N (S375), M (S376), G (S377), D (S378), I (S379), H (S380), C (S381), P (S382).** Unattended: take **F** (cross-tool consistency — it has been stalest for three sessions running; affordance: the DISABLED-control look, or finish the `role=button` tile-grid census S382 started, below), then B. **O next `torah_trainer.html`; C next torah (S298) then dictionary (S285); M next `index.html` (S246) then torah; N next trope (S248) then index (S249); D next flash_cards or the Font Maker; H next the dashboard (S46) or the generator (S56); P next the dashboard or the dictionary (S382 covered the generator + flash cards + the shared modules); the next A sweeps `4d6c02d..HEAD` and must add the two NEW rows registered this session (`synced-key-wiped-without-forgetRow`, `tile-grid-built-hidden-never-roved`) plus the `vh-capped-sheet-without-dvh-twin` and `dark-print-shadow-slab` rows; the next E re-runs its 17 arms over `408dbe4..HEAD` — note E has never seen the backend landing either; the next L re-runs vs `4a1f186`; the next K over `4a1f186..HEAD`; the next I re-runs `i379/gates.mjs`.**

**⚑ P-PASS HARNESS (standing, NEW S382): the backend smokes are the ONLY verification that sees a signed-in page — the generic recipe route-aborts the SDK CDN and the project URL, so it silently tests the anonymous path. `smoke-account`/`saves`/`tools` all hardcode port 8080 (run one at a time); fontmaker/account-page/migration take 8081–8084 and run in parallel. `smoke-tools.mjs --only <file>` narrows to one page. THREE false-clean traps S382 hit and you will hit again: (1) a literal-key grep over reset handlers reports clean on every page because the wipe is `removeItem(CONSTANT)` — resolve `const X = 'hebrew…'` first; (2) AllTools registers synced keys by `ivritKey` alias and inside helper functions, so a raw-lsKey search manufactures hits; (3) a probe seeding `ivritSuite_syncMeta` MUST include `v: 1` or `metaAll()` discards the fixture and `forgetRow` silently no-ops — and `addInitScript` re-seeds on the reload `startFreshSetup()` triggers, restoring what you just deleted. `metaDelete` leaves the kind branch as `{}` (truthy), so assert on the ROW (`branch.default`), not the branch.**

**⚑ TOP UNGATED CANDIDATES (S382): the dictionary letter-picker grids (P3 — `.lpt` ×32 on 7 containers = 224 Tab stops, plus 100 word cards; one `roveTileGrid` line each, the shipped shape); the tour-card h4 → h2 ×7 (it is the "next touch of a tour engine" — extract the shared block in the same session); the remaining sub-floor M calls (bingo stepper, scaled toggles, `#rulerCorner`); the `.rw-letter-tile .name` 7.2px grid (a layout decision); the unreachable nikkud hint (P4, half gate 2). Decide-then-sweep: whether an action TOOLBAR should rove like a selection grid (dashboard ×3, torah ×2 — S382 census). Gate 2: the flash-card colour swatch labels (raw English, S207); the generator's `restoreLastSetup` corrupt-drop (forget or not); the dictionary drag-freeze and select-all preview-cap (S378); the corpus's split article tokens (S377); the printed teacher-copy wording; the board's day plates; the trope FAQ a7 copy; the four English-font picker headers; the resources preview stack's Taamim fallback. Gate 3/M: the Font Maker stage annotations. Gate 4 (attended only): the flash-card sticky-bar and landscape-card proposals. The 5 torah/trope credit rows: wire-or-prune. **Seed bench: 26. Gate 1 next attended session: the trope "drill the marks I missed" and the three dual-audience S seeds first.**

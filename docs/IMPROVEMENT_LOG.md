...
- [ ] P3 (**NEW S351 Pass G — paper waste; ungated, print-only CSS**) | trope_tutor.html | **The 26-mark reference chart runs 13 sheets at Letter AND A4** (cards 300–544px tall in a 2-column grid at the 741px print width; most sheets hold ONE row — 2 cards — and leave ~45% blank; per-page card counts Letter [2,2,1,2,2,1,3,2,1,2,4,1,3], A4 [1,2,2,2,2,2,2,2,2,3,2,2,2]). A print-only compaction …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P4 (**NEW S352 — seen in the Pass D screenshots; one attribute, ungated**) | index.html | **In the Hebrew UI the Manual-input export box renders its JSON right-to-left** (`#ieTextarea` inherits `dir="rtl"`, so `"_hebrewBlenderExport": 1,` paints with its punctuation reordered and each line starts at the right edge). It is machine text a teacher copies, never reads; `dir="ltr"` on the …[full text: IMPROVEMENT_ARCHIVE.md]

- [ ] P4 (**NEW S351 Pass G — GATE 2 copy, for L/the maintainer**) | trope_tutor.html (`locales/ui-strings.csv`, the FAQ JSON-LD) | **The FAQ answer `trope.footer.faq_a7` and its JSON-LD copy promise "prints all 26 marks as one reference sheet"; the chart is 13 sheets.** "as one reference chart" (the button's own wording) would be accurate; or the compaction candidate above narrows the gap. | …[full text: IMPROVEMENT_ARCHIVE.md]
...
- **S352 (2026-09-09):** torah_trainer **2026-09-09 (×2, `97b68b4` handout row, `4b9273f` print band; S351 ×1)**; trope_tutor **2026-09-09 (×1, `43ae79e` Drill-tab print; S351 ×2)**; index **2026-09-09 (×1, `f7c9a88` export textarea; S350 ×1)**; contact 2026-09-08 (S350 ×1, `ac5f930`); 404 2026-09-08 (S349 ×1, `5e64c95`); flash_cards 2026-09-08 (S347 ×1, `7c6337a`; S346 ×1); classroom_dashboard 2026-09-08 (S345 ×1, `4e25247`); hebrew_blend_generator 2026-09-08 (S344 ×1, `608aa28`); resources 2026-09-04 (S341 ×1, `39cf34a`); Hebrew_Font_Maker …[full text: IMPROVEMENT_ARCHIVE.md]
...
**Next session (S353):** **BRANCH/PR: S352 ran on the fresh `claude/inspiring-meitner-rhqskk` → draft PR #227 (base `origin/main` `f2384a7`; S352 alone on it — 5 commits). CONTINUE that branch and PR if still open and unmerged; if merged, cut a fresh `claude/*` off latest `origin/main`. VERIFY via the API — this note is a snapshot.** 0 check runs is correct (no workflows); deploy is verifiable only AFTER a merge. **Drift note: `sw.js` is v677, `FONT_MAKER_VERSION` 5.39** — re-read both. **Outside-loop landing to know: `f2384a7` (PR #226) added the `'IvritSuite Taamim'` fallback face to every Hebrew font stack + the keyboard's `.hk-key-glyph` and precached `fonts/NotoSerifHebrew-Taamim.ttf`** — CLAUDE.md now carries the rule; A/E/B/K should look at that surface (shared-block sha parity of the keyboard CSS across carriers especially).

**⚑ THE STALEST PASS IS I (S339), THEN C (S340), F (S341), B (S342), A (S343), E (S344), L (S345), O (S346), K (S347), N (S348), M (S349), H (S350), G (S351), D (S352).** Take I (first-load & empty-state, every tool in a fresh context — 27th run; weight the surfaces `f2384a7` touched: the keyboard's Trop tab, the Trope Tutor tiles, the Torah Trainer verses on a virgin profile). **O is attended-only; its next surface is `torah_trainer.html`.** **K owes its next run two arms** (runtime HE census; whole-corpus plain-argument probe). **N's next surface is `flash_cards.html` (S244).** **H's next is likely the generator (S251) — verify.** **G's next is the dashboard (S231).** **D's next is trope_tutor (S232).** Harness (standing): O detector = the `skill-v4.1.3` tag; torah verification stubs Sefaria by fulfilling `**/*sefaria.org/**` (calendars → `{calendar_items:[{title:{en:'Parashat Hashavua'},displayValue:{en:'Bereshit'},ref:'Genesis 1:1-6:8'}]}`, v3 texts → 2-D per chapter for a cross-chapter ref); hidden toggle inputs → click `label.toggle:has(#id)`; the HTTP server does not survive a compaction or a long idle — curl it before every Playwright run; pypdf imports only with a stub `cryptography` package on `sys.path` (scratchpad d352/pdftext.py); a print path is measured through the page's own button with `window.print` stubbed to dispatch `beforeprint`. **Ledger lesson (standing):** write open candidates without the `NON_ACTIONABLE` phrases; run `compact-ledger --apply` ONCE.

**⚑ TWO UNGATED CANDIDATES ARE OPEN — take them in order:** (1) trope 13-sheet chart compaction (P3, print-only CSS; render and inspect — the Hebrew examples must stay legible); (2) hub `#ieTextarea` `dir="ltr"` (P4, one attribute, S352). The S285 tooltip-binding divergence (P4) stays filed for F; every other candidate is a gate or a decision. **Seed bench:** the tool-card data-indicator seed (index, S; dual-audience), the submit-a-font form (resources, M), the dashboard per-day overrides (M) — SHIPPED: scroll-style handout S351 (`dca9609`), erase-offers-backup S350 (`aab1392`), handout selection S349 (`c3d2e11`), holiday share link S348 (`4329006`).

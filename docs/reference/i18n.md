...
- **Dynamic JS** → `I18n.t('key')` or `I18n.t('key', {name, count})` (`{placeholder}` substitution). This
  ...
  string** (and `console.warn`s), so a raw `page.feature.x` on screen means an unbuilt/typo'd key. A caller
  whose fallback for an absent key is by design (the Font Maker's `gName()`: its non-Hebrew glyph tables have
  no rows) asks **`I18n.has(key)`** first, which never warns, so the warning keeps meaning a key is missing.
...
3. **`js/i18n.js`** — add `'X'` to `SUPPORTED`; if right-to-left, add it to `RTL_LANGS` (lang→dir is
   list-driven — no other dir code); add a `SWITCHER_LANGS` entry (`{code, label, aria}`) and, if the language has one, a `FLAGS[code]` SVG (the closed switcher shows it beside the label; without one the label stands alone) — the switcher on

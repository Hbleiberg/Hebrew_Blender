...
| `docs/tropepatterns.md` | The printed cantillation chart transcribed note for note from clean scans: the 41 Torah and 33 High Holiday phrase patterns as machine-read row blocks (the source of `trope_phrases.json`, and of the per-mark figures in `trope_motifs.json` and `trope_motifs_hh.json`), what the chart teaches about context, and what a staff of a whole parasha needs |
| `data/trope/trope_phrases.json` | Every row of both printed charts as data — notes, syllables, triplets, ties and slurs — plus each mark's figures by context, for a future staff of a whole reading (not fetched by any page yet) |
| `scripts/build-trope-phrases.mjs` | Builds `trope_phrases.json` from the row blocks in `docs/tropepatterns.md` (plain Node, zero deps) and checks every Trope Tutor staff against its chart row; `--census` counts which mark contexts the Torah text needs against what the chart prints |
